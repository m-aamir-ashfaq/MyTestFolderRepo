...
// -----------------------------------------
//
// Source: MoviePlayback2.java
// Date  : 03 Nov 2016 15:24:14 ECT
 ...
 * Automatically generated load test program.
 */
public class MoviePlayback2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "MoviePlayback2ExtParams.dat";        // input file name for very large parameter values
	...
	private Object plugin0003 = null;                             // plug-in #3: "movieIdChooserScript" - definition from web admin var handler: construction scope = per loop
	private static String plugin0003movieIdChooserScript = null;  // internal - not used in this load test program, but used by the GUI - reflective information
	private Object plugin0002 = null;                             // plug-in #2: "authTokenChooserScript" - definition from web admin var handler: construction scope = per loop
	private static String plugin0002authTokenChooserScript = null;// internal - not used in this load test program, but used by the GUI - reflective information
	private Object plugin0001 = null;                             // plug-in #1: "assetIdSubtitleIdChooserScript" - definition from web admin var handler: construction scope = per loop
	private static String plugin0001assetIdSubtitleIdChooserScript = null;// internal - not used in this load test program, but used by the GUI - reflective information
	 ...
	 */
	public MoviePlayback2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		try
		{
			...
			Class pluginClass0 = getPluginClassLoader().getPluginClass("movieIdChooserScript");
			plugin0003 = pluginClass0.newInstance();
			((GenericPluginInterface) plugin0003).construct(loopPluginContext);
			
			// define object for plug-in #2 and call plug-in constructor
			Class pluginClass1 = getPluginClassLoader().getPluginClass("authTokenChooserScript");
			...
			Class pluginClass2 = getPluginClassLoader().getPluginClass("assetIdSubtitleIdChooserScript");
			plugin0001 = pluginClass2.newInstance();
		...
		// execute plug-in #3 at start of loop
		log(">>> execute plug-in #3: movieIdChooserScript");
		((GenericPluginInterface) plugin0003).setInputParameter(0, env);		// set plug-in input parameter
		((GenericPluginInterface) plugin0003).setInputParameter(2, staging_movie_id);		// set plug-in input parameter
		((GenericPluginInterface) plugin0003).setInputParameter(1, prod_movie_id);		// set plug-in input parameter
		((GenericPluginInterface) plugin0003).execute(loopPluginContext);			// execute plug-in
		...
		movieId = (String) ((GenericPluginInterface) plugin0003).getOutputParameter(0);		// get plug-in output parameter
		log("<<< movieId = " + movieId);
		
		// execute plug-in #2 at start of loop
		log(">>> execute plug-in #2: authTokenChooserScript");
		((GenericPluginInterface) plugin0002).setInputParameter(2, staging_bearer_token);		// set plug-in input parameter
		((GenericPluginInterface) plugin0002).setInputParameter(1, production_bearer_token);		// set plug-in input parameter
		((GenericPluginInterface) plugin0002).setInputParameter(0, env);		// set plug-in input parameter
		((GenericPluginInterface) plugin0002).execute(loopPluginContext);			// execute plug-in
		...
		bearer_token = (String) ((GenericPluginInterface) plugin0002).getOutputParameter(0);		// get plug-in output parameter
		log("<<< bearer_token = " + bearer_token);
		
		// execute plug-in #1 at start of loop
		log(">>> execute plug-in #1: assetIdSubtitleIdChooserScript");
		((GenericPluginInterface) plugin0001).setInputParameter(1, prod_assetId);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).setInputParameter(3, staging_assetId);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).setInputParameter(2, prod_subtitleId);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).setInputParameter(4, staging_subtitleId);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).setInputParameter(0, env);		// set plug-in input parameter
		...
		assetId_with_subtitles = (String) ((GenericPluginInterface) plugin0001).getOutputParameter(0);		// get plug-in output parameter
		log("<<< assetId_with_subtitles = " + assetId_with_subtitles);
		subtitleId = (String) ((GenericPluginInterface) plugin0001).getOutputParameter(1);		// get plug-in output parameter
		log("<<< subtitleId = " + subtitleId);
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("MoviePlayback2", concurrentUsers);
		...
		performanceData.setInfoText("MoviePlayback2");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new MoviePlayback2());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				MoviePlayback2 simulatedUser = new MoviePlayback2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
