...
// -----------------------------------------
//
// Source: demo8_IO.java
// Date  : 11 Aug 2016 10:00:59 ECT
 ...
 * Automatically generated load test program.
 */
public class demo8_IO extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "demo8_IOExtParams.dat";              // input file name for very large parameter values
	 ...
	 */
	public demo8_IO(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log();
		log();
		log("# Page #2: move away");
		log("# ------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: move away", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "socket.leftronic.com";
		...
		// verify response: status code = 200, content type = "TEXT/PLAIN", header text fragment = [verification disabled], recorded content size = 71
		// content test algorithm: check size +/- 5%, defined content size = 71
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/PLAIN", null, 71, 5);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		String requestFile0002 = "/socket.io/1/websocket/18316928541233246933";
		String requestHeader0002 = "GET " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: SaXOck81JANXGYK0Ikd4rA==\r\n" + 
		...
		webSocketData = getWebSocketData_1470901503461();
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [4] <- WEB ADMIN Index 24 ---
		log();
		log("# title: Home | Zauq Group");
		String requestProt0004 = "http";
		String requestHost0004 = "zauq.com";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 5060
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div class=\"collapse navbar-collapse navbar-right\"");		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [5] <- WEB ADMIN Index 25 ---
		String requestProt0005 = "http";
		String requestHost0005 = "zauq.com";
		int    requestPort0005 = 80;
		String requestFile0005 = "/js/pace.min.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 4860, 5);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
		...
		// --- HTTP REQUEST: Test [6] <- WEB ADMIN Index 26 ---
		String requestProt0006 = "http";
		String requestHost0006 = "zauq.com";
		int    requestPort0006 = 80;
		String requestFile0006 = "/css/pace.css";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 205, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [7] <- WEB ADMIN Index 27 ---
		String requestProt0007 = "http";
		String requestHost0007 = "zauq.com";
		int    requestPort0007 = 80;
		String requestFile0007 = "/css/animate.min.css";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 6624, 5);		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [8] <- WEB ADMIN Index 28 ---
		String requestProt0008 = "http";
		String requestHost0008 = "zauq.com";
		int    requestPort0008 = 80;
		String requestFile0008 = "/css/bootstrap.min.css";
		...
		// verify response: status code = 200, content type = "TEXT/CSS", header text fragment = [verification disabled], recorded content size = 21176
		// content test algorithm: check size +/- 5%, defined content size = 21176
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 21176, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [9] <- WEB ADMIN Index 29 ---
		String requestProt0009 = "http";
		String requestHost0009 = "zauq.com";
		int    requestPort0009 = 80;
		String requestFile0009 = "/css/prettyPhoto.css";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 3472, 5);		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [10] <- WEB ADMIN Index 30 ---
		String requestProt0010 = "http";
		String requestHost0010 = "zauq.com";
		int    requestPort0010 = 80;
		String requestFile0010 = "/css/font-awesome.min.css";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 4500, 5);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [11] <- WEB ADMIN Index 31 ---
		String requestProt0011 = "http";
		String requestHost0011 = "zauq.com";
		int    requestPort0011 = 80;
		String requestFile0011 = "/js/jquery.prettyPhoto.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 6948, 5);		// Test [11] <- Index 11
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [12] <- WEB ADMIN Index 32 ---
		...
		// verify response: status code = 200, content type = "APPLICATION/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 38206
		// content test algorithm: check size +/- 5%, defined content size = 38206
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 38206, 5);		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [13] <- WEB ADMIN Index 33 ---
		String requestProt0013 = "http";
		String requestHost0013 = "zauq.com";
		int    requestPort0013 = 80;
		String requestFile0013 = "/css/responsive.css";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 1231, 5);		// Test [13] <- Index 13
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 10912, 5);		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0015 = "/js/main.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 833, 5);		// Test [15] <- Index 15
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [16] <- WEB ADMIN Index 36 ---
		String requestProt0016 = "http";
		String requestHost0016 = "zauq.com";
		int    requestPort0016 = 80;
		String requestFile0016 = "/js/bootstrap.min.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 8684, 5);		// Test [16] <- Index 16
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 1858, 5);		// Test [17] <- Index 17
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [18] <- WEB ADMIN Index 38 ---
		String requestProt0018 = "http";
		String requestHost0018 = "zauq.com";
		int    requestPort0018 = 80;
		String requestFile0018 = "/js/jquery.isotope.min.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 5729, 5);		// Test [18] <- Index 18
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("demo8_IO", concurrentUsers);
		...
		performanceData.setInfoText("demo8_IO");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				demo8_IO simulatedUser = new demo8_IO(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	public static WebSocketData getWebSocketData_1470901503461()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1470901503580L);
		webSocketData.setConnectionStopMillis(1470901537024L);
		String payLoad_0 = "1::";
		String payLoad_1 = "5:::{\"name\":\"signature\",\"args\":[\"/node?dashboard=DiWAJ5&group=0&shared=1&time=1470901531&user=35632&signature=7f10fa348dfdc7ebcaeadb2e37970390e1b0c3ce\"]}";
		...
		String payLoad_7 = "2::";
		String payLoad_8 = "2::";
		String payLoad_9 = "5:::{\"name\":\"ping\",\"args\":[null]}";
		String payLoad_10 = "4:::{\"pong\":true}";
		String payLoad_11 = "\u0003\u00fd";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , false , (byte) 1 , 1470901503583L,1470901503585L , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1470901503591L,1470901503592L , payLoad_1 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1470901503593L,1470901503594L , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , false , (byte) 1 , 1470901503707L,1470901503707L , payLoad_3 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1470901503708L,1470901503709L , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , false , (byte) 1 , 1470901503819L,1470901503820L , payLoad_5 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1470901504321L,1470901504321L , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , false , (byte) 1 , 1470901528579L,1470901528580L , payLoad_7 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1470901528581L,1470901528582L , payLoad_8 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1470901533592L,1470901533593L , payLoad_9 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , false , (byte) 1 , 1470901533709L,1470901533710L , payLoad_10 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1470901536904L,1470901536904L , payLoad_11 ));
