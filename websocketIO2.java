...
// -----------------------------------------
//
// Source: websocketIO2.java
// Date  : 15 Jul 2016 11:09:04 ECT
 ...
 * Automatically generated load test program.
 */
public class websocketIO2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "websocketIO2ExtParams.dat";          // input file name for very large parameter values
	...
	private static String name = "AMIR";                                         // var declaration from web admin var handler: scope = global
	 ...
	 */
	public websocketIO2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log();
		log();
		log("# Page #2: moving away");
		log("# --------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: moving away", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
		...
		log("# Page #3: done");
		log("# -------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #3: done", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 120, 5);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 3507, 5);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0003 = "/r/collect" +
				"?v=1" +
				"&_v=j44" +
				"&a=388208870" +
				"&t=pageview" +
		...
		// verify response: status code = 302, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 366
		...
		String requestHost0004 = "stats.g.doubleclick.net";
		int    requestPort0004 = 443;
		String requestFile0004 = "/r/collect" +
				"?v=1" +
				"&aip=1" +
				"&t=dc" +
				"&_r=3" +
		...
		String requestHost0005 = "assets.zendesk.com";
		int    requestPort0005 = 80;
		String requestFile0005 = "/embeddable_framework/update.html" +
				"?1468573358688";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 206, 5);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0006 = "/embeddable/blips";
		String requestContent0006 = 
				"{\"url\":\"http://www.websocket.org/echo.html\",\"buid\":\"f15b0390b97eb4d94abfa46559b7e0c3\",\"suid\":\"c7d844c88ebaadfcbae0609fbd25ee63\",\"version\":\"e6d767c\",\"timestamp\":\"2016-07-15T09:02:38.692Z\",\"pageView\":{\"referrer\":\"http://www.websocket.org/echo.html\",\"time\":0,\"loadTime\":20.683389,\"navigatorLanguage\":\"en-GB\",\"pageTitle\":\"websocket.org Echo Test - Powered by Kaazing\",\"userAgent\":\"Mozilla/5.0 (Macintosh; Intel Mac OS X 10.11; rv:47.0) Gecko/20100101 Firefox/47.0\",\"helpCenterDedup\":false}}";
		String requestHeader0006 = "POST " + requestFile0006 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Content-Length: " + requestContent0006.length() + "\r\n" + 		// Content-Length: 486
				"Origin: http://www.websocket.org\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0006, requestHost0006, requestPort0006, requestHeader0006, requestContent0006.getBytes(), requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		requestContent0006 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {202}, "TEXT/PLAIN", null, 0, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0007 = "v2.zopim.com";
		int    requestPort0007 = 80;
		String requestFile0007 = "/" +
		...
		String requestHost0008 = "echo.websocket.org";
		...
		String requestHeader0008 = "GET " + requestFile0008 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: 2O5GhLW6cUgz+l9ENAeAKQ==\r\n" + 
		...
		webSocketData = getWebSocketData_1468573362213();
		...
		// content test algorithm: check size +/- 5%, defined content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {101}, null, null, 0, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0009 = "ie02.zopim.com";
		int    requestPort0009 = 443;
		String requestFile0009 = "/s/W/ws/dPYcH6sE8J3A9Axl/c/1468573359824";
		String requestHeader0009 = "GET " + requestFile0009 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Origin: http://www.websocket.org\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: XjH872hzDSjqmU3maLJoEA==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		webSocketData = getWebSocketData_1468573362280();
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0011 = "www.google.com";
		int    requestPort0011 = 443;
		String requestFile0011 = "/complete/search" +
				"?client=firefox" +
				"&q=za";
		...
		// verify response: status code = 200, content type = "TEXT/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 101
		// content test algorithm: check size +/- 5%, defined content size = 101
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 101, 5);		// Test [11] <- Index 11
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [12] <- WEB ADMIN Index 46 ---
		log();
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 5026
		...
		// --- HTTP REQUEST: Test [13] <- WEB ADMIN Index 47 ---
		String requestProt0013 = "https";
		String requestHost0013 = "ie02.zopim.com";
		int    requestPort0013 = 443;
		String requestFile0013 = "/s/W/ws/dPYcH6sE8J3A9Axl/c/1468573465371";
		String requestHeader0013 = "GET " + requestFile0013 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Origin: http://www.websocket.org\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: 8WqGAHYaFOhgcPDxxttczg==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		webSocketData = getWebSocketData_1468573465558();
		...
		// --- HTTP REQUEST: Test [14] <- WEB ADMIN Index 48 ---
		log();
		log("# title: 302 Moved");
		String requestProt0014 = "https";
		String requestHost0014 = "www.google-analytics.com";
		int    requestPort0014 = 443;
		String requestFile0014 = "/r/collect" +
				"?v=1" +
				"&_v=j44" +
				"&a=1619974426" +
				"&t=pageview" +
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {302}, null, null, null);		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [15] <- WEB ADMIN Index 49 ---
		String requestProt0015 = "https";
		String requestHost0015 = "stats.g.doubleclick.net";
		int    requestPort0015 = 443;
		String requestFile0015 = "/r/collect" +
				"?v=1" +
				"&aip=1" +
				"&t=dc" +
				"&_r=3" +
		...
		// verify response: status code = 200, content type = "IMAGE/GIF", header text fragment = [verification disabled], recorded content size = 35
		// content test algorithm: check size +/- 5%, defined content size = 35
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/GIF", null, 35, 5);		// Test [15] <- Index 15
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("websocketIO2", concurrentUsers);
		...
		performanceData.setInfoText("websocketIO2");
		...
		{
			System.out.println("global var <<< name = " + name);
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				websocketIO2 simulatedUser = new websocketIO2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	public static WebSocketData getWebSocketData_1468573362213()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1468573362318L);
		webSocketData.setConnectionStopMillis(1468573465317L);
		...
		String payLoad_6 = "bye";
		String payLoad_7 = "bye";
		String payLoad_8 = "\u0003\u00fd";
		String payLoad_9 = "\u0003\u00fd";

		// search and replace frame payload "HTML" with variable 'name' on all occurrences of all frames.
		payLoad_0 = Lib.replaceTextPattern(payLoad_0, "HTML", name, 0);
		payLoad_1 = Lib.replaceTextPattern(payLoad_1, "HTML", name, 0);
		payLoad_2 = Lib.replaceTextPattern(payLoad_2, "HTML", name, 0);
		payLoad_3 = Lib.replaceTextPattern(payLoad_3, "HTML", name, 0);
		payLoad_4 = Lib.replaceTextPattern(payLoad_4, "HTML", name, 0);
		payLoad_5 = Lib.replaceTextPattern(payLoad_5, "HTML", name, 0);
		payLoad_6 = Lib.replaceTextPattern(payLoad_6, "HTML", name, 0);
		payLoad_7 = Lib.replaceTextPattern(payLoad_7, "HTML", name, 0);
		payLoad_8 = Lib.replaceTextPattern(payLoad_8, "HTML", name, 0);
		payLoad_9 = Lib.replaceTextPattern(payLoad_9, "HTML", name, 0);

		// search and replace frame payload "it" with variable 'var1' on all occurrences of all frames.
		payLoad_0 = Lib.replaceTextPattern(payLoad_0, "it", var1, 0);
		payLoad_1 = Lib.replaceTextPattern(payLoad_1, "it", var1, 0);
		payLoad_2 = Lib.replaceTextPattern(payLoad_2, "it", var1, 0);
		payLoad_3 = Lib.replaceTextPattern(payLoad_3, "it", var1, 0);
		payLoad_4 = Lib.replaceTextPattern(payLoad_4, "it", var1, 0);
		payLoad_5 = Lib.replaceTextPattern(payLoad_5, "it", var1, 0);
		payLoad_6 = Lib.replaceTextPattern(payLoad_6, "it", var1, 0);
		payLoad_7 = Lib.replaceTextPattern(payLoad_7, "it", var1, 0);
		payLoad_8 = Lib.replaceTextPattern(payLoad_8, "it", var1, 0);
		payLoad_9 = Lib.replaceTextPattern(payLoad_9, "it", var1, 0);


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573366385L,1468573366386L , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1468573366493L,1468573366495L , payLoad_1 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573371234L,1468573371235L , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1468573371341L,1468573371343L , payLoad_3 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573384515L,1468573384516L , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1468573384625L,1468573384627L , payLoad_5 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573452371L,1468573452371L , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1468573452476L,1468573452477L , payLoad_7 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 8 , 1468573465208L,1468573465209L , payLoad_8 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 8 , 1468573465315L,1468573465316L , payLoad_9 ));
		return webSocketData;
	}
	
	// source code getting the webSocket context.
	public static WebSocketData getWebSocketData_1468573362280()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1468573362334L);
		webSocketData.setConnectionStopMillis(1468573465267L);
		String payLoad_0 = "1468573362272\n2447\n1\n0\nn\n30000";
		String payLoad_1 = "1468573362339\n0\n1\n0\nd\n{\"__type\":\"register\",\"accountKey\":\"3vCaGRDlZbbiX5tJXSlIABEcEDu5cGAg\",\"cookie_law\":false,\"dt\":\"desktop\",\"mID\":\"bUfgAhQNNLqO0c\",\"ref\":\"\",\"rev\":\"474f203da1ffd43d29a9d7241197dbc8fac35a05\",\"source\":\"widget_v2\",\"theme\":\"simple\",\"title\":\"websocket.org Echo Test - Powered by Kaazing\",\"ua\":\"Mozilla/5.0 (Macintosh; Intel Mac OS X 10.11; rv:47.0) Gecko/20100101 Firefox/47.0\",\"url\":\"http://www.websocket.org/echo.html\"}";
		String payLoad_2 = "1468573362322\n2447\n2\n0\nd\n{\"path\":\"connection\",\"update\":{\"status$string\":\"connected\",\"server$string\":\"ie02.zopim.com\",\"server_retired$bool\":false,\"server_ready$bool\":false}}";
		String payLoad_3 = "1468573362390\n2443.7310252996003\n3\n1\nd\n{\"path\":\"livechat.settings.package\",\"update\":{\"color_customization_enabled$int\":1,\"widget_customization_enabled$int\":1},\"seq\":2}\n{\"path\":\"livechat.settings.file_sending\",\"update\":{\"allowed_extensions$string\":\"png,jpg,jpeg,gif,txt,pdf,zip,rar,html,java,class,exe,cs,xml,css,db,pw,ppk,pem,jar,dll,bat,js,m,h,framework\",\"enabled$bool\":true},\"seq\":3}\n{\"path\":\"livechat\",\"update\":{\"features\":null},\"seq\":4}\n{\"path\":\"livechat\",\"update\":{\"features\":{\"agents_dept_view$bool\":true,\"monitor$bool\":true,\"file_sending_settings$bool\":true,\"agent_list$bool\":true,\"tagsv2$bool\":true,\"new_label\":{\"chat_tags\":{\"end$string\":\"2015-12-05\",\"start$string\":\"2015-11-05\"}},\"analytics$bool\":true,\"timezone_migration_notice$bool\":true,\"testabc$bool\":true,\"skip_dept_online_validation$bool\":true,\"nps\":{\"start$string\":\"2016-04-25 00:00Z\",\"end$string\":\"2016-05-01 23:59Z\"},\"test_feature\":{\"setA\":{\"when$string\":\"at_chat_end\",\"cond$string\":\"visitor_id_divided_by_2\"},\"setB\":{\"when$string\":\"at_chat_end\",\"cond$string\":\"visitor_id_not_divided_by_2\"}},\"transfer_department$bool\":true,\"request_rating$bool\":true}},\"seq\":5}";
		String payLoad_4 = "1468573362791\n2443.7310252996003\n4\n1\nd\n{\"path\":\"livechat.settings\",\"update\":{\"sound\":{\"disabled$bool\":false},\"rating\":{\"enabled$bool\":true},\"language\":{\"language$string\":\"--\"},\"bubble\":{\"title$string\":\"Questions?\",\"enabled$bool\":true,\"text$string\":\"Click here to chat with us.\"},\"chat_window\":{\"mobile_mode$string\":\"overlay\",\"title_bar\":{\"title$string\":\"support\"}},\"forms\":{\"pre_chat_form\":{\"message$string\":\"\",\"required$bool\":false,\"form\":{\"0\":{\"name$string\":\"name\",\"required$bool\":false},\"1\":{\"name$string\":\"email\",\"required$bool\":false},\"2\":{\"label$string\":\"Choose a Department\",\"name$string\":\"department\",\"required$bool\":false,\"type$string\":\"department\"},\"3\":{\"label$string\":\"Message\",\"name$string\":\"message\",\"required$bool\":false,\"type$string\":\"textarea\"},\"4\":{\"label$string\":\"Phone\",\"hidden$bool\":true,\"type$string\":\"text\",\"name$string\":\"phone\",\"required$bool\":false}},\"profile_required$bool\":false},\"offline_form\":{\"message$string\":\"Sorry, we aren't online at the moment. Leave a message and we'll get back to you.\",\"form\":{\"3\":{\"hidden$bool\":true,\"required$bool\":false}}}},\"theme\":{\"colors\":{\"bubble$string\":\"#e59341\",\"banner$string\":\"#eeeeee\",\"primary$string\":\"#f47d31\"},\"chat_window\":{\"size$string\":\"medium\",\"position$string\":\"br\"},\"name$string\":\"zendesk\",\"message_type$string\":\"bubble_avatar\",\"chat_button\":{\"position_mobile$string\":\"br\",\"position$string\":\"br\"}},\"concierge\":{\"title$string\":\"Ask us anything.\",\"avatar_path$string\":\"\",\"display_name$string\":\"Kaazing Global Support\"},\"behavior\":{\"do_not_display$bool\":false},\"login\":{\"restrict_profile$bool\":false,\"phone_display$bool\":false,\"allowed_types\":{\"google$bool\":true,\"facebook$bool\":true}},\"banner\":{\"layout$string\":\"image_right\",\"image_path$string\":\"\",\"enabled$bool\":true,\"text$string\":\"Chat with us.\"},\"chat_button\":{\"hide_when_offline$bool\":true},\"greetings\":{\"offline$string\":\"Leave a Message\",\"online$string\":\"Chat With Us\"},\"branding\":{\"hide_branding$bool\":true}},\"seq\":6}\n{\"path\":\"livechat\",\"update\":{\"departments\":null},\"seq\":7}\n{\"path\":\"livechat\",\"update\":{\"account\":{\"key$string\":\"3vCaGRDlZbbiX5tJXSlIABEcEDu5cGAg\",\"status$string\":\"online\"}},\"seq\":8}\n{\"path\":\"livechat.profile\",\"update\":{\"sid$string\":\"160715.1702333.6j13ZPpsrtsj1mlh6\",\"mid$string\":\"bUfgAhQNNLqO0c\",\"nick$string\":\"visitor:1702333-bUfgAhQNNLqO0c\",\"email$string\":\"\",\"display_name$string\":\"Visitor 68865169\"},\"seq\":9}\n{\"path\":\"livechat\",\"update\":{\"agents\":null},\"seq\":10}\n{\"path\":\"livechat.channel\",\"update\":{\"chatting$bool\":false,\"rating$string\":null,\"comment$string\":null,\"department_id$int\":null,\"start$time\":null,\"last_read\":null},\"seq\":11}\n{\"path\":\"connection\",\"update\":{\"status$string\":\"registered\",\"registered_timestamp$int\":1468573362754},\"seq\":12}\n{\"path\":\"livechat.profile\",\"update\":{\"uid$string\":\"ig1IXj6vDgrOBlSV\"},\"seq\":13}\n{\"path\":\"connection\",\"update\":{\"status$string\":\"reattached\",\"server_ready$bool\":true,\"reattached_timestamp$int\":1468573362754},\"seq\":14}\n{\"path\":\"livechat.profile\",\"update\":{\"phone$string\":\"\"},\"seq\":15}";
		String payLoad_5 = "1468573363273\n70.19778571021828\n2\n4\nd\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"start_exec\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:474f203da1ffd43d29a9d7241197dbc8fac35a05\"],\"value\":0.294},{\"method\":\"increment\",\"name\":\"conn\",\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:474f203da1ffd43d29a9d7241197dbc8fac35a05\",\"tries:1\"],\"value\":1},{\"method\":\"histogram\",\"name\":\"conn_open\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:474f203da1ffd43d29a9d7241197dbc8fac35a05\",\"proxy:ie02.zopim.com\"],\"value\":2.514},{\"method\":\"histogram\",\"name\":\"conn_status_connected\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:474f203da1ffd43d29a9d7241197dbc8fac35a05\"],\"value\":2.596}]}\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"conn_status_registered\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:474f203da1ffd43d29a9d7241197dbc8fac35a05\"],\"value\":3.09}]}\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"conn_status_reattached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:474f203da1ffd43d29a9d7241197dbc8fac35a05\"],\"value\":3.091}]}\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"active$int\":1468573359825}}\n{\"path\":[\"root\",\"livechat\",\"channel\",\"tags\"],\"value\":{\"added$string\":\"websocket_org\"}}\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"disconnect_timeout$int\":36}}\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"init_ui_components_no_cached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:474f203da1ffd43d29a9d7241197dbc8fac35a05\"],\"value\":3.092}]}";
		String payLoad_6 = "1468573363311\n70.19778571021828\n3\n4\nd\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"theme_loaded_no_cached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:474f203da1ffd43d29a9d7241197dbc8fac35a05\"],\"value\":3.474}]}";
		String payLoad_7 = "1468573379829\n70.19778571021828\n4\n4\nd\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"active$int\":1468573379827}}";
		String payLoad_8 = "1468573399831\n70.19778571021828\n5\n4\nd\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"active$int\":1468573399830}}";
		String payLoad_9 = "1468573429837\n70.19778571021828\n6\n4\nd\nnull";
		String payLoad_10 = "1468573439846\n70.19778571021828\n7\n4\nd\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"active$int\":1468573439839}}";
		String payLoad_11 = "1468573459850\n70.19778571021828\n8\n4\nd\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"active$int\":1468573459844}}";
		String payLoad_12 = "\u0003\u00fd";
		String payLoad_13 = "\u0003\u00fd";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1468573362337L,1468573362338L , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573362340L,1468573362341L , payLoad_1 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1468573362386L,1468573362388L , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1468573362453L,1468573362455L , payLoad_3 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1468573362879L,1468573362881L , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573363273L,1468573363276L , payLoad_5 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573363311L,1468573363313L , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573379830L,1468573379832L , payLoad_7 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573399831L,1468573399833L , payLoad_8 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573429838L,1468573429839L , payLoad_9 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573439847L,1468573439849L , payLoad_10 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468573459850L,1468573459853L , payLoad_11 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 8 , 1468573465208L,1468573465211L , payLoad_12 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 8 , 1468573465265L,1468573465266L , payLoad_13 ));
		return webSocketData;
	}
	
	// source code getting the webSocket context.
	public static WebSocketData getWebSocketData_1468573465558()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1468573465614L);
		webSocketData.setConnectionStopMillis(1468573465716L);
		String payLoad_0 = "1468573465550\n382.22243560138054\n5\n8\np\n30000";
		String payLoad_1 = "\u0003\u00fd";
		String payLoad_2 = "\u0003\u00fd";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1468573465615L,1468573465617L , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 8 , 1468573465661L,1468573465662L , payLoad_1 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 8 , 1468573465715L,1468573465716L , payLoad_2 ));
		return webSocketData;
	}
