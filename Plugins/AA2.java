...
//
// Source: AA2.java
// Date  : 05 Oct 2016 09:18:09 ECT
// Author: Apica ZebraTester V5.4-Q / automatically generated
//
 ...
 * Automatically generated load test program.
 */
public class AA2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	public static final String prxVersion = "V5.4-Q";
	...
	private static final String EXTERNAL_PARAM_FILE = "AA2ExtParams.dat";                   // input file name for very large parameter values
	 ...
	 */
	public AA2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		if (!urlCallPassed)
		{
			...
			((GenericPluginInterface) plugin0001).setInputParameter(0, time);		// set plug-in input parameter
			...
			kkj = (String) ((GenericPluginInterface) plugin0001).getOutputParameter(0);		// get plug-in output parameter
			log("<<< kkj = " + kkj);
		...
		// verify response: status code = 101, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {101}, null, null, 3, 0,"xtra");		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("AA2", concurrentUsers);
		...
		performanceData.setInfoText("AA2");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				AA2 simulatedUser = new AA2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
