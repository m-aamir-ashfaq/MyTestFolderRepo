...
// -----------------------------------------
//
// Source: TV4_stream_rev2.java
// Date  : 11 Mar 2016 14:08:25 ECT
 ...
 * Automatically generated load test program.
 */
public class TV4_stream_rev2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "se.tv4.tv4play/1.0 iOS/9.0 AppleTV/9.0 model/AppleTV5,3 hwp/t7000 build/13T396 (3; dt:119)";
	public static String USER_AGENT_2 = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.11; rv:43.0) Gecko/20100101 Firefox/43.0";
	...
	public static final String USER_INPUT_FIELD_chunks = "AQAGY2h1bmtzAQAGY2h1bmtzAQABMw@@";		// internal object-string representation of user input field 'chunks'
	...
	private static final String EXTERNAL_PARAM_FILE = "TV4_stream_rev2ExtParams.dat";       // input file name for very large parameter values
	...
	private static String chunks = null;                                         // var declaration from web admin var handler: scope = global
	...
	private static Object plugin0001 = null;                      // plug-in #1: "AssignVarToUrlPath" - definition from web admin var handler: construction scope = global
	private static String plugin0001AssignVarToUrlPath = null;    // internal - not used in this load test program, but used by the GUI - reflective information

	private String allchunks = null;                     // var declaration from web admin var handler: scope = per loop
	private String allQualities = null;                  // var declaration from web admin var handler: scope = per loop
	private String desiredchunk = null;                  // var declaration from web admin var handler: scope = per loop
	private String masterManifest = null;                // var declaration from web admin var handler: scope = per loop
	private String quality = null;                       // var declaration from web admin var handler: scope = per loop
	
	private String chunkindex = null;                    // var declaration from web admin var handler: scope = per inner loop
	
	private Object plugin0002 = null;                             // plug-in #2: "GetQuality" - definition from web admin var handler: construction scope = per loop
	private static String plugin0002GetQuality = null;            // internal - not used in this load test program, but used by the GUI - reflective information
	private Object plugin0003 = null;                             // plug-in #3: "GetChunks" - definition from web admin var handler: construction scope = per loop
	private static String plugin0003GetChunks = null;             // internal - not used in this load test program, but used by the GUI - reflective information


	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public TV4_stream_rev2()
	{
		super();
	}
	 ...
	 */
	public TV4_stream_rev2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		quality = null;
		log();
		log("<<< allchunks = " + allchunks);
		log("<<< allQualities = " + allQualities);
		log("<<< desiredchunk = " + desiredchunk);
		log("<<< masterManifest = " + masterManifest);
		log("<<< quality = " + quality);
		log("<<< chunks = " + chunks);
		...
		try
		{
			...
			Class pluginClass0 = Class.forName("GetQuality");
			...
			Class pluginClass1 = Class.forName("GetChunks");
			plugin0003 = pluginClass1.newInstance();
			((GenericPluginInterface) plugin0003).construct(loopPluginContext);
		}
		...
		log();
		log();
		log("# Page #1: Choose content");
		log("# -----------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #1: Choose content", 0, 0, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		...
		log();
		log();
		log("# Page #2: Get Manifest");
		log("# ---------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: Get Manifest", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
		...
		log();
		log();
		log("# Page #3: Master manifest");
		log("# ------------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #3: Master manifest", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
		...
		log();
		log();
		log("# Page #4: Choose quality");
		log("# -----------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #4: Choose quality", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
		...
		for (innerLoopCounter1 = 0; innerLoopCounter1 < Integer.valueOf(chunks).intValue(); innerLoopCounter1++)
		{
			...
			log();
			log();
			log("# Page #5: HLS Chunks");
			log("# -------------------");
			threadStep = setPageBreak(performanceData, threadStep, "Page #5: HLS Chunks", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
			pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
			log();

			if (!executePage_5(totalLoopCounter, loopPluginContext, innerLoopContext1, httpLogVectorObject))
			{
				// execution of page 5 failed
				synchResponsesParallelRequestsPage_5(totalLoopCounter, loopPluginContext, innerLoopContext1, httpLogVectorObject);
				return false;
			}
	...
	private void endOfExecuteLoop(boolean loopPassed, HttpTestURL testURL, int threadStep, LoadtestPluginContext loopPluginContext) throws Exception
	{
		...
		((GenericPluginInterface) plugin0003).deconstruct(loopPluginContext);
	}
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "atv-live.tv4play.se";
		int    requestPort0001 = 443;
		String requestFile0001 = "/programs/leila-bakar/episodes/1017580" +
				"?video_ids=" +
				"&retail=false" +
				"&user_state=anonymous";
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: atv-live.tv4play.se\r\n" + 
				"X-Apple-TV-Version: 9.0\r\n" + 
				...
				"X-Apple-Store-Front: 143456-2,33\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/XML", header text fragment = [verification disabled], recorded content size = 2547
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/XML", null, null);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		String requestHost0002 = "atv-live.tv4play.se";
		int    requestPort0002 = 443;
		String requestFile0002 = "/assets/pages/episode-775462f7b4ba2f6c795c9b68c63e649f.js" +
				"?retail=false" +
				"&user_state=anonymous";
		...
		// verify response: status code = 200, content type = "APPLICATION/X-JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 89099
		// content test algorithm: none - no content check
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0004 = "prima.tv4play.se";
		int    requestPort0004 = 443;
		String requestFile0004 = "/api/stb1/asset/1017580/play.json" +
				"?protocol=hls2" +
				"&videoFormat=mp4+webvtts+livehls+m3u8+yshls";
		String requestHeader0004 = "GET " + requestFile0004 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: prima.tv4play.se\r\n" + 
				"X-Apple-TV-Version: 9.0\r\n" + 
				"Accept: */*\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Connection: Keep-Alive\r\n" + 
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 1444
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, null);		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		try
		{
			// extract var 'masterManifest'
			masterManifest = xmlContentParser.getNodeValue("<JSON><playback><items><item><url>", 1);		// recorded value = http://tv4play3-vh.akamaihd.net/i/mp4root/2010-05-26/pid2697563_1017580_,T3MP43,T3MP48,T3MP415,_.mp4.csmil/master.m3u8
			log("<<< masterManifest = " + masterManifest);
		}
		catch (Exception e) { masterManifest = null; log(e); }
		if (masterManifest == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'masterManifest' from xml response data";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
	...
	boolean executePage_3(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0006 = "tv4play3-vh.akamaihd.net";
		int    requestPort0006 = 80;
		String requestFile0006 = "/i/mp4root/2010-05-26/pid2697563_1017580_,T3MP43,T3MP48,T3MP415,_.mp4.csmil/master.m3u8";
		String requestHeader0006 = "GET " + requestFile0006 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: tv4play3-vh.akamaihd.net\r\n" + 
		...
		log(">>> execute plug-in #1: AssignVarToUrlPath");
		synchronized (plugin0001)
		{
			((GenericPluginInterface) plugin0001).setInputParameter(0, masterManifest);		// set plug-in input parameter
			((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0006);			// execute plug-in
		...
		LoadtestPluginContext urlPluginContext0006 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);

		// verify response: status code = 200, content type = "APPLICATION/VND.APPLE.MPEGURL", header text fragment = [verification disabled], recorded content size = 673
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/VND.APPLE.MPEGURL", null, null);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		
			// execute plug-in #2 after url call
			urlPluginContext0006.setUrlPassed(false);
			log(">>> execute plug-in #2: GetQuality");
			((GenericPluginInterface) plugin0002).setInputParameter(0, allQualities);		// set plug-in input parameter
			((GenericPluginInterface) plugin0002).execute(urlPluginContext0006);			// execute plug-in
			...
			quality = (String) ((GenericPluginInterface) plugin0002).getOutputParameter(0);		// get plug-in output parameter
			log("<<< quality = " + quality);
		...
		try
		{
			// extract var 'allQualities'
			allQualities = testURL.getContentString("ISO-8859-1");
			log("<<< allQualities = " + allQualities);
		}
		catch (Exception e) { allQualities = null; log(e); }
		if (allQualities == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'allQualities' from http response content";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		// execute plug-in #2 after url call
		urlPluginContext0006.setUrlPassed(urlCallPassed);
		log(">>> execute plug-in #2: GetQuality");
		((GenericPluginInterface) plugin0002).setInputParameter(0, allQualities);		// set plug-in input parameter
		((GenericPluginInterface) plugin0002).execute(urlPluginContext0006);			// execute plug-in
		if (((GenericPluginInterface) plugin0002).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0002).getLogVector().getLog());
		if (urlPluginContext0006.getSpecificErrorMessage() != null)
		{
			...
			log(urlPluginContext0006.getSpecificErrorMessage());
			performanceData.getPerformanceDataRecord(threadStep).modifyFailureActionType(HttpTestURL.FAILURE_ACTION_HANDLE_AS_ERROR);
			performanceData.setFailed(threadStep, urlPluginContext0006.getSpecificErrorStatusType(), urlPluginContext0006.getSpecificErrorMessage(), testURL, this);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			endOfExecuteLoop(false, testURL, threadStep, loopPluginContext);
			return false;
		}
	...
	boolean executePage_4(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0008 = "tv4play3-vh.akamaihd.net";
		int    requestPort0008 = 80;
		String requestFile0008 = "/i/mp4root/2010-05-26/pid2697563_1017580_,T3MP43,T3MP48,T3MP415,_.mp4.csmil/index_2_av.m3u8";
		requestFile0008 = Lib.replaceTextPattern(requestFile0008, "/i/mp4root/2010-05-26/pid2697563_1017580_,T3MP43,T3MP48,T3MP415,_.mp4.csmil/index_2_av.m3u8", quality, 0);		// search and replace request file fragment "/i/mp4root/2010-05-26/pid2697563_1017580_,T3MP43,T3MP48,T3MP415,_.mp4.csmil/index_2_av.m3u8" with variable 'quality' on all occurrences
		...
		// verify response: status code = 200, content type = "APPLICATION/VND.APPLE.MPEGURL", header text fragment = [verification disabled], recorded content size = 20755
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/VND.APPLE.MPEGURL", null, null);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		try
		{
			// extract var 'allchunks'
			allchunks = testURL.getContentString("ISO-8859-1");
			log("<<< allchunks = " + allchunks);
		}
		catch (Exception e) { allchunks = null; log(e); }
		if (allchunks == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'allchunks' from http response content";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
	...
	boolean executePage_5(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0010 = "tv4play3-vh.akamaihd.net";
		int    requestPort0010 = 80;
		String requestFile0010 = "/i/mp4root/2010-05-26/pid2697563_1017580_,T3MP43,T3MP48,T3MP415,_.mp4.csmil/segment1_2_av.ts";
		requestFile0010 = Lib.replaceTextPattern(requestFile0010, "/i/mp4root/2010-05-26/pid2697563_1017580_,T3MP43,T3MP48,T3MP415,_.mp4.csmil/segment1_2_av.ts", desiredchunk, 0);		// search and replace request file fragment "/i/mp4root/2010-05-26/pid2697563_1017580_,T3MP43,T3MP48,T3MP415,_.mp4.csmil/segment1_2_av.ts" with variable 'desiredchunk' on all occurrences
		...
		LoadtestPluginContext urlPluginBeforeCallContext0010 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #3 before url call
		log(">>> execute plug-in #3: GetChunks");
		((GenericPluginInterface) plugin0003).setInputParameter(0, allchunks);		// set plug-in input parameter
		((GenericPluginInterface) plugin0003).setInputParameter(1, chunkindex);		// set plug-in input parameter
		((GenericPluginInterface) plugin0003).execute(urlPluginBeforeCallContext0010);			// execute plug-in
		...
		desiredchunk = (String) ((GenericPluginInterface) plugin0003).getOutputParameter(0);		// get plug-in output parameter
		log("<<< desiredchunk = " + desiredchunk);
		if (urlPluginBeforeCallContext0010.getContinueInnerLoopFlag() == true)
		{
			log("   - continue of inner loop");
			innerLoopContext.setContinueLoopFlag(true);
			return true;
		}
		if (urlPluginBeforeCallContext0010.getBreakInnerLoopFlag() == true)
		{
			log("   - break of inner loop");
			innerLoopContext.setBreakLoopFlag(true);
			return true;
		}
		...
		// verify response: status code = 200, content type = "VIDEO/MP2T", header text fragment = [verification disabled], recorded content size = 1739000
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "VIDEO/MP2T", null, null);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	public static void main(String[] args)
	{
		...
		if ((ParseArgs.hasOption(args, "-h")) || (ParseArgs.hasOption(args, "-help")))
		{
			...
			System.out.println();
			System.out.println("-chunks <user-input-field>               ->> required argument: chunks");
			System.out.println();
		...
		// parse -chunks argument or ask --> <user-input-field>: proposed value = "3"
		try
		{
			chunks = UserInputField.readFromObjectString(USER_INPUT_FIELD_chunks).readUserInputValue(args);
			if (chunks.startsWith("\"") && chunks.endsWith("\""))
				chunks = chunks.substring(1, chunks.length() - 1);
		}
		catch (IOException ie)
		{
			System.out.println("*** INTERNAL ERROR: invalid user input field object-string declaration for 'chunks' ***");
			ie.printStackTrace();
			System.exit(-2);
		}
		...
		String genericFileName = PerformanceData.proposeFileName("TV4_stream_rev2", concurrentUsers);
		...
		{
			System.out.println("# global var chunks = " + chunks);
		}
		System.out.println();
		...
		performanceData.setInfoText("TV4_stream_rev2");
		performanceData.setExecutorsAnnotation(testRunAnnotation);
		performanceData.addTestDescription("*** inner loop - page #5 to page #5 (loop 1) ***");
		...
		performanceData.addUserInputField("chunks", chunks);		// add user input field name and value to performance data
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new TV4_stream_rev2());
		try
		{
			// define object for plug-in #1 and call plug-in constructor
			Class pluginClass0 = Class.forName("AssignVarToUrlPath");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				TV4_stream_rev2 simulatedUser = new TV4_stream_rev2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
