...
// -----------------------------------------
//
// Source: INLINE_SCRIPT_03.java
// Date  : 22 Jan 2016 13:50:43 ECT
// Author: Apica ZebraTester V5.4-F / automatically generated
//
 ...
 * Automatically generated load test program.
 */
public class INLINE_SCRIPT_03 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	public static final String prxVersion = "V5.4-F";
	...
	private static final String EXTERNAL_PARAM_FILE = "INLINE_SCRIPT_03ExtParams.dat";      // input file name for very large parameter values
	 ...
	 */
	public INLINE_SCRIPT_03(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		socketPool = new HttpSocketPool(sslProtocolVersion, sslSessionCache, sslStatistic, sslcmode);
		...
		int innerLoopEndThreadStep1 = threadStep + 4;		// save threadStep counter for end of this inner loop
		...
		for (innerLoopCounter1 = 0; innerLoopCounter1 < 1; innerLoopCounter1++)
		{
			...
			vInnerLoop2 = System.getProperty("os.name", "Windows");
			vInnerLoop1 = Lib.getNaturalRandomNumberInRangeAsString(1000000l, 9999999l, false);

			log();
			log("<<< vInnerLoop2 = " + vInnerLoop2);
			log("<<< vInnerLoop1 = " + vInnerLoop1);
			log();
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("INLINE_SCRIPT_03", concurrentUsers);
		...
		performanceData.setInfoText("INLINE_SCRIPT_03");
		performanceData.setExecutorsAnnotation(testRunAnnotation);
		performanceData.addTestDescription("*** inner loop - page #2 to page #2 (loop 1) ***");
		...
		if (debugSsl)
			performanceData.addTestDescription("*** warning: debug ssl (https) enabled ***");
		if (!sslProtocolVersion.equalsIgnoreCase("all"))
			performanceData.addTestDescription("*** warning: ssl protocol version fixed to " + sslProtocolVersion + " ***");
		if (sslSessionCacheTimeout == 0)
			performanceData.addTestDescription("*** warning: ssl session cache disabled ***");
		if (singleUserMode)
		{
		...
		// execute inline script "try2nd" at start of test
		LoadtestInlineScriptContext inlineScriptContext_1453467027519 = new LoadtestInlineScriptContext("try2nd", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_GLOBAL_START, "", performanceData, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15);
		InlineScriptExecutor inlineScriptExecutor_1453467027519 = new InlineScriptExecutor(getInlineScriptCode_1453467027519(), inlineScriptContext_1453467027519);
		Throwable inlineScriptThrowable_1453467027519 = null;
		if (debugLoops)
			System.out.println("executing inline script \"" + inlineScriptContext_1453467027519.getScriptTitle() + "\"");
		try
		{
			LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("globalVar1", Lib.nullToBlank(globalVar1), 3);		// note: parameter no. 3 is the scope of the var
			inlineScriptContext_1453467027519.addInputVar(inputVar1);
			LoadtestInlineScriptVar inputVar2 = new LoadtestInlineScriptVar("globalVar2", Lib.nullToBlank(globalVar2), 3);		// note: parameter no. 3 is the scope of the var
			inlineScriptContext_1453467027519.addInputVar(inputVar2);
			LoadtestInlineScriptVar inputVar3 = new LoadtestInlineScriptVar("vInputField", Lib.nullToBlank(vInputField), 3);		// note: parameter no. 3 is the scope of the var
			inlineScriptContext_1453467027519.addInputVar(inputVar3);
			inlineScriptExecutor_1453467027519.execute();		// execute inline script
		}
		catch (Throwable th_1453467027519)
		{
			inlineScriptThrowable_1453467027519 = th_1453467027519;
		}
		if (debugLoops)
		{
			for (String stdoutLine : inlineScriptContext_1453467027519.getOutputStreamData())
				System.out.println(inlineScriptContext_1453467027519.getScriptTitle() + ": " + stdoutLine);
		}
		for (String stderrLine : inlineScriptContext_1453467027519.getErrorStreamData())
			System.err.println(inlineScriptContext_1453467027519.getScriptTitle() + ": " + stderrLine);
		if ((!inlineScriptExecutor_1453467027519.wasSuccessFulExecution()) || (inlineScriptThrowable_1453467027519 != null))
		{
			System.out.println("*** ERROR: Execution of inline script \"" + inlineScriptContext_1453467027519.getScriptTitle() + "\" failed ***");
			if (inlineScriptThrowable_1453467027519 != null)
				inlineScriptThrowable_1453467027519.printStackTrace(System.err);
			System.exit(-2);
		}
		
		// execute inline script "test1" at start of test
		LoadtestInlineScriptContext inlineScriptContext_1453466295206 = new LoadtestInlineScriptContext("test1", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_GLOBAL_START, "", performanceData, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15);
		InlineScriptExecutor inlineScriptExecutor_1453466295206 = new InlineScriptExecutor(getInlineScriptCode_1453466295206(), inlineScriptContext_1453466295206);
		Throwable inlineScriptThrowable_1453466295206 = null;
		if (debugLoops)
			System.out.println("executing inline script \"" + inlineScriptContext_1453466295206.getScriptTitle() + "\"");
		try
		{
			LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("globalVar1", Lib.nullToBlank(globalVar1), 3);		// note: parameter no. 3 is the scope of the var
			inlineScriptContext_1453466295206.addInputVar(inputVar1);
			LoadtestInlineScriptVar inputVar2 = new LoadtestInlineScriptVar("globalVar2", Lib.nullToBlank(globalVar2), 3);		// note: parameter no. 3 is the scope of the var
			inlineScriptContext_1453466295206.addInputVar(inputVar2);
			inlineScriptExecutor_1453466295206.execute();		// execute inline script
		}
		catch (Throwable th_1453466295206)
		{
			inlineScriptThrowable_1453466295206 = th_1453466295206;
		}
		if (debugLoops)
		{
			for (String stdoutLine : inlineScriptContext_1453466295206.getOutputStreamData())
				System.out.println(inlineScriptContext_1453466295206.getScriptTitle() + ": " + stdoutLine);
		}
		for (String stderrLine : inlineScriptContext_1453466295206.getErrorStreamData())
			System.err.println(inlineScriptContext_1453466295206.getScriptTitle() + ": " + stderrLine);
		if ((!inlineScriptExecutor_1453466295206.wasSuccessFulExecution()) || (inlineScriptThrowable_1453466295206 != null))
		{
			System.out.println("*** ERROR: Execution of inline script \"" + inlineScriptContext_1453466295206.getScriptTitle() + "\" failed ***");
			if (inlineScriptThrowable_1453466295206 != null)
				inlineScriptThrowable_1453466295206.printStackTrace(System.err);
			System.exit(-2);
		}
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				INLINE_SCRIPT_03 simulatedUser = new INLINE_SCRIPT_03(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "try2nd"
	public static String getInlineScriptCode_1453467027519()
	{
		return "print \"try2nd\"\r\n\r\nprint vInputField";
	}
	
	// source code of inline script "test1"
	public static String getInlineScriptCode_1453466295206()
	{
		return "//include \"fileToBeincluded.txt\"\r\n\r\nprint \"globalVar1 = \"+globalVar1\r\n\r\nprint 5 + 8 / 9 * 9 - 6\r\nprint 3 + 3 * ( ( 4 + 4 * 4 - 5) / 2 ) / 5 + -5\r\nV1 = \"ss\"\r\nV1 = 23.89 + 45 + V1\r\nprint  V1\r\nV1 = V1 + \"amir\"\r\nprint V1\r\nV1 = V1 + 5\r\n\r\n\r\nV1 = StrSubString(V1,\"4\",8) \r\nprint V1\r\n\r\n\r\n\r\nV1 = StrToUpper(V1)\r\nprint V1\r\n\r\n\r\n//printloop()\r\n";
	}
