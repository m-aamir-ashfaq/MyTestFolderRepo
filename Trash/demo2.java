 ...
 * Automatically generated load test program.
 */
public class demo2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "demo2ExtParams.dat";                 // input file name for very large parameter values
	...
	private String var3 = null;                          // var declaration from web admin var handler: scope = per loop
	 ...
	 */
	public demo2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [1] <- WEB ADMIN Index 45 ---
		...
		{
			// extract var 'var3'
			var3 = Lib.extractTextToken( testURL.getWebSocketContext().getRecordedData().getWebSocketFramesWithoutPingPong().get(3).getPayloadString(), 1, "< >'=&\"");		// recorded value = Rock
			log("<<< var3 = " + var3);
		}
		catch (Exception e) { var3 = null; log(e); }
		if (var3 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'var3' from websocket frame pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [3] <- WEB ADMIN Index 50 ---
		log();
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("demo2", concurrentUsers);
		...
		performanceData.setInfoText("demo2");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				demo2 simulatedUser = new demo2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
