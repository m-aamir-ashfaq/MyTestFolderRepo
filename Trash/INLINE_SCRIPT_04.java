...
// -----------------------------------------
//
// Source: INLINE_SCRIPT_04.java
// Date  : 26 Apr 2016 11:21:49 ECT
 ...
 * Automatically generated load test program.
 */
public class INLINE_SCRIPT_04 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:41.0) Gecko/20100101 Firefox/41.0";
	...
	public static final String USER_INPUT_FIELD_vInputField = "AQALdklucHV0RmllbGQBAAtJbnB1dCBGaWVsZAEACEhhbGxvIGR1";		// internal object-string representation of user input field 'vInputField'
	...
	private static final String EXTERNAL_PARAM_FILE = "INLINE_SCRIPT_04ExtParams.dat";      // input file name for very large parameter values
	...
	private static String vInputField = null;                                    // var declaration from web admin var handler: scope = global
	private static String globalVar2 = "3";                                      // var declaration from web admin var handler: scope = global
	private static String google = "a";                                          // var declaration from web admin var handler: scope = global
	private static String globalVar1 = "2";                                      // var declaration from web admin var handler: scope = global
	private static String justAssign = "en";                                     // var declaration from web admin var handler: scope = global
	...
	private String userVar2 = "0";                                        // var declaration from web admin var handler: scope = per user
	private String userVar1 = "" + System.currentTimeMillis();            // var declaration from web admin var handler: scope = per user

	private String vEtag18 = null;                       // var declaration from web admin var handler: scope = per loop
	private String dataFromReplaced = null;              // var declaration from web admin var handler: scope = per loop
	private String LoopVar2 = null;                      // var declaration from web admin var handler: scope = per loop
	private String LoopVar1 = null;                      // var declaration from web admin var handler: scope = per loop
	private String V1 = null;                            // var declaration from web admin var handler: scope = per loop
	
	private String vInnerLoop2 = null;                   // var declaration from web admin var handler: scope = per inner loop
	private String vInnerLoop1 = null;                   // var declaration from web admin var handler: scope = per inner loop
	

	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public INLINE_SCRIPT_04()
	{
		super();
	}
	 ...
	 */
	public INLINE_SCRIPT_04(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
		...
		userVar2 = "" + threadNumber;		// initialize current user counter
		...
		transactionHandler.addTransaction(new UserTransactionContext(1445448560502L, "T1"));
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log();
		log("<<< userVar2 = " + userVar2);
		log("<<< userVar1 = " + userVar1);
		...
		vEtag18 = null;
		dataFromReplaced = null;
		LoopVar2 = Lib.getFormattedCurrentDateAndTime("yyyy-MM-dd", 0, "ECT");
		LoopVar1 = "cldemo.apicasystem.com";
		V1 = null;
		log();
		log("<<< vEtag18 = " + vEtag18);
		log("<<< dataFromReplaced = " + dataFromReplaced);
		log("<<< LoopVar2 = " + LoopVar2);
		log("<<< LoopVar1 = " + LoopVar1);
		log("<<< V1 = " + V1);
		log("<<< vInputField = " + vInputField);
		...
		log("# Page #2: Tickets");
		log("# ----------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: Tickets", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
		...
		log("# Page #3: Media");
		log("# --------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #3: Media", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
		...
		log("# Page #4: Cart");
		log("# -------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #4: Cart", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		testURL.addUserTransactionContextStart(transactionHandler.getTransaction(1445448560502L), threadStep);
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 3756
		...
		// setup var extractor for 'V1'
		ContentTokenExtractorItem contentTokenExtractorItem0001_1 = new ContentTokenExtractorItem("\"/WebResource.axd?d", "\t\n\r &(),={}", 1);
		...
		{
			// extract var 'V1'
			V1 = contentTokenExtractorItem0001_1.getResultToken();
			log("<<< V1 = " + V1);
		}
		catch (Exception e) { V1 = null; log(e); }
		if (V1 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'V1' from content token pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		requestFile0004 = Lib.replaceTextPattern(requestFile0004, "SwCcMWJbC5HPR0GyMZBQBtBVcjnGUcMopW87tvl8Uit8R50NU80PEVSNB0QE3yO4B2yncMJIGpp_qLe8p71gNcCZXqg1", V1, 0);		// search and replace request file fragment "SwCcMWJbC5HPR0GyMZBQBtBVcjnGUcMopW87tvl8Uit8R50NU80PEVSNB0QE3yO4B2yncMJIGpp_qLe8p71gNcCZXqg1" with variable 'V1' on all occurrences
		...
		testURL.setDNSContext(userDNSContext);
		testURL.addUserTransactionContextEnd(transactionHandler.getTransaction(1445448560502L), threadStep);
		testURL.setFailureActionType(HttpTestURL.FAILURE_ACTION_IGNORE_ERROR);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		...
		performanceData.addUserTransactionMeasuredSample(transactionHandler.getTransaction(1445448560502L));
		...
		// execute inline script "googlevarinitializer"
		inlineScriptContext = executeInlineScript_1458720326898(testURL, urlCallPassed, threadStep - 1, totalLoopCounter, innerLoopContext);
		...
		String requestFile0008 = "/Images/Banners/WindowsServer.jpg";
		String requestHeader0008 = "GET " + requestFile0008 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: cldemo.apicasystem.com\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: image/png,image/*;q=0.8,*/*;q=0.5\r\n" + 
				"Accept-Language: " + justAssign + "\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"Pragma: no-cache\r\n" + 
				"\r\n";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 1408, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0009 = "/Images/img.jpg";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 46192, 5);		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// execute inline script "V1UserScript"
		inlineScriptContext = executeInlineScript_1458721563956(testURL, urlCallPassed, threadStep - 1, totalLoopCounter, innerLoopContext);
		if (inlineScriptContext.isScriptAbort())
		{
			// special case: url already executed but inline script aborted - set URL as failed and abort current loop
			log("*** INLINE SCRIPT \"" + inlineScriptContext.getScriptTitle() + "\" ABORTED / CURRENT LOOP ABORTED ***");
		...
		String requestFile0012 = "/Images/Banners/WindowsAzure.jpg";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 1238, 5);		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0013 = "/Images/cart.png";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 423, 5);		// Test [13] <- Index 13
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0014 = "/Images/Banners/Amazon.jpg";
		...
		// verify response: status code = 200, content type = "IMAGE/JPEG", header text fragment = [verification disabled], recorded content size = 2128031
		// content test algorithm: check size +/- 5%, defined content size = 918
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 918, 5);		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		try
		{
			// extract var 'dataFromReplaced'
			dataFromReplaced = testURL.getContentString("ISO-8859-1");
			log("<<< dataFromReplaced = " + dataFromReplaced);
		}
		catch (Exception e) { dataFromReplaced = null; }		// failover error handling for this var is enabled - ignore any var assignment error
		if (dataFromReplaced == null)
		{
			dataFromReplaced = "abc";
			log("runtime note: the extraction of a new value for the variable 'dataFromReplaced' has failed - a substitute value was set");
			log("<<< dataFromReplaced = " + dataFromReplaced);
		}
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 4964, 5);		// Test [15] <- Index 15
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 6391, 5);		// Test [16] <- Index 16
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [17] <- WEB ADMIN Index 18 ---
		...
		{
			// extract var 'vEtag18'
			vEtag18 = testURL.getHeaderField("ETag");		// recorded value = "03fdfae9f12cf1:0"
			log("<<< vEtag18 = " + vEtag18);
		}
		catch (Exception e) { vEtag18 = null; log(e); }
		if (vEtag18 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'vEtag18' from http response header field";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 60220, 5);		// Test [18] <- Index 18
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 130542, 5);		// Test [19] <- Index 19
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 1446, 5);		// Test [20] <- Index 20
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	boolean executePage_1_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 234, 5);		// Test [21] <- Index 21
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [22] <- WEB ADMIN Index 24 ---
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 346, 5);		// Test [23] <- Index 23
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 445, 5);		// Test [24] <- Index 24
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 8180, 5);		// Test [25] <- Index 25
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [26] <- WEB ADMIN Index 28 ---
		String requestProt0026 = "http";
		String requestHost0026 = "cldemo.apicasystem.com";
		int    requestPort0026 = 80;
		String requestFile0026 = "/font/myriadpro-semibold_6-webfont.ttf";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/OCTET-STREAM", null, 48268, 5);		// Test [26] <- Index 26
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 272, 5);		// Test [27] <- Index 27
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 427, 5);		// Test [28] <- Index 28
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 192, 5);		// Test [29] <- Index 29
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 271, 5);		// Test [30] <- Index 30
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0031 = "/images/footer_mid.png";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 212, 5);		// Test [31] <- Index 31
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0032 = "/images/navright_h.png";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 183, 5);		// Test [32] <- Index 32
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 6331
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "option value=\"FC Internazionale Milano\"");		// Test [34] <- Index 34
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0035 = "/Images/Wait/spining_wheel_notrans.gif";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/GIF", null, 6337, 5);		// Test [35] <- Index 35
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0036 = "/images/green_button.gif";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/GIF", null, 154, 5);		// Test [36] <- Index 36
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	boolean executePage_3(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0038 = "/Videos.aspx";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 3816
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "function __doPostBack(eventTarget, eventArgument) {");		// Test [38] <- Index 38
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0039 = "/Images/ChLeagueThree.JPG";
		...
		// verify response: status code = 200, content type = "IMAGE/JPEG", header text fragment = [verification disabled], recorded content size = 75051
		// content test algorithm: check size +/- 5%, defined content size = 75051
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 75051, 5);		// Test [39] <- Index 39
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0040 = "/Images/ChLeagueFive.JPG";
		...
		// verify response: status code = 200, content type = "IMAGE/JPEG", header text fragment = [verification disabled], recorded content size = 100229
		// content test algorithm: check size +/- 5%, defined content size = 100229
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 100229, 5);		// Test [40] <- Index 40
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// verify response: status code = 200, content type = "IMAGE/JPEG", header text fragment = [verification disabled], recorded content size = 63542
		// content test algorithm: check size +/- 5%, defined content size = 63542
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 63542, 5);		// Test [41] <- Index 41
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// verify response: status code = 200, content type = "IMAGE/JPEG", header text fragment = [verification disabled], recorded content size = 49862
		// content test algorithm: check size +/- 5%, defined content size = 49862
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 49862, 5);		// Test [42] <- Index 42
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	boolean executePage_4(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [44] <- WEB ADMIN Index 56 ---
		log();
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 4097
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "You have no items in your shopping cart yet.");		// Test [44] <- Index 44
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		if ((ParseArgs.hasOption(args, "-h")) || (ParseArgs.hasOption(args, "-help")))
		{
			...
			System.out.println();
			System.out.println("-vInputField <user-input-field>          ->> required argument: Input Field");
			System.out.println();
		...
		// parse -vInputField argument or ask --> <user-input-field>: proposed value = "Hallo du"
		try
		{
			vInputField = UserInputField.readFromObjectString(USER_INPUT_FIELD_vInputField).readUserInputValue(args);
			if (vInputField.startsWith("\"") && vInputField.endsWith("\""))
				vInputField = vInputField.substring(1, vInputField.length() - 1);
		}
		catch (IOException ie)
		{
			System.out.println("*** INTERNAL ERROR: invalid user input field object-string declaration for 'vInputField' ***");
			ie.printStackTrace();
			System.exit(-2);
		}
		...
		String genericFileName = PerformanceData.proposeFileName("INLINE_SCRIPT_04", concurrentUsers);
		...
		{
			System.out.println("# global var globalVar1 = " + globalVar1);
			System.out.println("# global var globalVar2 = " + globalVar2);
			System.out.println("# global var google = " + google);
			System.out.println("# global var justAssign = " + justAssign);
			System.out.println("# global var vInputField = " + vInputField);
		}
		System.out.println();
		...
		final int MAX_THREAD_STEPS = 41;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("INLINE_SCRIPT_04");
		...
		performanceData.addUserInputField("Input Field", vInputField);		// add user input field name and value to performance data
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				INLINE_SCRIPT_04 simulatedUser = new INLINE_SCRIPT_04(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
		...
		while (!allDone)
		{
			...
			if (abortedByRemote() || abortedByEofInputFile())
			{
				...
				synchronized (dumpLock)
				{
					...
					// execute inline script "datatest" at end of test
					LoadtestInlineScriptContext inlineScriptContext_1461662371941 = new LoadtestInlineScriptContext("datatest", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_GLOBAL_END, "", performanceData, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15);
					InlineScriptExecutor inlineScriptExecutor_1461662371941 = new InlineScriptExecutor(getInlineScriptCode_1461662371941(), inlineScriptContext_1461662371941);
					Throwable inlineScriptThrowable_1461662371941 = null;
					if (debugLoops)
						System.out.println("Executing inline script \"" + inlineScriptContext_1461662371941.getScriptTitle() + "\"");
					try
					{
						inlineScriptExecutor_1461662371941.execute();		// execute inline script
					}
					catch (Throwable th_1461662371941)
					{
						inlineScriptThrowable_1461662371941 = th_1461662371941;
					}
					if (debugLoops)
					{
						for (String stdoutLine : inlineScriptContext_1461662371941.getOutputStreamData())
							System.out.println(inlineScriptContext_1461662371941.getScriptTitle() + ": " + stdoutLine);
					}
					for (String stderrLine : inlineScriptContext_1461662371941.getErrorStreamData())
						System.err.println(inlineScriptContext_1461662371941.getScriptTitle() + ": " + stderrLine);
					if ((!inlineScriptExecutor_1461662371941.wasSuccessFulExecution()) || (inlineScriptThrowable_1461662371941 != null))
					{
						if (inlineScriptContext_1461662371941.isScriptAbort())
						{
							System.out.println("*** INLINE SCRIPT \"" + inlineScriptContext_1461662371941.getScriptTitle() + "\" ABORTED / LOAD TEST CONTINUED ***");
							System.out.println("Abort Message = " + inlineScriptContext_1461662371941.getScriptAbortMessage());
						}
						else
						{
							System.out.println("*** ERROR: Execution of inline script \"" + inlineScriptContext_1461662371941.getScriptTitle() + "\" failed ***");
							if (inlineScriptThrowable_1461662371941 != null)
								inlineScriptThrowable_1461662371941.printStackTrace(System.err);
							System.exit(-2);
						}
					}
	...
	// source code of inline script "datatest"
	public static String getInlineScriptCode_1461662371941()
	{
		return "writeToFile(\"testWiikData.txt\",dataFromReplaced)";
	}
	
	// source code of inline script "googlevarinitializer"
	public static String getInlineScriptCode_1458720326898()
	{
		return "google = \"google\"";
	}
	
	// source code of inline script "V1UserScript"
	public static String getInlineScriptCode_1458721563956()
	{
		return "print V1";
	}
	...
	/**
	* Execute the inline script "googlevarinitializer"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1458720326898(HttpTestURL testURL, boolean urlCallPassed, int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("googlevarinitializer", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_URL_END, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler, testURL, threadStep);
		inlineScriptContext.setUrlCallPassed(urlCallPassed);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1458720326898(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("google", Lib.nullToBlank(google), 3);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar1);
		inlineScriptExecutor.execute();		// execute inline script
		google = inlineScriptContext.getOutputVar("google").getVarValue();
		log("<<< google = " + google);
	...
	/**
	* Execute the inline script "V1UserScript"
	* 
	* @param testURL 	the context of the executed URL call
	* @param urlCallPassed 	a flag that signals (when set to false) if a 'yellow' error did occur after the URL call (triggered by response verification settings)
	* @param threadStep 	the current execution step of the simulated user (page break or URL call)
	...
	* @return the context of the inline script
	*/
	public LoadtestInlineScriptContext executeInlineScript_1458721563956(HttpTestURL testURL, boolean urlCallPassed, int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("V1UserScript", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_URL_END, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler, testURL, threadStep);
		inlineScriptContext.setUrlCallPassed(urlCallPassed);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1458721563956(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("V1", Lib.nullToBlank(V1), 1);		// note: parameter no. 3 is the scope of the var
