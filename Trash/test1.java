...
public class test1 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static String payload = "This is a collaborative process, so dig in and try to help out! There's tons of details missing, just waiting for you to add them! So send us a pull request, please!"; // var declaration from web admin var handler: scope = global
	...
	private String cId = null;                           // var declaration from web admin var handler: scope = per loop
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [1] <- WEB ADMIN Index 25 ---
		...
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: ZDPr/oP64M5PtA1gFJpkZQ==\r\n" + 
		...
		// execute inline script "generateClientID"
		inlineScriptContext = executeInlineScript_1484060216505(testURL, threadStep, totalLoopCounter, innerLoopContext);
		...
		webSocketData = getWebSocketData_1484059980485();
		...
		// --- HTTP REQUEST: Test [2] <- WEB ADMIN Index 33 ---
		String requestProt0002 = "http";
		String requestHost0002 = "shuttle3";
		...
		String requestHeader0002 = "GET " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Protocol: mqttv3.1\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: +1A2t7eNDRaYoNxSXs+Utw==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		webSocketData = getWebSocketData_1484060020509();
		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, null, null));
	...
	public static void main(String[] args)
	{
		...
		final int MAX_THREAD_STEPS = 2;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		{
			System.out.println("global var <<< payload = " + payload);
	...
	// source code of inline script "generateClientID"
	public static String getInlineScriptCode_1484060216505()
	{
		return "cId = generateGUID(\"clientId-xxxxxxxxxx\");";
	}
	
	
	/**
	* Execute the inline script "generateClientID"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1484060216505(HttpTestURL testURL, int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("generateClientID", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_URL_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler, testURL, threadStep);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1484060216505(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("cId", Lib.nullToBlank(cId), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("cId", Lib.nullToBlank(cId), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar1);
		inlineScriptExecutor.execute();		// execute inline script
		cId = inlineScriptContext.getOutputVar("cId").getVarValue();
		log("<<< cId = " + cId);
	...
	public WebSocketData getWebSocketData_1484059980485()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1484059980527L);
		webSocketData.setConnectionStopMillis(1484060004191L);
		webSocketData.setSecProtocol("mqtt");
		String payLoad_0 = "{\"DUP\":false,\"QoS\":0,\"userNameFlag\":0,\"last_will_Flag\":0,\"keep_alive\":60,\"retain\":false,\"client_id\":\"clientId-wwoJgBJRm5\",\"clean_session\":true,\"type\":\"CONNECT\",\"passwordFlag\":0,\"protocol_name\":\"MQIsdp\",\"protocol_version\":3}";
		String payLoad_2 = "{\"subscribe_topics\":[{\"subscribe_topic\":\"testtopic/testAmir\",\"subscribe_QoS\":2}],\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"SUBSCRIBE\"}";
		String payLoad_4 = "{\"DUP\":false,\"QoS\":0,\"contents\":\"mmm\",\"publish_topic\":\"testtopic/testAmir\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_6 = "{\"DUP\":false,\"QoS\":1,\"messageID\":1,\"contents\":\"mmm\",\"publish_topic\":\"testtopic/testAmir\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_9 = "{\"DUP\":false,\"QoS\":0,\"messageID\":1,\"retain\":false,\"type\":\"PUBACK\"}";
		String payLoad_10 = "{\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"UNSUBSCRIBE\",\"unsubscribe_topic\":[\"testtopic/testAmir\"]}";
		String payLoad_12 = "{\"DUP\":false,\"QoS\":0,\"retain\":false,\"type\":\"DISCONNECT\"}";
		String payLoad_13 = "";

		// replace field  "client_id" with variable 'cId'  of frame no 0
		payLoad_0 = Lib.replaceJsonField(payLoad_0, "client_id", cId);


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484059980559L,1484059980584L , payLoad_0, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484059986921L,1484559833824L , payLoad_2, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484059992920L,1484559835478L , payLoad_4, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484059996051L,1484559839819L , payLoad_6, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484059996132L,1484059996133L , payLoad_9, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484060001810L,1484559840857L , payLoad_10, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484060004148L,1484559843234L , payLoad_12, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1484060004149L,1484060004151L , payLoad_13, WebSocketFrame.MQTT ));
	

		 // Add all new Frames and delays 'ProxySnifferVarSourceWebSocketNewFrame'.
	
		String topicFrame_1 = "testtopic/testMoona";
		int qosFrame_1 = 1;
		long offsetFrame_1 = 3000;
		webSocketData.addWebSocketSubscribeFrame(topicFrame_1 , qosFrame_1,offsetFrame_1);
	
		String topicFrame_2 = "testtopic/testMoona";
		int qosFrame_2 = 0;
		boolean retainFrame_2 = false;
		String contentFrame_2 = payload;
		long offsetFrame_2 = 13000;
		webSocketData.addWebSocketPublishFrame(topicFrame_2 , qosFrame_2 , retainFrame_2 , contentFrame_2 , offsetFrame_2);
	
		String topicFrame_3 = "vcf";
		int qosFrame_3 = 1;
		boolean retainFrame_3 = false;
		String contentFrame_3 = "134242r";
		long offsetFrame_3 = 12500;
		webSocketData.addWebSocketPublishFrame(topicFrame_3 , qosFrame_3 , retainFrame_3 , contentFrame_3 , offsetFrame_3);
	
		return webSocketData;
	}
	
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1484060020509()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1484060020520L);
		webSocketData.setConnectionStopMillis(1484060035408L);
		webSocketData.setSecProtocol("mqtt");
		String payLoad_0 = "{\"DUP\":false,\"QoS\":0,\"userNameFlag\":0,\"last_will_Flag\":0,\"keep_alive\":60,\"retain\":false,\"client_id\":\"clientId-wwoJgBJRm3\",\"clean_session\":true,\"type\":\"CONNECT\",\"passwordFlag\":0,\"protocol_name\":\"MQIsdp\",\"protocol_version\":3}";
		String payLoad_2 = "{\"subscribe_topics\":[{\"subscribe_topic\":\"testtopic/testAmir\",\"subscribe_QoS\":2}],\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"SUBSCRIBE\"}";
		String payLoad_4 = "{\"DUP\":false,\"QoS\":1,\"messageID\":1,\"contents\":\"fgbgb\",\"publish_topic\":\"testtopic/testAmir\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_7 = "{\"DUP\":false,\"QoS\":0,\"messageID\":1,\"retain\":false,\"type\":\"PUBACK\"}";
		String payLoad_8 = "{\"DUP\":false,\"QoS\":0,\"contents\":\"fgbgb\",\"publish_topic\":\"testtopic/testAmir\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_10 = "{\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"UNSUBSCRIBE\",\"unsubscribe_topic\":[\"testtopic/testAmir\"]}";
		String payLoad_12 = "{\"DUP\":false,\"QoS\":0,\"retain\":false,\"type\":\"DISCONNECT\"}";
		String payLoad_13 = "";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484060020522L,1484060020523L , payLoad_0, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484060024822L,1484060024823L , payLoad_2, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484060028044L,1484060028045L , payLoad_4, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484060028102L,1484060028103L , payLoad_7, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484060030868L,1484060030869L , payLoad_8, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484060033477L,1484060033478L , payLoad_10, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484060035397L,1484060035398L , payLoad_12, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1484060035398L,1484060035400L , payLoad_13, WebSocketFrame.MQTT ));
