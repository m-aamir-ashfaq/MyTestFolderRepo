 ...
 * Automatically generated load test program.
 */
public class test55 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:46.0) Gecko/20100101 Firefox/46.0";
	...
	private static final String EXTERNAL_PARAM_FILE = "test55ExtParams.dat";                // input file name for very large parameter values
	 ...
	 */
	public test55(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log();
		log();
		log("# Page #1: PageStart");
		log("# ------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #1: PageStart", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "www.anything.com";
		...
		performanceData.setInfoTextAndDataType(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
 /* [non-executable URL - commented-out lines start] 
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 5548
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "link rel=\"preconnect\" href=\"//fonts.googleapis.com\"");		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
 ...
 [non-executable URL - commented-out lines end] */ 
		threadStep = threadStep + 1;		// only used for non-executable URL calls - remove this line of code if you uncomment the non-executable code sequence
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("test55", concurrentUsers);
		...
		performanceData.setInfoText("test55");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new test55());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				test55 simulatedUser = new test55(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
