...
// -----------------------------------------
//
// Source: websocket008_IO.java
// Date  : 14 Jul 2016 17:20:05 ECT
 ...
 * Automatically generated load test program.
 */
public class websocket008_IO extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "websocket008_IOExtParams.dat";       // input file name for very large parameter values
	...
	private static String var1 = "some string";                                  // var declaration from web admin var handler: scope = global
	 ...
	 */
	public websocket008_IO(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "chat.socket.io";
		int    requestPort0001 = 80;
		String requestFile0001 = "/socket.io/" +
				"?EIO=3" +
				"&transport=polling" +
				"&t=LNfoveC";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/OCTET-STREAM", null, 101, 5);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		String requestHost0002 = "chat.socket.io";
		int    requestPort0002 = 80;
		String requestFile0002 = "/socket.io/" +
				"?EIO=3" +
				"&transport=polling" +
				"&t=LNfowDG" +
				"&sid=xJ93wztVUj8-27K1BV89";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/OCTET-STREAM", null, 5, 5);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0003 = "chat.socket.io";
		int    requestPort0003 = 80;
		String requestFile0003 = "/socket.io/" +
				"?EIO=3" +
				"&transport=polling" +
				"&t=LNfowGe" +
				"&sid=xJ93wztVUj8-27K1BV89";
		...
		// verify response: status code = 200, content type = "APPLICATION/OCTET-STREAM", header text fragment = [verification disabled], recorded content size = 30
		// content test algorithm: check size +/- 5%, defined content size = 30
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/OCTET-STREAM", null, 30, 5);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0004 = "chat.socket.io";
		int    requestPort0004 = 80;
		String requestFile0004 = "/socket.io/" +
				"?EIO=3" +
				"&transport=websocket" +
				"&sid=xJ93wztVUj8-27K1BV89";
		String requestHeader0004 = "GET " + requestFile0004 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: Tn7v/VhgcgODvo71Jb1fyA==\r\n" + 
		...
		webSocketData = getWebSocketData_1468506285163();
		...
		String requestHost0005 = "chat.socket.io";
		int    requestPort0005 = 80;
		String requestFile0005 = "/socket.io/" +
				"?EIO=3" +
				"&transport=polling" +
				"&t=LNfowGd" +
				"&sid=xJ93wztVUj8-27K1BV89";
		String requestContent0005 = 
				"21:42[\"add user\",\"khan\"]";
		byte[] requestContentBinary0005 = requestContent0005.getBytes("UTF-8");
		String requestHeader0005 = "POST " + requestFile0005 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Content-Length: " + requestContentBinary0005.length + "\r\n" + 		// Content-Length: 24
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0005, requestHost0005, requestPort0005, requestHeader0005, requestContentBinary0005, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		requestContentBinary0005 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 2, 5);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0006 = "chat.socket.io";
		int    requestPort0006 = 80;
		String requestFile0006 = "/socket.io/" +
				"?EIO=3" +
				"&transport=polling" +
				"&t=LNfowNN" +
				"&sid=xJ93wztVUj8-27K1BV89";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/OCTET-STREAM", null, 4, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("websocket008_IO", concurrentUsers);
		...
		performanceData.setInfoText("websocket008_IO");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				websocket008_IO simulatedUser = new websocket008_IO(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	public static WebSocketData getWebSocketData_1468506285163()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1468506285370L);
		webSocketData.setConnectionStopMillis(1468506293320L);

		 // Actual payload was encoded using Per Message Deflate Extension. following are Inflated payloads.
		String payLoad_0 = "2probe";
		String payLoad_1 = "";
		String payLoad_2 = "5";
		String payLoad_3 = "42[\"typing\"]";
		String payLoad_4 = "42[\"stop typing\"]";
		String payLoad_5 = "42[\"typing\"]";
		String payLoad_6 = "42[\"stop typing\"]";
		String payLoad_7 = "42[\"typing\"]";
		String payLoad_8 = "42[\"stop typing\"]";
		String payLoad_9 = "42[\"new message\",\"message 1\"]";
		String payLoad_10 = "42[\"stop typing\"]";
		String payLoad_11 = "";
		String payLoad_12 = "";

		// search and replace frame payload "typing" with variable 'var1' on all occurrences of all frames.
		payLoad_0 = Lib.replaceTextPattern(payLoad_0, "typing", var1, 0);
		payLoad_1 = Lib.replaceTextPattern(payLoad_1, "typing", var1, 0);
		payLoad_2 = Lib.replaceTextPattern(payLoad_2, "typing", var1, 0);
		payLoad_3 = Lib.replaceTextPattern(payLoad_3, "typing", var1, 0);
		payLoad_4 = Lib.replaceTextPattern(payLoad_4, "typing", var1, 0);
		payLoad_5 = Lib.replaceTextPattern(payLoad_5, "typing", var1, 0);
		payLoad_6 = Lib.replaceTextPattern(payLoad_6, "typing", var1, 0);
		payLoad_7 = Lib.replaceTextPattern(payLoad_7, "typing", var1, 0);
		payLoad_8 = Lib.replaceTextPattern(payLoad_8, "typing", var1, 0);
		payLoad_9 = Lib.replaceTextPattern(payLoad_9, "typing", var1, 0);
		payLoad_10 = Lib.replaceTextPattern(payLoad_10, "typing", var1, 0);
		payLoad_11 = Lib.replaceTextPattern(payLoad_11, "typing", var1, 0);
		payLoad_12 = Lib.replaceTextPattern(payLoad_12, "typing", var1, 0);


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506285378L,1468506285381L , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506285378L,1468506285381L , DeflateUtility.compress("payLoad_0".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1468506285588L,1468506285590L , payLoad_1 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1468506285588L,1468506285590L , DeflateUtility.compress("payLoad_1".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506285794L,1468506285795L , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506285794L,1468506285795L , DeflateUtility.compress("payLoad_2".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506287291L,1468506287292L , payLoad_3 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506287291L,1468506287292L , DeflateUtility.compress("payLoad_3".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506287988L,1468506287990L , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506287988L,1468506287990L , DeflateUtility.compress("payLoad_4".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506288745L,1468506288747L , payLoad_5 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506288745L,1468506288747L , DeflateUtility.compress("payLoad_5".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506290577L,1468506290578L , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506290577L,1468506290578L , DeflateUtility.compress("payLoad_6".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506290653L,1468506290654L , payLoad_7 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506290653L,1468506290654L , DeflateUtility.compress("payLoad_7".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506291057L,1468506291059L , payLoad_8 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506291057L,1468506291059L , DeflateUtility.compress("payLoad_8".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506291489L,1468506291490L , payLoad_9 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506291489L,1468506291490L , DeflateUtility.compress("payLoad_9".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506291493L,1468506291494L , payLoad_10 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1468506291493L,1468506291494L , DeflateUtility.compress("payLoad_10".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 8 , 1468506293104L,1468506293105L , payLoad_11 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 8 , 1468506293104L,1468506293105L , DeflateUtility.compress("payLoad_11".getBytes()) ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 8 , 1468506293318L,1468506293320L , payLoad_12 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 8 , 1468506293318L,1468506293320L , DeflateUtility.compress("payLoad_12".getBytes()) ));
