...
// -----------------------------------------
//
// Source: sessionWithWebSocketDa22.java
// Date  : 14 Jun 2016 14:49:30 ECT
 ...
 * Automatically generated load test program.
 */
public class sessionWithWebSocketDa22 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
...
WebSocketData webSocketData = null;
 ...
 byte[] extendedPayloadByte  = null;
	...
	private static final String EXTERNAL_PARAM_FILE = "sessionWithWebSocketDa22ExtParams.dat";// input file name for very large parameter values
	 ...
	 */
	public sessionWithWebSocketDa22()
	{
		super();
	}
	 ...
	 */
	public sessionWithWebSocketDa22(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0001 = "/submit/telemetry/439632c4-1af3-5643-a37e-f39878b8d964/main/Firefox/45.0.2/release/20160407164938" +
				"?v=4";
		String requestContent0001 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "sessionWithWebSocketDa22_RequestContent0001.txt").getFileDataAsString("UTF-8");		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/PLAIN", null, 20, 5);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		String requestHost0002 = "incoming.telemetry.mozilla.org";
		int    requestPort0002 = 443;
		String requestFile0002 = "/submit/telemetry/01cf17d7-0c3d-2940-a67e-2ca1c09ffae8/main/Firefox/45.0.2/release/20160407164938" +
				"?v=4";
		String requestContent0002 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "sessionWithWebSocketDa22_RequestContent0002.txt").getFileDataAsString("UTF-8");		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
		byte[] requestContentBinary0002 = requestContent0002.getBytes("UTF-8");
		String requestHeader0002 = "POST " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: incoming.telemetry.mozilla.org\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n" + 
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				...
				"Content-Type: application/json; charset=UTF-8\r\n" + 
				"Content-Encoding: gzip\r\n" + 
				"Content-Length: " + requestContentBinary0002.length + "\r\n" + 		// Content-Length: 12104
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0002, requestHost0002, requestPort0002, requestHeader0002, requestContentBinary0002, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		requestContent0002 = null;		// support garbage collector to reduce memory
		requestContentBinary0002 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());

		// verify response: status code = 200, content type = "TEXT/PLAIN", header text fragment = [verification disabled], recorded content size = 20
		// content test algorithm: check size +/- 5%, defined content size = 20
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/PLAIN", null, 20, 5);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
...
payload  = new byte[0];
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)9,false,0,maskingKey,payload,false,0,(byte)-119,(byte)0,extendedPayloadByte,1464269123688L,1464269123689L));
...
maskingKey[3] = 92;
payload  = new byte[0];
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)10,true,0,maskingKey,payload,false,0,(byte)-118,(byte)-128,extendedPayloadByte,1464269123690L,1464269123691L));
 maskingKey  = new byte[4];
maskingKey[0] = -4;
maskingKey[1] = 73;
maskingKey[2] = -4;
...
payload[33] = 122;
payload[34] = -55;
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,true,35,maskingKey,payload,false,0,(byte)-127,(byte)-93,extendedPayloadByte,1464269134255L,1464269134256L));
payload  = new byte[35];
payload[0] = 49;
payload[1] = 50;
payload[2] = 51;
...
payload[33] = 51;
payload[34] = 53;
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,false,35,maskingKey,payload,false,0,(byte)-127,(byte)35,extendedPayloadByte,1464269134372L,1464269134373L));
 maskingKey  = new byte[4];
maskingKey[0] = 17;
maskingKey[1] = -118;
maskingKey[2] = 92;
maskingKey[3] = 29;
payload  = new byte[126];
payload[0] = 32;
payload[1] = -72;
payload[2] = 111;
payload[3] = 44;
...
payload[124] = 35;
payload[125] = -68;
 extendedPayloadByte  = new byte[2];
extendedPayloadByte[0] = 0;
extendedPayloadByte[1] = 126;
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,true,126,maskingKey,payload,false,2,(byte)-127,(byte)-2,extendedPayloadByte,1464269143131L,1464269143132L));
payload  = new byte[126];
payload[0] = 49;
payload[1] = 50;
payload[2] = 51;
...
extendedPayloadByte[0] = 0;
extendedPayloadByte[1] = 126;
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,false,126,maskingKey,payload,false,2,(byte)-127,(byte)126,extendedPayloadByte,1464269143248L,1464269143249L));
 maskingKey  = new byte[4];
maskingKey[0] = 2;
maskingKey[1] = -43;
maskingKey[2] = -90;
...
payload[126] = -111;
 extendedPayloadByte  = new byte[2];
extendedPayloadByte[0] = 0;
extendedPayloadByte[1] = 127;
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,true,127,maskingKey,payload,false,2,(byte)-127,(byte)-2,extendedPayloadByte,1464269152577L,1464269152578L));
payload  = new byte[127];
payload[0] = 49;
payload[1] = 50;
payload[2] = 51;
...
extendedPayloadByte[0] = 0;
extendedPayloadByte[1] = 127;
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,false,127,maskingKey,payload,false,2,(byte)-127,(byte)126,extendedPayloadByte,1464269152707L,1464269152709L));
 maskingKey  = new byte[4];
maskingKey[0] = -62;
maskingKey[1] = -22;
maskingKey[2] = -35;
...
payload[127] = 99;
 extendedPayloadByte  = new byte[2];
extendedPayloadByte[0] = 0;
extendedPayloadByte[1] = -128;
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,true,128,maskingKey,payload,false,2,(byte)-127,(byte)-2,extendedPayloadByte,1464269160872L,1464269160873L));
payload  = new byte[128];
payload[0] = 49;
payload[1] = 50;
payload[2] = 51;
...
extendedPayloadByte[0] = 0;
extendedPayloadByte[1] = -128;
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,false,128,maskingKey,payload,false,2,(byte)-127,(byte)126,extendedPayloadByte,1464269160997L,1464269160998L));
 maskingKey  = new byte[4];
maskingKey[0] = -31;
maskingKey[1] = -82;
maskingKey[2] = -112;
maskingKey[3] = 45;
payload  = new byte[778];
payload[0] = -61;
payload[1] = -30;
payload[2] = 83;
payload[3] = -119;
...
payload[777] = -116;
 extendedPayloadByte  = new byte[2];
extendedPayloadByte[0] = 3;
extendedPayloadByte[1] = 10;
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,true,778,maskingKey,payload,false,2,(byte)-127,(byte)-2,extendedPayloadByte,1464269183504L,1464269183505L));
payload  = new byte[778];
payload[0] = 34;
payload[1] = 76;
payload[2] = -61;
...
extendedPayloadByte[0] = 3;
extendedPayloadByte[1] = 10;
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,false,778,maskingKey,payload,false,2,(byte)-127,(byte)126,extendedPayloadByte,1464269183623L,1464269183624L));
 maskingKey  = new byte[4];
maskingKey[0] = 40;
maskingKey[1] = -123;
maskingKey[2] = -8;
maskingKey[3] = 66;
payload  = new byte[0];
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)8,true,0,maskingKey,payload,false,0,(byte)-120,(byte)-128,extendedPayloadByte,1464269192388L,1464269192389L));
payload  = new byte[0];
webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)8,false,0,maskingKey,payload,false,0,(byte)-120,(byte)0,extendedPayloadByte,1464269192519L,1464269192520L));
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0005 = "safebrowsing.google.com";
		int    requestPort0005 = 443;
		String requestFile0005 = "/safebrowsing/downloads" +
				"?client=navclient-auto-ffox" +
				"&appver=45.0.2" +
				"&pver=2.2" +
				"&key=AIzaSyD_Drzahe4dBzGCZ9ArvowCvrPx_yFrlCM";
		String requestContent0005 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "sessionWithWebSocketDa22_RequestContent0005.txt").getFileDataAsString();		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
		String requestHeader0005 = "POST " + requestFile0005 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate, br\r\n" + 
				"Content-Length: " + requestContent0005.length() + "\r\n" + 		// Content-Length: 10339
		...
		// execute request
		testURL = new HttpTestURL(requestProt0005, requestHost0005, requestPort0005, requestHeader0005, requestContent0005.getBytes(), requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		requestContent0005 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());

		// verify response: status code = 200, content type = "APPLICATION/VND.GOOGLE.SAFEBROWSING-UPDATE", header text fragment = [verification disabled], recorded content size = 798
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/VND.GOOGLE.SAFEBROWSING-UPDATE", null, "u:safebrowsing-cache.google.com/safebrowsing/rd/ChRnb29nLXVud2FudGVkLXNoYXZhcjgAQAJKDAgBEO__Axi1gAQgAUoPCAAQwq8EGIWwBCABKgEF");		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0006 = "/safebrowsing/rd/ChVnb29nLWJhZGJpbnVybC1zaGF2YXI4AEACSgwIARD3xwYYxMgGIAFKDwgAENq4BhicuQYgASoBBw";
		...
		// verify response: status code = 200, content type = "APPLICATION/VND.GOOGLE.SAFEBROWSING-CHUNK", header text fragment = [verification disabled], recorded content size = 7607
		// content test algorithm: check size +/- 5%, defined content size = 7607
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/VND.GOOGLE.SAFEBROWSING-CHUNK", null, 7607, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0007 = "safebrowsing-cache.google.com";
		int    requestPort0007 = 443;
		String requestFile0007 = "/safebrowsing/rd/ChNnb29nLW1hbHdhcmUtc2hhdmFyOABAAkoMCAEQgcQOGM_EDiABSgwIABCL5w4Y0ucOIAE";
		...
		// verify response: status code = 200, content type = "APPLICATION/VND.GOOGLE.SAFEBROWSING-CHUNK", header text fragment = [verification disabled], recorded content size = 81996
		// content test algorithm: check size +/- 5%, defined content size = 81996
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/VND.GOOGLE.SAFEBROWSING-CHUNK", null, 81996, 5);		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0008 = "safebrowsing-cache.google.com";
		int    requestPort0008 = 443;
		String requestFile0008 = "/safebrowsing/rd/ChFnb29nLXBoaXNoLXNoYXZhcjgAQAJKDAgBEIOyEBitshAgAQ";
		...
		// verify response: status code = 200, content type = "APPLICATION/VND.GOOGLE.SAFEBROWSING-CHUNK", header text fragment = [verification disabled], recorded content size = 67627
		// content test algorithm: check size +/- 5%, defined content size = 67627
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/VND.GOOGLE.SAFEBROWSING-CHUNK", null, 67627, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0009 = "safebrowsing-cache.google.com";
		int    requestPort0009 = 443;
		String requestFile0009 = "/safebrowsing/rd/ChFnb29nLXBoaXNoLXNoYXZhcjgAQAJKDAgBEK6yEBjSshAgAUoMCAAQsK4bGImvGyAB";
		...
		// verify response: status code = 200, content type = "APPLICATION/VND.GOOGLE.SAFEBROWSING-CHUNK", header text fragment = [verification disabled], recorded content size = 187077
		// content test algorithm: check size +/- 5%, defined content size = 187077
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/VND.GOOGLE.SAFEBROWSING-CHUNK", null, 187077, 5);		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0010 = "safebrowsing-cache.google.com";
		int    requestPort0010 = 443;
		String requestFile0010 = "/safebrowsing/rd/ChRnb29nLXVud2FudGVkLXNoYXZhcjgAQAJKDAgBEO__Axi1gAQgAUoPCAAQwq8EGIWwBCABKgEF";
		...
		// verify response: status code = 200, content type = "APPLICATION/VND.GOOGLE.SAFEBROWSING-CHUNK", header text fragment = [verification disabled], recorded content size = 5393
		// content test algorithm: check size +/- 5%, defined content size = 5393
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/VND.GOOGLE.SAFEBROWSING-CHUNK", null, 5393, 5);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("sessionWithWebSocketDa22", concurrentUsers);
		...
		final int MAX_THREAD_STEPS = 9;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("sessionWithWebSocketDa22");
		...
		try
		{
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_1);		// sessionWithWebSocketDa22_RequestContent0001.txt
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_2);		// sessionWithWebSocketDa22_RequestContent0002.txt
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_3);		// sessionWithWebSocketDa22_RequestContent0005.txt
		}
		catch (IOException ie)
		{
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				sessionWithWebSocketDa22 simulatedUser = new sessionWithWebSocketDa22(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// --------------------------------------------------------------------------------
	public static final String REQUEST_FILE_CACHE_FILE_1 = "sessionWithWebSocketDa22_RequestContent0001.txt";		// must be zipped together with the compiled class of this load test program
	public static final String REQUEST_FILE_CACHE_FILE_2 = "sessionWithWebSocketDa22_RequestContent0002.txt";		// must be zipped together with the compiled class of this load test program
	public static final String REQUEST_FILE_CACHE_FILE_3 = "sessionWithWebSocketDa22_RequestContent0005.txt";		// must be zipped together with the compiled class of this load test program
