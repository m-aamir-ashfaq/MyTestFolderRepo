 ...
 * Automatically generated load test program.
 */
public class Az2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "Az2ExtParams.dat";                   // input file name for very large parameter values
	...
	private static String user_pass = "passwordLahore";                          // var declaration from web admin var handler: scope = global
	private static String user_lastName = "Bhi";                                 // var declaration from web admin var handler: scope = global
	private static String user_email = "bhiNaseer@yahoo.com";                    // var declaration from web admin var handler: scope = global
	private static String user_fName = "Naseer";                                 // var declaration from web admin var handler: scope = global
	 ...
	 */
	public Az2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "127.0.0.1";
		int    requestPort0001 = 7990;
		String requestFile0001 = "/dfischer/webadmininterface/BrowserExtensionWeblet" +
				"?action=getRecordState";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 3
		// content test algorithm: check size +/- 5%, defined content size = 3
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 3, 5);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		String requestHost0002 = "snippets-stats.eu-west.moz.works";
		int    requestPort0002 = 443;
		String requestFile0002 = "/foo" +
				"?snippet_name=6299" +
				"&snippet_full_name=moco-global-moz-simple-%20delight-never%20read%20the%20comments--EN-Release" +
				"&locale=en-GB" +
				"&country=se" +
				"&metric=impression" +
				"&campaign=delightFEB";
		String requestHeader0002 = "GET " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: snippets-stats.eu-west.moz.works\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: */*\r\n" + 
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
		...
		// verify response: status code = 302, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 10
		...
		String requestHost0003 = "snippets-stats.mozilla.org";
		int    requestPort0003 = 443;
		String requestFile0003 = "/foo.html" +
				"?snippet_name=6299" +
				"&snippet_full_name=moco-global-moz-simple-%20delight-never%20read%20the%20comments--EN-Release" +
				"&locale=en-GB" +
				"&country=se" +
		...
		String requestHeader0003 = "GET " + requestFile0003 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: snippets-stats.mozilla.org\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: */*\r\n" + 
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate, br\r\n" + 
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 0, 5);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "TEXT/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 104
		// content test algorithm: check size +/- 5%, defined content size = 104
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 104, 5);		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
		...
		String requestHost0005 = "www.google.com";
		int    requestPort0005 = 443;
		String requestFile0005 = "/complete/search" +
				"?client=firefox" +
				"&q=dev";
		...
		// verify response: status code = 200, content type = "TEXT/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 99
		// content test algorithm: check size +/- 5%, defined content size = 99
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 99, 5);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "var redirectLocation = window.location.origin;");		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0007 = "/api/configuration/site/gethostagent";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 281, 5);		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0008 = "/api/configuration/site/getconfig";
		String requestHeader0008 = "GET " + requestFile0008 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: bWlQYmQzUFQyRngzWkVtR241c2FkVkZOZXNjdm53NU13Z1Q1dWhsVmZNdz06YW5vbnltb3VzOjYzNjIzMDE4NTg2NDI0MDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 1363
		// content test algorithm: check size +/- 5%, defined content size = 1363
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 1363, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0009 = "/api/authorization/account/isauthenticated";
		String requestHeader0009 = "GET " + requestFile0009 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: Wk5yUTB6R1M0SG5GY2w0UzVINVRCRTFiNzlRMTl0MTVpSWdqOGZabWtsST06YW5vbnltb3VzOjYzNjIzMDE4NTg2NDg2MDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 228, 5);		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0010 = "/api/events/channel/getchannels";
		String requestHeader0010 = "GET " + requestFile0010 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: YXJGUjZiU05kMkpOcExDeDAwTSthYS9qTVR0cHB2UkFJZzNjSm9BUVBXMD06YW5vbnltb3VzOjYzNjIzMDE4NTg2OTQ4MDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 8327
		// content test algorithm: check size +/- 5%, defined content size = 8327
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 8327, 5);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0011 = "/api/events/event/getplayingevent";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 648
		// content test algorithm: check size +/- 5%, defined content size = 648
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 648, 5);		// Test [11] <- Index 11
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0012 = "/api/events/event/getaggregatedevents";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 6611
		// content test algorithm: check size +/- 5%, defined content size = 6611
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 6611, 5);		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0013 = "/api/events/event/getcategorizedevents";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 12814
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "A bunch of bug fixes & several minor improvements");		// Test [13] <- Index 13
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0014 = "dev.dev.playsite.devmpp.com";
		int    requestPort0014 = 80;
		String requestFile0014 = "/api/events/event/CheckCanPlayEvent/e5cd29ca906b4be48fcddfd11b45561a" +
		...
		String requestHeader0014 = "GET " + requestFile0014 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: THBvNEwzVGR3eThBVWNMbmFVM3l2SWlSUXByeFlwaXdXajhNWXhNSFhEST06YW5vbnltb3VzOjYzNjIzMDE4NTg3Mzk1MDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 351
		// content test algorithm: check size +/- 5%, defined content size = 351
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 351, 5);		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0015 = "mediasvcsk8gh98mxvd2r.blob.core.windows.net";
		int    requestPort0015 = 443;
		String requestFile0015 = "/asset-9ed4f95e-8534-481c-9306-b5484dac50b5/Mix_Tape-_Shane_Warne,_Volume_1_1.jpg" +
				"?sv=2012-02-12" +
				"&sr=c" +
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {403}, null, null, null);		// Test [15] <- Index 15
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0016 = "cdn.sharepointonline.com";
		int    requestPort0016 = 80;
		String requestFile0016 = "/15772/_layouts/15/fonts/Office365Icons.woff";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/FONT-WOFF", null, 47204, 5);		// Test [16] <- Index 16
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0017 = "kumo2stage.blob.core.windows.net";
		int    requestPort0017 = 443;
		String requestFile0017 = "/asset-7cd70796-e388-459d-b289-d669e3b718b5/f1fea1185cc04dabcf83e9662204c3e5_000001.jpg" +
				"?sv=2012-02-12" +
				"&sr=c" +
		...
		// verify response: status code = 403, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 404
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {403}, null, null, null);		// Test [17] <- Index 17
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0018 = "127.0.0.1";
		int    requestPort0018 = 7990;
		String requestFile0018 = "/dfischer/webadmininterface/BrowserExtensionWeblet" +
				"?action=getRecordState";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 4
		// content test algorithm: check size +/- 5%, defined content size = 4
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 4, 5);		// Test [18] <- Index 18
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0019 = "player.mpp5.devmpp.com";
		int    requestPort0019 = 443;
		String requestFile0019 = "/play/channel/6559faa5cbee47149bbb751afd3cbd13/e5cd29ca906b4be48fcddfd11b45561a" +
				"?autoplay=true";
		String requestHeader0019 = "GET " + requestFile0019 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: player.mpp5.devmpp.com\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n" + 
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate, br\r\n" + 
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 3430
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div class=\"countdownColon3 countdownColons\"");		// Test [19] <- Index 19
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 1961
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "var redirectLocation = window.location.origin;");		// Test [20] <- Index 20
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	boolean executePage_1_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 281
		// content test algorithm: check size +/- 5%, defined content size = 281
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 281, 5);		// Test [21] <- Index 21
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [22] <- WEB ADMIN Index 28 ---
		...
		String requestHeader0022 = "GET " + requestFile0022 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: Q3pNbEgrNWxSc2pvcVFhTXFCQVhkR3p3THc2bjBneWxWV1lRZlQvL04rMD06YW5vbnltb3VzOjYzNjIzMDE4NTkwODUyMDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 1363
		// content test algorithm: check size +/- 5%, defined content size = 1363
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 1363, 5);		// Test [22] <- Index 22
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [23] <- WEB ADMIN Index 29 ---
		String requestProt0023 = "http";
		String requestHost0023 = "dev.dev.playsite.devmpp.com";
		int    requestPort0023 = 80;
		String requestFile0023 = "/api/authorization/account/isauthenticated";
		String requestHeader0023 = "GET " + requestFile0023 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: ak5lRGFjRS8zWXBZREtEeEdqMC9FVGR3dzNqNko2WVJhSGxJVkVod1FBWT06YW5vbnltb3VzOjYzNjIzMDE4NTkxMjAyMDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 228
		// content test algorithm: check size +/- 5%, defined content size = 228
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 228, 5);		// Test [23] <- Index 23
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [24] <- WEB ADMIN Index 30 ---
		String requestProt0024 = "http";
		String requestHost0024 = "dev.dev.playsite.devmpp.com";
		int    requestPort0024 = 80;
		String requestFile0024 = "/api/authorization/account/getaccountoverview";
		String requestHeader0024 = "GET " + requestFile0024 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: RlJ2MSszc0FpVzZQNGpsU0VPaFd2U3pvaFVPdng5WnJ4RDkxejJSa1Uycz06YW5vbnltb3VzOjYzNjIzMDE4NTkxNTcxMDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 213, 5);		// Test [24] <- Index 24
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [25] <- WEB ADMIN Index 31 ---
		String requestProt0025 = "http";
		String requestHost0025 = "127.0.0.1";
		int    requestPort0025 = 7990;
		String requestFile0025 = "/dfischer/webadmininterface/BrowserExtensionWeblet" +
				"?action=getRecordState";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 4
		// content test algorithm: check size +/- 5%, defined content size = 4
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 4, 5);		// Test [25] <- Index 25
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [26] <- WEB ADMIN Index 32 ---
		String requestProt0026 = "http";
		String requestHost0026 = "127.0.0.1";
		int    requestPort0026 = 7990;
		String requestFile0026 = "/dfischer/webadmininterface/BrowserExtensionWeblet" +
				"?action=getRecordState";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 4
		// content test algorithm: check size +/- 5%, defined content size = 4
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 4, 5);		// Test [26] <- Index 26
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [27] <- WEB ADMIN Index 33 ---
		String requestProt0027 = "http";
		String requestHost0027 = "127.0.0.1";
		int    requestPort0027 = 7990;
		String requestFile0027 = "/dfischer/webadmininterface/BrowserExtensionWeblet" +
				"?action=getRecordState";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 4
		// content test algorithm: check size +/- 5%, defined content size = 4
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 4, 5);		// Test [27] <- Index 27
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [28] <- WEB ADMIN Index 34 ---
		String requestProt0028 = "http";
		String requestHost0028 = "dev.dev.playsite.devmpp.com";
		int    requestPort0028 = 80;
		String requestFile0028 = "/api/authorization/account/register";
		String requestContent0028 = 
				"{\"email\":\"amir1106@gmail.com\",\"password\":\"paistanLAHORE\",\"firstName\":\"Amir\",\"lastName\":\"Khan\"}";
		requestContent0028 = Lib.replaceTextPattern(requestContent0028, "paistanLAHORE", user_pass, 0);		// search and replace content fragment "paistanLAHORE" with variable 'user_pass' on all occurrences
		requestContent0028 = Lib.replaceTextPattern(requestContent0028, "Khan", user_lastName, 0);		// search and replace content fragment "Khan" with variable 'user_lastName' on all occurrences
		requestContent0028 = Lib.replaceTextPattern(requestContent0028, "amir1106@gmail.com", user_email, 0);		// search and replace content fragment "amir1106@gmail.com" with variable 'user_email' on all occurrences
		requestContent0028 = Lib.replaceTextPattern(requestContent0028, "Amir", user_fName, 0);		// search and replace content fragment "Amir" with variable 'user_fName' on all occurrences

		byte[] requestContentBinary0028 = requestContent0028.getBytes("UTF-8");
		String requestHeader0028 = "POST " + requestFile0028 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Authorization: cnRCSk1NZ0NpdEpHY0tCclNhZ0ROVFovZG9nUVU4ZW5iSk9WM1ZjRWd2bz06YW5vbnltb3VzOjYzNjIzMDE4NjEwNjI1MDAwMA==\r\n" + 
				"Content-Length: " + requestContentBinary0028.length + "\r\n" + 		// Content-Length: 94
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0028, requestHost0028, requestPort0028, requestHeader0028, requestContentBinary0028, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		requestContentBinary0028 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());

		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 371
		// content test algorithm: check size +/- 5%, defined content size = 371
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 371, 5);		// Test [28] <- Index 28
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [29] <- WEB ADMIN Index 35 ---
		String requestProt0029 = "http";
		String requestHost0029 = "127.0.0.1";
		int    requestPort0029 = 7990;
		String requestFile0029 = "/dfischer/webadmininterface/BrowserExtensionWeblet" +
				"?action=getRecordState";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 4
		// content test algorithm: check size +/- 5%, defined content size = 4
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 4, 5);		// Test [29] <- Index 29
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 281
		// content test algorithm: check size +/- 5%, defined content size = 281
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 281, 5);		// Test [30] <- Index 30
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [31] <- WEB ADMIN Index 37 ---
		String requestProt0031 = "http";
		String requestHost0031 = "dev.dev.playsite.devmpp.com";
		int    requestPort0031 = 80;
		String requestFile0031 = "/api/authorization/account/login";
		String requestContent0031 = 
				"{\"username\":\"amir1106@gmail.com\",\"email\":\"\",\"password\":\"paistanLAHORE\",\"confirmpassword\":\"\"}";
		requestContent0031 = Lib.replaceTextPattern(requestContent0031, "paistanLAHORE", user_pass, 0);		// search and replace content fragment "paistanLAHORE" with variable 'user_pass' on all occurrences
		requestContent0031 = Lib.replaceTextPattern(requestContent0031, "amir1106@gmail.com", user_email, 0);		// search and replace content fragment "amir1106@gmail.com" with variable 'user_email' on all occurrences

		byte[] requestContentBinary0031 = requestContent0031.getBytes("UTF-8");
		String requestHeader0031 = "POST " + requestFile0031 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Content-Type: application/json;charset=utf-8\r\n" + 
				"Authorization: dWg2bVFvNU5HRyt5NkkxRzh6ekFlS0ZOMkJUTVN3NlB4Q1FSdlZpK0FhUT06YW1pcjExMDZAZ21haWwuY29tOjYzNjIzMDE4NjIxODg1MDAwMA==\r\n" + 
				"Content-Length: " + requestContentBinary0031.length + "\r\n" + 		// Content-Length: 92
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0031, requestHost0031, requestPort0031, requestHeader0031, requestContentBinary0031, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		requestContentBinary0031 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 353, 5);		// Test [31] <- Index 31
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [32] <- WEB ADMIN Index 38 ---
		String requestProt0032 = "http";
		String requestHost0032 = "dev.dev.playsite.devmpp.com";
		int    requestPort0032 = 80;
		String requestFile0032 = "/api/authorization/account/getaccountoverview";
		String requestHeader0032 = "GET " + requestFile0032 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: dEN2UzY5WmVPeHVlM2psSVRiTUJGSXRjQVZOWlEvWDdsZlNERU1va3I4QT06YW1pcjExMDZAZ21haWwuY29tOjYzNjIzMDE4NjIyMDkyMDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 6358
		// content test algorithm: check size +/- 5%, defined content size = 6358
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 6358, 5);		// Test [32] <- Index 32
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [33] <- WEB ADMIN Index 39 ---
		String requestProt0033 = "http";
		String requestHost0033 = "127.0.0.1";
		int    requestPort0033 = 7990;
		String requestFile0033 = "/dfischer/webadmininterface/BrowserExtensionWeblet" +
				"?action=getRecordState";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 4
		// content test algorithm: check size +/- 5%, defined content size = 4
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 4, 5);		// Test [33] <- Index 33
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [34] <- WEB ADMIN Index 40 ---
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 1961
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "var redirectLocation = window.location.origin;");		// Test [34] <- Index 34
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 281
		// content test algorithm: check size +/- 5%, defined content size = 281
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 281, 5);		// Test [35] <- Index 35
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHeader0036 = "GET " + requestFile0036 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: dVpaeEMwNGs2TFpwWXg4UDBMNDN5eTVNcklqWDlKKzJyOWYzakxKb0JqTT06YW1pcjExMDZAZ21haWwuY29tOjYzNjIzMDE4NjIzODY4MDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 1363
		// content test algorithm: check size +/- 5%, defined content size = 1363
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 1363, 5);		// Test [36] <- Index 36
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [37] <- WEB ADMIN Index 43 ---
		String requestProt0037 = "http";
		String requestHost0037 = "dev.dev.playsite.devmpp.com";
		int    requestPort0037 = 80;
		String requestFile0037 = "/api/authorization/account/isauthenticated";
		String requestHeader0037 = "GET " + requestFile0037 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: SUZVZktQbWlkTjJwQTJRTndSQWlZKzBJenJwMkhzK01iS1BnTjRncWpIaz06YW1pcjExMDZAZ21haWwuY29tOjYzNjIzMDE4NjIzOTQxMDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 228
		// content test algorithm: check size +/- 5%, defined content size = 228
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 228, 5);		// Test [37] <- Index 37
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [38] <- WEB ADMIN Index 44 ---
		String requestProt0038 = "http";
		String requestHost0038 = "dev.dev.playsite.devmpp.com";
		int    requestPort0038 = 80;
		String requestFile0038 = "/api/authorization/account/getaccountoverview";
		String requestHeader0038 = "GET " + requestFile0038 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: UFZIenowd2p0a2dVbWVYU3Q2c05kdlpXU2IzZG1oYXd1UmtibE0rR3ZHYz06YW1pcjExMDZAZ21haWwuY29tOjYzNjIzMDE4NjI0MDQ2MDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 6358
		// content test algorithm: check size +/- 5%, defined content size = 6358
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 6358, 5);		// Test [38] <- Index 38
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [39] <- WEB ADMIN Index 45 ---
		...
		String requestHeader0039 = "GET " + requestFile0039 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: SXdaOVlqSTlrdGlic21lRk1TSXhKQlQ3ZFFGNU5YcytNUUp5YldpZUxoUT06YW1pcjExMDZAZ21haWwuY29tOjYzNjIzMDE4NjI0NzMwMDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 8327
		// content test algorithm: check size +/- 5%, defined content size = 8327
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 8327, 5);		// Test [39] <- Index 39
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 648
		// content test algorithm: check size +/- 5%, defined content size = 648
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 648, 5);		// Test [40] <- Index 40
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	boolean executePage_1_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0041 = "/api/events/event/getaggregatedevents";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 6611
		// content test algorithm: check size +/- 5%, defined content size = 6611
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 6611, 5);		// Test [41] <- Index 41
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [42] <- WEB ADMIN Index 48 ---
		String requestProt0042 = "http";
		String requestHost0042 = "dev.dev.playsite.devmpp.com";
		int    requestPort0042 = 80;
		String requestFile0042 = "/api/events/event/getcategorizedevents";
		String requestHeader0042 = "GET " + requestFile0042 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: RU5INWVYV000WnZWNGxHMGVHcWQvRFRTS2Rpd0d4ZkYvYlFBb3RZbTBzVT06YW1pcjExMDZAZ21haWwuY29tOjYzNjIzMDE4NjI0NzMxMDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 12814
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "A bunch of bug fixes & several minor improvements");		// Test [42] <- Index 42
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [43] <- WEB ADMIN Index 49 ---
		String requestProt0043 = "https";
		String requestHost0043 = "mediasvcsk8gh98mxvd2r.blob.core.windows.net";
		int    requestPort0043 = 443;
		String requestFile0043 = "/asset-9ed4f95e-8534-481c-9306-b5484dac50b5/Mix_Tape-_Shane_Warne,_Volume_1_1.jpg" +
				"?sv=2012-02-12" +
				"&sr=c" +
				"&si=abdcfcd6-5c33-40b1-b368-2783be8cc318" +
				"&sig=859uj99l5fZIMTFO2atvPbTV41jYQERYriGxyCTJpHY%3D" +
		...
		// --- HTTP REQUEST: Test [44] <- WEB ADMIN Index 50 ---
		String requestProt0044 = "https";
		String requestHost0044 = "kumo2stage.blob.core.windows.net";
		int    requestPort0044 = 443;
		String requestFile0044 = "/asset-7cd70796-e388-459d-b289-d669e3b718b5/f1fea1185cc04dabcf83e9662204c3e5_000001.jpg" +
				"?sv=2012-02-12" +
				"&sr=c" +
				"&si=14f48bf5-09d3-45be-aeaf-f3f790c89492" +
				"&sig=h5oFcKhBUr%2FHERlr0%2BkLxQ%2BIy0IVP85%2FeyESwD2woEE%3D" +
		...
		// --- HTTP REQUEST: Test [45] <- WEB ADMIN Index 51 ---
		String requestProt0045 = "http";
		String requestHost0045 = "dev.dev.playsite.devmpp.com";
		int    requestPort0045 = 80;
		String requestFile0045 = "/api/events/event/CheckCanPlayEvent/e5cd29ca906b4be48fcddfd11b45561a" +
				"?eventTypeString=Channel" +
				"&eventHolderId=6559faa5cbee47149bbb751afd3cbd13" +
				"&withAutoStart=true";
		String requestHeader0045 = "GET " + requestFile0045 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Authorization: S0RzWmVqREJzc2x0MEVzWE1KOEhUM1dkcXlGS3Npaml6MTE4MFBCMmQvcz06YW1pcjExMDZAZ21haWwuY29tOjYzNjIzMDE4NjI1NTc1MDAwMA==\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 679
		// content test algorithm: check size +/- 5%, defined content size = 679
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 679, 5);		// Test [45] <- Index 45
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [46] <- WEB ADMIN Index 52 ---
		String requestProt0046 = "http";
		String requestHost0046 = "cdn.sharepointonline.com";
		int    requestPort0046 = 80;
		String requestFile0046 = "/15772/_layouts/15/fonts/Office365Icons.woff";
		...
		// verify response: status code = 200, content type = "APPLICATION/FONT-WOFF", header text fragment = [verification disabled], recorded content size = 47204
		// content test algorithm: check size +/- 5%, defined content size = 47204
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/FONT-WOFF", null, 47204, 5);		// Test [46] <- Index 46
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [47] <- WEB ADMIN Index 53 ---
		String requestProt0047 = "https";
		String requestHost0047 = "player.mpp5.devmpp.com";
		int    requestPort0047 = 443;
		String requestFile0047 = "/play/channel/6559faa5cbee47149bbb751afd3cbd13/e5cd29ca906b4be48fcddfd11b45561a" +
				"?autoplay=true" +
				"&u=954f974aa0fb4a33902cf62540b9c436" +
				"&un=amir1106@gmail.com" +
				"&ut=2dcee9402d500598fd12522f6659cb4b6217f2fdde8fac6e3fd1fe2ee212a7f0,oGBM2yemf1" +
				"&text=92.254.162.241channel6559faa5cbee47149bbb751afd3cbd13e5cd29ca906b4be48fcddfd11b45561a954f974aa0fb4a33902cf62540b9c436amir1106@gmail.comabc123oGBM2yemf1" +
				"&apidebug=%20ServerVariableKey:HTTP_X_FORWARDED_FOR%20IP:92.254.162.241%20MS_HttpContextIP:92.254.162.241%20UserHostAddress:92.254.162.241%20UserAgent:Mozilla/5.0%20(Macintosh;%20Intel%20Mac%20OS%20X%2010.12;%20rv:51.0)%20Gecko/20100101%20Firefox/51.0";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 3680
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div class=\"countdownColon3 countdownColons\"");		// Test [47] <- Index 47
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("Az2", concurrentUsers);
		...
		final int MAX_THREAD_STEPS = 47;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("Az2");
		...
		{
			System.out.println("global var <<< user_email = " + user_email);
			System.out.println("global var <<< user_fName = " + user_fName);
			System.out.println("global var <<< user_lastName = " + user_lastName);
			System.out.println("global var <<< user_pass = " + user_pass);
			System.out.println();
		}
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new Az2());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				Az2 simulatedUser = new Az2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
