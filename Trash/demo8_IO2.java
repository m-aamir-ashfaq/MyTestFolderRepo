...
// -----------------------------------------
//
// Source: demo8_IO2.java
// Date  : 11 Aug 2016 10:37:59 ECT
 ...
 * Automatically generated load test program.
 */
public class demo8_IO2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "demo8_IO2ExtParams.dat";             // input file name for very large parameter values
	...
	private static String time = "" + System.currentTimeMillis();                // var declaration from web admin var handler: scope = global
	 ...
	 */
	public demo8_IO2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log("# Page #2: loaded");
		log("# ---------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: loaded", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		log();
		log("# title: Apica Panel");
		String requestProt0001 = "https";
		String requestHost0001 = "panel.apicasystem.com";
		int    requestPort0001 = 443;
		String requestFile0001 = "/share/DiWAJ5/";
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: panel.apicasystem.com\r\n" + 
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 38617
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "alert('You are using an incompatible version of Internet Explorer' +");		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		String requestHost0002 = "socket.leftronic.com";
		int    requestPort0002 = 443;
		String requestFile0002 = "/socket.io/1/" +
				"?t=1470903812860";
		String requestHeader0002 = "GET " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: socket.leftronic.com\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n" + 
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate, br\r\n" + 
				"Origin: https://panel.apicasystem.com\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "TEXT/PLAIN", header text fragment = [verification disabled], recorded content size = 70
		// content test algorithm: check size +/- 5%, defined content size = 70
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/PLAIN", null, 70, 5);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0003 = "panel.apicasystem.com";
		int    requestPort0003 = 443;
		String requestFile0003 = "/lt/ping/DiWAJ5";
		...
		String requestHeader0003 = "POST " + requestFile0003 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"X-CSRFToken: j8mPubqjB8SVvUmJNQ5KAE5XpuvETnaa\r\n" + 
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 59
		// content test algorithm: check size +/- 5%, defined content size = 59
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 59, 5);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [4] <- WEB ADMIN Index 7 ---
		// execute inline script "makeTime10Digit"
		inlineScriptContext = executeInlineScript_1470904662212(threadStep, totalLoopCounter, innerLoopContext);
		...
		String requestHost0004 = "socket.leftronic.com";
		int    requestPort0004 = 443;
		String requestFile0004 = "/socket.io/1/websocket/1686981358193297010";
		String requestHeader0004 = "GET " + requestFile0004 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: XIf6BVjxjdcOJDTEcpzvJA==\r\n" + 
		...
		webSocketData = getWebSocketData_1470903813108();
		...
		// --- HTTP REQUEST: Test [5] <- WEB ADMIN Index 8 ---
		String requestProt0005 = "https";
		String requestHost0005 = "api.mixpanel.com";
		int    requestPort0005 = 443;
		String requestFile0005 = "/engage/" +
				"?data=eyIkYWRkIjogeyJMb2FkZWQgYXBwIjogMX0sIiR0b2tlbiI6ICIyY2M1YWY2ZTYxZWYzYjg1MDg5MTZlODIxZDQ2OTA2MiIsIiRkaXN0aW5jdF9pZCI6ICIxOTkzOCJ9" +
				"&ip=1" +
				"&_=1470903813063";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 1
		// content test algorithm: check size +/- 5%, defined content size = 1
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 1, 5);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [6] <- WEB ADMIN Index 9 ---
		String requestProt0006 = "https";
		String requestHost0006 = "api.mixpanel.com";
		int    requestPort0006 = 443;
		String requestFile0006 = "/decide/" +
				"?verbose=1" +
				"&version=1" +
				"&lib=web" +
				"&token=2cc5af6e61ef3b8508916e821d469062" +
				"&ip=1" +
				"&_=1470903813054";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 84
		// content test algorithm: check size +/- 5%, defined content size = 84
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 84, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [7] <- WEB ADMIN Index 10 ---
		String requestProt0007 = "https";
		String requestHost0007 = "api.mixpanel.com";
		int    requestPort0007 = 443;
		String requestFile0007 = "/decide/" +
				"?verbose=1" +
				"&version=1" +
				"&lib=web" +
				"&token=2cc5af6e61ef3b8508916e821d469062" +
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 84
		// content test algorithm: check size +/- 5%, defined content size = 84
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 84, 5);		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [8] <- WEB ADMIN Index 11 ---
		String requestProt0008 = "https";
		String requestHost0008 = "api.mixpanel.com";
		int    requestPort0008 = 443;
		String requestFile0008 = "/track/" +
				"?data=%3D" +
				"&ip=1" +
				"&_=1470903813074";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 1
		// content test algorithm: check size +/- 5%, defined content size = 1
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 1, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0009 = "panel.apicasystem.com";
		int    requestPort0009 = 443;
		String requestFile0009 = "/api/config/widget/getDashboardWidgets/";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 2746
		// content test algorithm: check size +/- 5%, defined content size = 2746
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 2746, 5);		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [10] <- WEB ADMIN Index 13 ---
		String requestProt0010 = "https";
		String requestHost0010 = "heapanalytics.com";
		int    requestPort0010 = 443;
		String requestFile0010 = "/h" +
				"?a=3933777330" +
				"&u=8123875822259385" +
				"&v=5667824656034042" +
				"&s=7845791707513070" +
				"&b=web" +
				"&z=2" +
				"&h=%2Fshare%2FDiWAJ5%2F" +
				"&d=panel.apicasystem.com" +
				"&t=Apica%20Panel" +
				"&k=Account%20ID" +
				"&k=35632" +
				"&ts=1470903813182" +
				...
				"&sp=1470901268261" +
				"&sp=d" +
				"&sp=panel.apicasystem.com" +
				"&sp=h" +
				"&sp=%2Fshare%2FDiWAJ5%2F" +
				"&st=1470903813184";
		...
		// verify response: status code = 200, content type = "IMAGE/GIF", header text fragment = [verification disabled], recorded content size = 37
		// content test algorithm: check size +/- 5%, defined content size = 37
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/GIF", null, 37, 5);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
		...
		// --- HTTP REQUEST: Test [11] <- WEB ADMIN Index 14 ---
		String requestProt0011 = "https";
		String requestHost0011 = "heapanalytics.com";
		int    requestPort0011 = 443;
		String requestFile0011 = "/api/identify" +
				"?a=3933777330" +
				"&u=8123875822259385" +
				"&v=5667824656034042" +
				"&s=7845791707513070" +
		...
		// content test algorithm: check size +/- 5%, defined content size = 73
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 73, 5);		// Test [11] <- Index 11
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [12] <- WEB ADMIN Index 15 ---
		String requestProt0012 = "https";
		String requestHost0012 = "panel.apicasystem.com";
		int    requestPort0012 = 443;
		String requestFile0012 = "/api/retrieve/initDataStream/";
		String requestContent0012 = 
				"{\"dashboard\":\"DiWAJ5\",\"latestTimestamps\":{}}=";
		byte[] requestContentBinary0012 = requestContent0012.getBytes("UTF-8");
		String requestHeader0012 = "POST " + requestFile0012 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"X-CSRFToken: j8mPubqjB8SVvUmJNQ5KAE5XpuvETnaa\r\n" + 
				"X-Requested-With: XMLHttpRequest\r\n" + 
				"Content-Length: " + requestContentBinary0012.length + "\r\n" + 		// Content-Length: 44
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0012, requestHost0012, requestPort0012, requestHeader0012, requestContentBinary0012, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		requestContentBinary0012 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());

		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 25679
		// content test algorithm: check size +/- 5%, defined content size = 25679
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 25679, 5);		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 5078
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div class=\"collapse navbar-collapse navbar-right\"");		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0015 = "www.google-analytics.com";
		...
		String requestHost0016 = "stats.g.doubleclick.net";
		int    requestPort0016 = 443;
		String requestFile0016 = "/r/collect" +
				"?v=1" +
				"&aip=1" +
				"&t=dc" +
				"&_r=3" +
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("demo8_IO2", concurrentUsers);
		...
		final int MAX_THREAD_STEPS = 15;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("demo8_IO2");
		...
		{
			System.out.println("global var <<< time = " + time);
			System.out.println();
		}
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				demo8_IO2 simulatedUser = new demo8_IO2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "makeTime10Digit"
	public static String getInlineScriptCode_1470904662212()
	{
		return "print time\r\nstr = strSubString(time,1,10)\r\ntime = toNumeric(str)\r\nprint time";
	}
	
	
	/**
	* Execute the inline script "makeTime10Digit"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1470904662212(int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("makeTime10Digit", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_LOOP_ITEM_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler);
		inlineScriptContext.setThreadStep(threadStep);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1470904662212(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("time", Lib.nullToBlank(time), 3);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("time", Lib.nullToBlank(time), 3);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar1);
		inlineScriptExecutor.execute();		// execute inline script
		time = inlineScriptContext.getOutputVar("time").getVarValue();
		log("<<< time = " + time);
	...
	public static WebSocketData getWebSocketData_1470903813108()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1470903813227L);
		webSocketData.setConnectionStopMillis(1470903823847L);
		String payLoad_0 = "1::";
		String payLoad_1 = "5:::{\"name\":\"signature\",\"args\":[\"/node?dashboard=DiWAJ5&group=0&shared=1&time=1470903840&user=35632&signature=35e0ded608216b84f2df4c366615d2a5ce86bf30\"]}";
		String payLoad_2 = "5:::{\"name\":\"ping\",\"args\":[null]}";
		String payLoad_3 = "5:::{\"name\":\"request\",\"args\":[\"dashboardId\"]}";
		String payLoad_4 = "5:::{\"name\":\"message\",\"args\":[{\"dashboardId\":\"DiWAJ5\"}]}";
		String payLoad_5 = "4:::{\"pong\":true}";
		String payLoad_6 = "5:::{\"name\":\"message\",\"args\":[{\"dashboardId\":\"DiWAJ5\"}]}";
		String payLoad_7 = "\u0003\u00fd";

		// search and replace frame payload "1470903840" with variable 'time' on all occurrences of frame no 1
		payLoad_1 = Lib.replaceTextPattern(payLoad_1, "1470903840", time, 0);


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , false , (byte) 1 , 1470903813228L,1470903813229L , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1470903813231L,1470903813232L , payLoad_1 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1470903813233L,1470903813233L , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , false , (byte) 1 , 1470903813348L,1470903813349L , payLoad_3 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1470903813351L,1470903813352L , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , false , (byte) 1 , 1470903813464L,1470903813465L , payLoad_5 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1470903813926L,1470903813929L , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1470903823710L,1470903823712L , payLoad_7 ));
