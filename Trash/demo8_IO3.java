...
// -----------------------------------------
//
// Source: demo8_IO3.java
// Date  : 11 Aug 2016 11:38:01 ECT
 ...
 * Automatically generated load test program.
 */
public class demo8_IO3 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "demo8_IO3ExtParams.dat";             // input file name for very large parameter values
	...
	private static String guidSignature = "kkj";                                 // var declaration from web admin var handler: scope = global
	...
	private String timeStartLoop = null;                 // var declaration from web admin var handler: scope = per loop
	private String timeForFirstFrame = null;             // var declaration from web admin var handler: scope = per loop
	private String idFromContent = null;                 // var declaration from web admin var handler: scope = per loop
	 ...
	 */
	public demo8_IO3(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		timeForFirstFrame = "" + System.currentTimeMillis();
		idFromContent = null;
		log();
		log("<<< timeStartLoop = " + timeStartLoop);
		log("<<< timeForFirstFrame = " + timeForFirstFrame);
		log("<<< idFromContent = " + idFromContent);
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [1] <- WEB ADMIN Index 3 ---
		...
		String requestFile0001 = "/socket.io/1/" +
				"?t=1470903812860";
		requestFile0001 = Lib.replaceTextPattern(requestFile0001, "1470903812860", timeStartLoop, 0);		// search and replace request file fragment "1470903812860" with variable 'timeStartLoop' on all occurrences
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/PLAIN", null, 70, 5);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		// setup var extractor for 'idFromContent'
		...
		try
		{
			// extract var 'idFromContent'
			idFromContent = lineTokenExtractorItem0001_1.getResultToken();
			log("<<< idFromContent = " + idFromContent);
		}
		catch (Exception e) { idFromContent = null; log(e); }
		if (idFromContent == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'idFromContent' from text line pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0002 = "/socket.io/1/websocket/1686981358193297010";
		requestFile0002 = Lib.replaceTextPattern(requestFile0002, "1686981358193297010", idFromContent, 0);		// search and replace request file fragment "1686981358193297010" with variable 'idFromContent' on all occurrences
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [4] <- WEB ADMIN Index 17 ---
		log();
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("demo8_IO3", concurrentUsers);
		...
		final int MAX_THREAD_STEPS = 3;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("demo8_IO3");
		...
		{
			System.out.println("global var <<< guidSignature = " + guidSignature);
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				demo8_IO3 simulatedUser = new demo8_IO3(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	{
		return "str = strSubString(timeForFirstFrame,1,10)\r\ntimeForFirstFrame = toNumeric(str)\r\n\r\nguidSignature = generateGUID(\"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\")\r\n\r\n//print guidSignature\r\n\r\n";
	}
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1470904662212(int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		...
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("guidSignature", Lib.nullToBlank(guidSignature), 3);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar inputVar2 = new LoadtestInlineScriptVar("timeForFirstFrame", Lib.nullToBlank(timeForFirstFrame), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar2);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("guidSignature", Lib.nullToBlank(guidSignature), 3);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar1);
		LoadtestInlineScriptVar outputVar2 = new LoadtestInlineScriptVar("timeForFirstFrame", Lib.nullToBlank(timeForFirstFrame), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar2);
		inlineScriptExecutor.execute();		// execute inline script
		guidSignature = inlineScriptContext.getOutputVar("guidSignature").getVarValue();
		log("<<< guidSignature = " + guidSignature);
		timeForFirstFrame = inlineScriptContext.getOutputVar("timeForFirstFrame").getVarValue();
		log("<<< timeForFirstFrame = " + timeForFirstFrame);
	...
	public static WebSocketData getWebSocketData_1470903813108()
	{
		...
		// search and replace frame payload "35e0ded608216b84f2df4c366615d2a5ce86bf30" with variable 'guidSignature' on all occurrences of frame no 1
		payLoad_1 = Lib.replaceTextPattern(payLoad_1, "35e0ded608216b84f2df4c366615d2a5ce86bf30", guidSignature, 0);

		// search and replace frame payload "1470903840" with variable 'timeForFirstFrame' on all occurrences of frame no 1
		payLoad_1 = Lib.replaceTextPattern(payLoad_1, "1470903840", timeForFirstFrame, 0);
