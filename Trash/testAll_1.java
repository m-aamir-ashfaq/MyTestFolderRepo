...
// -----------------------------------------
//
// Source: testAll_1.java
// Date  : 27 Jan 2017 09:48:57 ECT
 ...
 * Automatically generated load test program.
 */
public class testAll_1 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "testAll_1ExtParams.dat";             // input file name for very large parameter values
	...
	private static String clientID = "";                                         // var declaration from web admin var handler: scope = global
	 ...
	 */
	public testAll_1(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "self-repair.mozilla.org";
		int    requestPort0001 = 443;
		String requestFile0001 = "/en-GB/repair";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 292
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "data-classify-url=\"/api/v1/classify_client/\"");		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		String requestHost0002 = "self-repair.mozilla.org";
		int    requestPort0002 = 443;
		String requestFile0002 = "/api/v1/recipe/" +
				"?enabled=true";
		...
		// content test algorithm: check size +/- 5%, defined content size = 970
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 970, 5);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0003 = "self-repair.mozilla.org";
		int    requestPort0003 = 443;
		String requestFile0003 = "/api/v1/classify_client/";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 80, 5);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0004 = "piwik.baycix.de";
		int    requestPort0004 = 80;
		String requestFile0004 = "/piwik.php" +
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/GIF", null, 50, 5);		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
		...
		// execute inline script "generate clientId"
		inlineScriptContext = executeInlineScript_1485506910996(testURL, urlCallPassed, threadStep - 1, totalLoopCounter, innerLoopContext);
		...
		String requestHost0005 = "broker.mqttdashboard.com";
		int    requestPort0005 = 8000;
		...
		String requestHost0006 = "cdnjs.cloudflare.com";
		int    requestPort0006 = 80;
		String requestFile0006 = "/ajax/libs/fancybox/2.1.5/fancybox_sprite.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 1362
		// content test algorithm: check size +/- 5%, defined content size = 1362
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 1362, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0007 = "incoming.telemetry.mozilla.org";
		int    requestPort0007 = 443;
		String requestFile0007 = "/submit/telemetry/e3f70652-7cb2-eb48-af70-990dfa0c7378/main/Firefox/50.1.0/release/20161208153507" +
				"?v=4";
		String requestContent0007 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "testAll_1_RequestContent0007.txt").getFileDataAsString("UTF-8");		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
		byte[] requestContentBinary0007 = requestContent0007.getBytes("UTF-8");
		String requestHeader0007 = "POST " + requestFile0007 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Date: Tue, 17 Jan 2017 14:40:51 GMT\r\n" + 
				"Content-Encoding: gzip\r\n" + 
				"Content-Length: " + requestContentBinary0007.length + "\r\n" + 		// Content-Length: 13303
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0007, requestHost0007, requestPort0007, requestHeader0007, requestContentBinary0007, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		requestContentBinary0007 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/PLAIN", null, 20, 5);		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0008 = "aus5.mozilla.org";
		int    requestPort0008 = 443;
		String requestFile0008 = "/update/3/GMP/50.1.0/20161208153507/Darwin_x86_64-gcc3-u-i386-x86_64/en-GB/release/Darwin%2016.3.0/default/default/update.xml";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, 443, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0009 = "shuttle3";
		int    requestPort0009 = 8000;
		String requestFile0009 = "/mqtt";
		...
		String requestFile0010 = "/r/collect" +
				"?v=1" +
				"&_v=j47" +
				"&a=1174468286" +
				"&t=pageview" +
				"&_s=1" +
				"&dl=http%3A%2F%2Fmitsuruog.github.io%2Fwhat-mqtt%2F" +
				"&ul=en-gb" +
				"&de=UTF-8" +
				"&dt=MQTT%20on%20Websocket%20sample" +
				"&sd=24-bit" +
				"&sr=1920x1080" +
				"&vp=1920x950" +
				"&je=1" +
		...
		log();
		log("# title: Site not found &middot; GitHub Pages");
		String requestProt0011 = "http";
		String requestHost0011 = "mitsuruog.github.io";
		...
		// verify response: status code = 404, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 5216
		...
		String requestHost0012 = "test.mosquitto.org";
		int    requestPort0012 = 8080;
		String requestFile0012 = "/mqtt";
		...
		// verify response: status code = 101, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: check size +/- 5%, defined content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {101}, null, null, 0, 5);		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0013 = "test.mosquitto.org";
		int    requestPort0013 = 8080;
		String requestFile0013 = "/mqtt";
		...
		String requestFile0014 = "/r/__utm.gif" +
				"?utmwv=5.6.7" +
				"&utms=1" +
				"&utmn=1970146331" +
				"&utmhn=test.mosquitto.org" +
				"&utmcs=windows-1252" +
				"&utmsr=1920x1080" +
				"&utmvp=1920x950" +
				"&utmsc=24-bit" +
				...
				"&utmje=1" +
				"&utmfl=24.0%20r0" +
				"&utmdt=MQTT%20over%20WebSockets" +
				"&utmhid=1431485049" +
				"&utmr=-" +
				"&utmp=%2Fws.html" +
				"&utmht=1484664127101" +
				"&utmac=UA-28176897-2" +
				"&utmcc=__utma%3D69922325.2089761595.1484655176.1484655176.1484664127.2%3B%2B__utmz%3D69922325.1484655176.1.1.utmcsr%3D(direct)%7Cutmccn%3D(direct)%7Cutmcmd%3D(none)%3B" +
				"&utmjid=146716857" +
				"&utmredir=1" +
		...
		// verify response: status code = 200, content type = "IMAGE/GIF", header text fragment = [verification disabled], recorded content size = 35
		// content test algorithm: check size +/- 5%, defined content size = 35
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/GIF", null, 35, 5);		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0015 = "test.mosquitto.org";
		int    requestPort0015 = 80;
		String requestFile0015 = "/favicon.ico";
		...
		// --- HTTP REQUEST: Test [16] <- WEB ADMIN Index 24 ---
		String requestProt0016 = "http";
		String requestHost0016 = "test.mosquitto.org";
		int    requestPort0016 = 8080;
		String requestFile0016 = "/mqtt";
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("testAll_1", concurrentUsers);
		...
		final int MAX_THREAD_STEPS = 16;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("testAll_1");
		...
		try
		{
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_1);		// testAll_1_RequestContent0007.txt
		}
		catch (IOException ie)
		{
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new testAll_1());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				testAll_1 simulatedUser = new testAll_1(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// --------------------------------------------------------------------------------
	public static final String REQUEST_FILE_CACHE_FILE_1 = "testAll_1_RequestContent0007.txt";		// must be zipped together with the compiled class of this load test program
	
	
	// source code of inline script "generate clientId"
	public static String getInlineScriptCode_1485506910996()
	{
		return "\r\n\r\nclientID = generateGUID(\"clientId-xxxxxxxxxx\")";
	}
	
	
	/**
	* Execute the inline script "generate clientId"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1485506910996(HttpTestURL testURL, boolean urlCallPassed, int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("generate clientId", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_URL_END, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler, testURL, threadStep);
		inlineScriptContext.setUrlCallPassed(urlCallPassed);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1485506910996(), inlineScriptContext);
