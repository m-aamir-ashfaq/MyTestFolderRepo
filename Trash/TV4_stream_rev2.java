...
public class TV4_stream_rev2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private String manifestUrl = null;                   // var declaration from web admin var handler: scope = per loop
	...
	private String desiredquality = null;                // var declaration from web admin var handler: scope = per loop
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log("<<< desiredquality = " + desiredquality);
		log("<<< chunks = " + chunks);
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// execute inline script "GetMenifestURL"
		executeInlineScript_1453736379573(testURL, urlCallPassed, threadStep - 1, totalLoopCounter, innerLoopContext);
	...
	boolean executePage_3(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// execute inline script "Getdesiredquality"
		executeInlineScript_1453736541277(testURL, urlCallPassed, threadStep - 1, totalLoopCounter, innerLoopContext);
	...
	// source code of inline script "Getdesiredquality"
	public static String getInlineScriptCode_1453736541277()
	{
		return "wholeresponse =  getHTTPResponseContent()\r\n//trace on\r\nqualityurls = strMatchRegExList(wholeresponse,\"http://(.)*.m3u8\")\r\n\r\ndesiredquality = qualityurls(3)\r\nprint desiredquality\r\ndesiredquality = strReplace(desiredquality,\"http://tv4play3-vh.akamaihd.net\",\"\" )\r\nprint desiredquality";
	}
	
	// source code of inline script "GetMenifestURL"
	public static String getInlineScriptCode_1453736379573()
	{
	...
	/**
	* Execute the inline script "Getdesiredquality"
	* 
	...
	*/
	public void executeInlineScript_1453736541277(HttpTestURL testURL, boolean urlCallPassed, int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("Getdesiredquality", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_URL_END, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler, testURL, threadStep);
		inlineScriptContext.setUrlCallPassed(urlCallPassed);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1453736541277(), inlineScriptContext);
		log("executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("desiredquality", Lib.nullToBlank(desiredquality), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("desiredquality", Lib.nullToBlank(desiredquality), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar1);
		inlineScriptExecutor.execute();		// execute inline script
		desiredquality = inlineScriptContext.getOutputVar("desiredquality").getVarValue();
		log("<<< desiredquality = " + desiredquality);
	...
	/**
	* Execute the inline script "GetMenifestURL"
	* 
	* @param testURL 	the context of the executed URL call
	* @param urlCallPassed 	a flag that signals (when set to false) if a 'yellow' error did occur after the URL call (triggered by response verification settings)
	* @param threadStep 	the current execution step of the simulated user (page break or URL call)
	...
	* @param innerLoopContext 	the context of the current inner loop, or null if the inline script in not executed inside an inner loop
	*/
	public void executeInlineScript_1453736379573(HttpTestURL testURL, boolean urlCallPassed, int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("GetMenifestURL", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_URL_END, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler, testURL, threadStep);
		inlineScriptContext.setUrlCallPassed(urlCallPassed);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1453736379573(), inlineScriptContext);
		log("executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("manifestUrl", Lib.nullToBlank(manifestUrl), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("manifestUrl", Lib.nullToBlank(manifestUrl), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar1);
		inlineScriptExecutor.execute();		// execute inline script
		manifestUrl = inlineScriptContext.getOutputVar("manifestUrl").getVarValue();
		log("<<< manifestUrl = " + manifestUrl);
		for (String stdoutLine : inlineScriptContext.getOutputStreamData())
			log(inlineScriptContext.getScriptTitle() + ": " + stdoutLine);
		for (String stderrLine : inlineScriptContext.getErrorStreamData())
