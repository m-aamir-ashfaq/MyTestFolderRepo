...
// -----------------------------------------
//
// Source: TV4_stream_rev27jan1133.java
// Date  : 02 Feb 2016 11:23:43 ECT
 ...
 * Automatically generated load test program.
 */
public class TV4_stream_rev27jan1133 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "TV4_stream_rev27jan1133ExtParams.dat";// input file name for very large parameter values
	...
	private String url = null;                           // var declaration from web admin var handler: scope = per loop
	

	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public TV4_stream_rev27jan1133()
	{
		super();
	}
	 ...
	 */
	public TV4_stream_rev27jan1133(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// execute inline script "abdi1"
		inlineScriptContext = executeInlineScript_1453881393960(testURL, urlCallPassed, threadStep - 1, totalLoopCounter, innerLoopContext);
	...
	boolean executePage_4(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// execute inline script "beforeurl8Inline"
		inlineScriptContext = executeInlineScript_1453890807205(threadStep, totalLoopCounter, innerLoopContext);
		if (inlineScriptContext.isScriptAbort())
		{
			log("*** INLINE SCRIPT \"" + inlineScriptContext.getScriptTitle() + "\" ABORTED / CURRENT LOOP ABORTED ***");
			log("Abort Message = " + inlineScriptContext.getScriptAbortMessage());
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("TV4_stream_rev27jan1133", concurrentUsers);
		...
		performanceData.setInfoText("TV4_stream_rev27jan1133");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				TV4_stream_rev27jan1133 simulatedUser = new TV4_stream_rev27jan1133(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "abdi1"
	public static String getInlineScriptCode_1453881393960()
	{
		return "print \"this is abdi1 script line 1\"\r\nif (isUrlCallPassed()) then\r\nresponse = getHTTPResponseContent()\r\nurl =  jsonGetData(response,\"playback.items.item.1.url\")\r\nprint url\r\nwriteToFile(\"nasir1.txt\",url)\r\nendif";
	}
	
	// source code of inline script "beforeurl8Inline"
	public static String getInlineScriptCode_1453890807205()
	{
		return "print url";
	}
	...
	/**
	* Execute the inline script "abdi1"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1453881393960(HttpTestURL testURL, boolean urlCallPassed, int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("abdi1", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_URL_END, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler, testURL, threadStep);
		inlineScriptContext.setUrlCallPassed(urlCallPassed);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1453881393960(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("url", Lib.nullToBlank(url), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("url", Lib.nullToBlank(url), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar1);
		inlineScriptExecutor.execute();		// execute inline script
		url = inlineScriptContext.getOutputVar("url").getVarValue();
		log("<<< url = " + url);
	...
	/**
	* Execute the inline script "beforeurl8Inline"
	* 
	* @param threadStep 	the current execution step of the simulated user (page break or URL call)
	* @param totalLoopCounter 	the total number of executed loops - counted overall simulated users
	* @param innerLoopContext 	the context of the current inner loop, or null if the inline script in not executed inside an inner loop
	...
	* @return the context of the inline script
	*/
	public LoadtestInlineScriptContext executeInlineScript_1453890807205(int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("beforeurl8Inline", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_LOOP_ITEM_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler);
		inlineScriptContext.setThreadStep(threadStep);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1453890807205(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("url", Lib.nullToBlank(url), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		inlineScriptExecutor.execute();		// execute inline script
