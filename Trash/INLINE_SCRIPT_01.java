...
// -----------------------------------------
//
// Source: INLINE_SCRIPT_01.java
// Date  : 14 Mar 2016 15:53:15 ECT
 ...
 * Automatically generated load test program.
 */
public class INLINE_SCRIPT_01 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "INLINE_SCRIPT_01ExtParams.dat";      // input file name for very large parameter values
	...
	private String latest = null;                        // var declaration from web admin var handler: scope = per loop
	 ...
	 */
	public INLINE_SCRIPT_01(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
		...
		transactionHandler.addTransaction(new UserTransactionContext(1457966371608L, "kkj"));
		transactionHandler.addTransaction(new UserTransactionContext(1457966411932L, "ttj"));
		...
		// execute inline script "justprintGV1" at start of user
		LoadtestInlineScriptContext inlineScriptContext_1457964798645 = new LoadtestInlineScriptContext("justprintGV1", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_USER_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber);
		InlineScriptExecutor inlineScriptExecutor_1457964798645 = new InlineScriptExecutor(getInlineScriptCode_1457964798645(), inlineScriptContext_1457964798645);
		Throwable inlineScriptThrowable_1457964798645 = null;
		log("Executing inline script \"" + inlineScriptContext_1457964798645.getScriptTitle() + "\"");
		try
		{
			...
			inlineScriptContext_1457964798645.addInputVar(inputVar1);
			inlineScriptExecutor_1457964798645.execute();		// execute inline script
		}
		catch (Throwable th_1457964798645)
		{
			inlineScriptThrowable_1457964798645 = th_1457964798645;
		}
		for (String stdoutLine : inlineScriptContext_1457964798645.getOutputStreamData())
			log(inlineScriptContext_1457964798645.getScriptTitle() + ": " + stdoutLine);
		for (String stderrLine : inlineScriptContext_1457964798645.getErrorStreamData())
			System.err.println(inlineScriptContext_1457964798645.getScriptTitle() + ": " + stderrLine);
		if ((!inlineScriptExecutor_1457964798645.wasSuccessFulExecution()) || (inlineScriptThrowable_1457964798645 != null))
		{
			if (inlineScriptContext_1457964798645.isScriptAbort())
			{
				terminateUser(inlineScriptContext_1457964798645);
				return;
			}
			else
			{
				log("*** ERROR: Execution of inline script \"" + inlineScriptContext_1457964798645.getScriptTitle() + "\" failed ***");
				synchronized (dumpLock)
				{
					dumpLog(System.out);
				}
				if (inlineScriptThrowable_1457964798645 != null)
					inlineScriptThrowable_1457964798645.printStackTrace(System.err);
				System.exit(-2);
			}
		}
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		testURL.addUserTransactionContextStart(transactionHandler.getTransaction(1457966371608L), threadStep);
		...
		testURL.setDNSContext(userDNSContext);
		testURL.addUserTransactionContextStart(transactionHandler.getTransaction(1457966411932L), threadStep);
		testURL.addUserTransactionContextEnd(transactionHandler.getTransaction(1457966371608L), threadStep);
		testURL.setFailureActionType(HttpTestURL.FAILURE_ACTION_IGNORE_ERROR);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		...
		// setup var extractor for 'latest'
		TextLineTokenExtractorItem lineTokenExtractorItem0006_1 = new TextLineTokenExtractorItem(TextLineTokenExtractorItem.LINE_CONTAINS_PATTERN, "window", 0, "< >'=&\"(", 1);
		textLineTokenExtractor.addTokenExtractorItem(lineTokenExtractorItem0006_1);

		textLineTokenExtractor.extractTokens();
		try
		{
			// extract var 'latest'
			latest = lineTokenExtractorItem0006_1.getResultToken();
			log("<<< latest = " + latest);
		}
		catch (Exception e) { latest = null; log(e); }
		if (latest == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'latest' from text line pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		// update all user-defined transactions which are ending at this url
		performanceData.addUserTransactionMeasuredSample(transactionHandler.getTransaction(1457966371608L));
	...
	boolean executePage_1_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		testURL.setDNSContext(userDNSContext);
		testURL.addUserTransactionContextEnd(transactionHandler.getTransaction(1457966411932L), threadStep);
		testURL.setFailureActionType(HttpTestURL.FAILURE_ACTION_IGNORE_ERROR);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		...
		// update all user-defined transactions which are ending at this url
		performanceData.addUserTransactionMeasuredSample(transactionHandler.getTransaction(1457966411932L));
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("INLINE_SCRIPT_01", concurrentUsers);
		...
		performanceData.setInfoText("INLINE_SCRIPT_01");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				INLINE_SCRIPT_01 simulatedUser = new INLINE_SCRIPT_01(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "justprintGV1"
	public static String getInlineScriptCode_1457964798645()
	{
		return "print globalVar1";
	}
