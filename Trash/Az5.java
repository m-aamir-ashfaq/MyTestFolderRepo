 ...
 * Automatically generated load test program.
 */
public class Az5 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "Az5ExtParams.dat";                   // input file name for very large parameter values
	...
	private static VarInputFileReader userDetails_txt = null;        // input file userDetails.txt, file scope: new line per user
	...
	private String lname = null;                                          // var declaration from web admin var handler: scope = per user
	private String email = null;                                          // var declaration from web admin var handler: scope = per user
	private String pass = null;                                           // var declaration from web admin var handler: scope = per user
	private String fname = null;                                          // var declaration from web admin var handler: scope = per user

	private String newVar = null;                        // var declaration from web admin var handler: scope = per loop
	 ...
	 */
	public Az5(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
		...
		try
		{
			...
			{
				if (!userDetails_txt.getNextLine())
					abortByEofInputFile();
				// extract vars from input file: scope = per user
				email = new String(userDetails_txt.getLineVar(1));
				fname = new String(userDetails_txt.getLineVar(3));
				lname = new String(userDetails_txt.getLineVar(4));
				pass = new String(userDetails_txt.getLineVar(2));
			}
		}
		catch (Exception ie)
		{
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [1] <- WEB ADMIN Index 6 ---
		...
		// setup var extractor for 'newVar'
		BoundaryBasedExtractorItem boundaryBasedContentExtractorItem0001_1 = new BoundaryBasedExtractorItem( "Scripts/bundles/Triona-ALLBundle.js?id=" , "\"></script>",1,false,false,0,false);
		...
		{
			// extract var 'newVar'
			newVar = boundaryBasedContentExtractorItem0001_1.getResultText();
			log("<<< newVar = " + newVar);
		}
		catch (Exception e) { newVar = null; }		// failover error handling for this var is enabled - ignore any var assignment error
		if (newVar == null)
		{
			newVar = "subsValueLetsSee";
			log("runtime note: the extraction of a new value for the variable 'newVar' has failed - a substitute value was set");
			log("<<< newVar = " + newVar);
		}
		...
		// execute inline script "letsSee"
		inlineScriptContext = executeInlineScript_1487589834391(testURL, urlCallPassed, threadStep - 1, totalLoopCounter, innerLoopContext);
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 281, 5);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0003 = "/api/configuration/site/getconfig";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 1363, 5);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0004 = "/api/authorization/account/isauthenticated";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 228, 5);		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0005 = "/api/events/channel/getchannels";
		...
		String requestFile0006 = "/api/events/event/getplayingevent";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 648, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0007 = "/api/events/event/getaggregatedevents";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 6611, 5);		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0008 = "/api/events/event/getcategorizedevents";
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "A bunch of bug fixes & several minor improvements");		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0009 = "/api/events/event/CheckCanPlayEvent/e5cd29ca906b4be48fcddfd11b45561a" +
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 351, 5);		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0010 = "mediasvcsk8gh98mxvd2r.blob.core.windows.net";
		int    requestPort0010 = 443;
		String requestFile0010 = "/asset-9ed4f95e-8534-481c-9306-b5484dac50b5/Mix_Tape-_Shane_Warne,_Volume_1_1.jpg" +
				"?sv=2012-02-12" +
				"&sr=c" +
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {403}, null, null, null);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0011 = "cdn.sharepointonline.com";
		int    requestPort0011 = 80;
		String requestFile0011 = "/15772/_layouts/15/fonts/Office365Icons.woff";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/FONT-WOFF", null, 47204, 5);		// Test [11] <- Index 11
		if (!urlCallPassed)
		{
		...
		String requestHost0012 = "kumo2stage.blob.core.windows.net";
		int    requestPort0012 = 443;
		String requestFile0012 = "/asset-7cd70796-e388-459d-b289-d669e3b718b5/f1fea1185cc04dabcf83e9662204c3e5_000001.jpg" +
				"?sv=2012-02-12" +
				"&sr=c" +
		...
		// verify response: status code = 403, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 404
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {403}, null, null, null);		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [13] <- WEB ADMIN Index 19 ---
		String requestProt0013 = "https";
		String requestHost0013 = "player.mpp5.devmpp.com";
		int    requestPort0013 = 443;
		String requestFile0013 = "/play/channel/6559faa5cbee47149bbb751afd3cbd13/e5cd29ca906b4be48fcddfd11b45561a" +
				"?autoplay=true";
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div class=\"countdownColon3 countdownColons\"");		// Test [13] <- Index 13
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0014 = "/Account/create/new_account";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 1961
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "var redirectLocation = window.location.origin;");		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0015 = "/api/configuration/site/gethostagent";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 281
		// content test algorithm: check size +/- 5%, defined content size = 281
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 281, 5);		// Test [15] <- Index 15
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0016 = "/api/configuration/site/getconfig";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 1363
		// content test algorithm: check size +/- 5%, defined content size = 1363
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 1363, 5);		// Test [16] <- Index 16
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [17] <- WEB ADMIN Index 29 ---
		String requestProt0017 = "http";
		String requestHost0017 = "dev.dev.playsite.devmpp.com";
		int    requestPort0017 = 80;
		String requestFile0017 = "/api/authorization/account/isauthenticated";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 228
		// content test algorithm: check size +/- 5%, defined content size = 228
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 228, 5);		// Test [17] <- Index 17
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [18] <- WEB ADMIN Index 30 ---
		String requestProt0018 = "http";
		String requestHost0018 = "dev.dev.playsite.devmpp.com";
		int    requestPort0018 = 80;
		String requestFile0018 = "/api/authorization/account/getaccountoverview";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 213, 5);		// Test [18] <- Index 18
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [19] <- WEB ADMIN Index 34 ---
		String requestProt0019 = "http";
		String requestHost0019 = "dev.dev.playsite.devmpp.com";
		int    requestPort0019 = 80;
		String requestFile0019 = "/api/authorization/account/register";
		String requestContent0019 = 
				"{\"email\":\"amir1106@gmail.com\",\"password\":\"paistanLAHORE\",\"firstName\":\"Amir\",\"lastName\":\"Khan\"}";
		requestContent0019 = Lib.replaceTextPattern(requestContent0019, "Khan", lname, 0);		// search and replace content fragment "Khan" with variable 'lname' on all occurrences
		requestContent0019 = Lib.replaceTextPattern(requestContent0019, "amir1106@gmail.com", email, 0);		// search and replace content fragment "amir1106@gmail.com" with variable 'email' on all occurrences
		requestContent0019 = Lib.replaceTextPattern(requestContent0019, "paistanLAHORE", pass, 0);		// search and replace content fragment "paistanLAHORE" with variable 'pass' on all occurrences
		requestContent0019 = Lib.replaceTextPattern(requestContent0019, "Amir", fname, 0);		// search and replace content fragment "Amir" with variable 'fname' on all occurrences

		byte[] requestContentBinary0019 = requestContent0019.getBytes("UTF-8");
		String requestHeader0019 = "POST " + requestFile0019 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Content-Length: " + requestContentBinary0019.length + "\r\n" + 		// Content-Length: 94
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0019, requestHost0019, requestPort0019, requestHeader0019, requestContentBinary0019, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		requestContentBinary0019 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 371, 5);		// Test [19] <- Index 19
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 281
		// content test algorithm: check size +/- 5%, defined content size = 281
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 281, 5);		// Test [20] <- Index 20
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	boolean executePage_1_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHeader0021 = "POST " + requestFile0021 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Content-Length: " + requestContentBinary0021.length + "\r\n" + 		// Content-Length: 92
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0021, requestHost0021, requestPort0021, requestHeader0021, requestContentBinary0021, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		requestContentBinary0021 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 353, 5);		// Test [21] <- Index 21
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [22] <- WEB ADMIN Index 38 ---
		String requestProt0022 = "http";
		String requestHost0022 = "dev.dev.playsite.devmpp.com";
		int    requestPort0022 = 80;
		String requestFile0022 = "/api/authorization/account/getaccountoverview";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 6358, 5);		// Test [22] <- Index 22
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 1961
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "var redirectLocation = window.location.origin;");		// Test [23] <- Index 23
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [24] <- WEB ADMIN Index 41 ---
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 1363
		// content test algorithm: check size +/- 5%, defined content size = 1363
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 1363, 5);		// Test [25] <- Index 25
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [26] <- WEB ADMIN Index 43 ---
		String requestProt0026 = "http";
		String requestHost0026 = "dev.dev.playsite.devmpp.com";
		int    requestPort0026 = 80;
		String requestFile0026 = "/api/authorization/account/isauthenticated";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 228
		// content test algorithm: check size +/- 5%, defined content size = 228
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 228, 5);		// Test [26] <- Index 26
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [27] <- WEB ADMIN Index 44 ---
		String requestProt0027 = "http";
		String requestHost0027 = "dev.dev.playsite.devmpp.com";
		int    requestPort0027 = 80;
		String requestFile0027 = "/api/authorization/account/getaccountoverview";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 6358
		// content test algorithm: check size +/- 5%, defined content size = 6358
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 6358, 5);		// Test [27] <- Index 27
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 8327, 5);		// Test [28] <- Index 28
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 648
		// content test algorithm: check size +/- 5%, defined content size = 648
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 648, 5);		// Test [29] <- Index 29
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 6611
		// content test algorithm: check size +/- 5%, defined content size = 6611
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 6611, 5);		// Test [30] <- Index 30
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [31] <- WEB ADMIN Index 48 ---
		String requestProt0031 = "http";
		String requestHost0031 = "dev.dev.playsite.devmpp.com";
		int    requestPort0031 = 80;
		String requestFile0031 = "/api/events/event/getcategorizedevents";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 12814
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "A bunch of bug fixes & several minor improvements");		// Test [31] <- Index 31
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [32] <- WEB ADMIN Index 49 ---
		String requestProt0032 = "https";
		String requestHost0032 = "mediasvcsk8gh98mxvd2r.blob.core.windows.net";
		int    requestPort0032 = 443;
		String requestFile0032 = "/asset-9ed4f95e-8534-481c-9306-b5484dac50b5/Mix_Tape-_Shane_Warne,_Volume_1_1.jpg" +
				"?sv=2012-02-12" +
				"&sr=c" +
				"&si=abdcfcd6-5c33-40b1-b368-2783be8cc318" +
				"&sig=859uj99l5fZIMTFO2atvPbTV41jYQERYriGxyCTJpHY%3D" +
		...
		// verify response: status code = 403, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 404
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {403}, null, null, null);		// Test [32] <- Index 32
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [33] <- WEB ADMIN Index 50 ---
		String requestProt0033 = "https";
		String requestHost0033 = "kumo2stage.blob.core.windows.net";
		int    requestPort0033 = 443;
		String requestFile0033 = "/asset-7cd70796-e388-459d-b289-d669e3b718b5/f1fea1185cc04dabcf83e9662204c3e5_000001.jpg" +
				"?sv=2012-02-12" +
				"&sr=c" +
				"&si=14f48bf5-09d3-45be-aeaf-f3f790c89492" +
				"&sig=h5oFcKhBUr%2FHERlr0%2BkLxQ%2BIy0IVP85%2FeyESwD2woEE%3D" +
		...
		// verify response: status code = 403, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 404
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {403}, null, null, null);		// Test [33] <- Index 33
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [34] <- WEB ADMIN Index 51 ---
		String requestProt0034 = "http";
		String requestHost0034 = "dev.dev.playsite.devmpp.com";
		int    requestPort0034 = 80;
		String requestFile0034 = "/api/events/event/CheckCanPlayEvent/e5cd29ca906b4be48fcddfd11b45561a" +
				"?eventTypeString=Channel" +
				"&eventHolderId=6559faa5cbee47149bbb751afd3cbd13" +
				"&withAutoStart=true";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 679, 5);		// Test [34] <- Index 34
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [35] <- WEB ADMIN Index 52 ---
		String requestProt0035 = "http";
		String requestHost0035 = "cdn.sharepointonline.com";
		int    requestPort0035 = 80;
		String requestFile0035 = "/15772/_layouts/15/fonts/Office365Icons.woff";
		...
		// verify response: status code = 200, content type = "APPLICATION/FONT-WOFF", header text fragment = [verification disabled], recorded content size = 47204
		// content test algorithm: check size +/- 5%, defined content size = 47204
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/FONT-WOFF", null, 47204, 5);		// Test [35] <- Index 35
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [36] <- WEB ADMIN Index 53 ---
		String requestProt0036 = "https";
		String requestHost0036 = "player.mpp5.devmpp.com";
		int    requestPort0036 = 443;
		String requestFile0036 = "/play/channel/6559faa5cbee47149bbb751afd3cbd13/e5cd29ca906b4be48fcddfd11b45561a" +
				"?autoplay=true" +
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div class=\"countdownColon3 countdownColons\"");		// Test [36] <- Index 36
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("Az5", concurrentUsers);
		...
		final int MAX_THREAD_STEPS = 36;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("Az5");
		...
		try
		{
			userDetails_txt = new VarInputFileReader(getSymmetricEncryptContext(), "userDetails.txt", "#", ",", AbstractInputFileReader.EOF_REOPEN_FILE, prxCharEncoding);
			userDetails_txt.setTrimValues(true);
		}
		catch (IOException ie)
		{
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new Az5());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				Az5 simulatedUser = new Az5(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "letsSee"
	public static String getInlineScriptCode_1487589834391()
	{
		return "ON ERROR GOTO \"myErrorHandler\"\r\nprint \" some line of code \"\r\nprint unDeclearedVariable //this line will cause error\r\n\r\nHandler \"myErrorHandler\"\r\nprint \" error handling code \"\r\nEnd Handler\r\n";
	}
	
	
	/**
	* Execute the inline script "letsSee"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1487589834391(HttpTestURL testURL, boolean urlCallPassed, int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("letsSee", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_URL_END, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler, testURL, threadStep);
		inlineScriptContext.setUrlCallPassed(urlCallPassed);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1487589834391(), inlineScriptContext);
