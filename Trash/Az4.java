...
public class Az4 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [1] <- WEB ADMIN Index 16 ---
		String requestProt0001 = "http";
		String requestHost0001 = "dev.dev.playsite.devmpp.com";
		int    requestPort0001 = 80;
		String requestFile0001 = "/api/authorization/account/register";
		String requestContent0001 = 
				"{\"email\":\"amir1106@gmail.com\",\"password\":\"paistanLAHORE\",\"firstName\":\"Amir\",\"lastName\":\"Khan\"}";
		requestContent0001 = Lib.replaceTextPattern(requestContent0001, "Khan", lname, 0);		// search and replace content fragment "Khan" with variable 'lname' on all occurrences
		requestContent0001 = Lib.replaceTextPattern(requestContent0001, "amir1106@gmail.com", email, 0);		// search and replace content fragment "amir1106@gmail.com" with variable 'email' on all occurrences
		requestContent0001 = Lib.replaceTextPattern(requestContent0001, "paistanLAHORE", pass, 0);		// search and replace content fragment "paistanLAHORE" with variable 'pass' on all occurrences
		requestContent0001 = Lib.replaceTextPattern(requestContent0001, "Amir", fname, 0);		// search and replace content fragment "Amir" with variable 'fname' on all occurrences
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 371, 5);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		// --- HTTP REQUEST: Test [2] <- WEB ADMIN Index 18 ---
		...
		// --- HTTP REQUEST: Test [3] <- WEB ADMIN Index 19 ---
		String requestProt0003 = "http";
		String requestHost0003 = "dev.dev.playsite.devmpp.com";
		int    requestPort0003 = 80;
		String requestFile0003 = "/api/authorization/account/login";
		String requestContent0003 = 
				"{\"username\":\"amir1106@gmail.com\",\"email\":\"\",\"password\":\"paistanLAHORE\",\"confirmpassword\":\"\"}";
		requestContent0003 = Lib.replaceTextPattern(requestContent0003, "amir1106@gmail.com", email, 0);		// search and replace content fragment "amir1106@gmail.com" with variable 'email' on all occurrences
		requestContent0003 = Lib.replaceTextPattern(requestContent0003, "paistanLAHORE", pass, 0);		// search and replace content fragment "paistanLAHORE" with variable 'pass' on all occurrences

		byte[] requestContentBinary0003 = requestContent0003.getBytes("UTF-8");
		String requestHeader0003 = "POST " + requestFile0003 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Content-Length: " + requestContentBinary0003.length + "\r\n" + 		// Content-Length: 92
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0003, requestHost0003, requestPort0003, requestHeader0003, requestContentBinary0003, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		requestContentBinary0003 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 353, 5);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [4] <- WEB ADMIN Index 20 ---
		String requestProt0004 = "http";
		String requestHost0004 = "dev.dev.playsite.devmpp.com";
		int    requestPort0004 = 80;
		String requestFile0004 = "/api/authorization/account/getaccountoverview";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 6358, 5);		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [5] <- WEB ADMIN Index 22 ---
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "var redirectLocation = window.location.origin;");		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 281
		// content test algorithm: check size +/- 5%, defined content size = 281
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 281, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 1363, 5);		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [8] <- WEB ADMIN Index 25 ---
		String requestProt0008 = "http";
		String requestHost0008 = "dev.dev.playsite.devmpp.com";
		int    requestPort0008 = 80;
		String requestFile0008 = "/api/authorization/account/isauthenticated";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 228, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [9] <- WEB ADMIN Index 26 ---
		String requestProt0009 = "http";
		String requestHost0009 = "dev.dev.playsite.devmpp.com";
		int    requestPort0009 = 80;
		String requestFile0009 = "/api/authorization/account/getaccountoverview";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 6358
		// content test algorithm: check size +/- 5%, defined content size = 6358
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 6358, 5);		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [10] <- WEB ADMIN Index 27 ---
		...
		// --- HTTP REQUEST: Test [11] <- WEB ADMIN Index 28 ---
		...
		// --- HTTP REQUEST: Test [12] <- WEB ADMIN Index 29 ---
		...
		// --- HTTP REQUEST: Test [13] <- WEB ADMIN Index 30 ---
		...
		// --- HTTP REQUEST: Test [14] <- WEB ADMIN Index 33 ---
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 679, 5);		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [15] <- WEB ADMIN Index 35 ---
		String requestProt0015 = "https";
		String requestHost0015 = "player.mpp5.devmpp.com";
		int    requestPort0015 = 443;
		String requestFile0015 = "/play/channel/6559faa5cbee47149bbb751afd3cbd13/e5cd29ca906b4be48fcddfd11b45561a" +
				"?autoplay=true" +
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div class=\"countdownColon3 countdownColons\"");		// Test [15] <- Index 15
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
