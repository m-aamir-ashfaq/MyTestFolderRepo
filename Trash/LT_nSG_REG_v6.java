...
// -----------------------------------------
//
// Source: LT_nSG_REG_v6.java
// Date  : 24 Feb 2016 16:49:23 ECT
// Author: Apica ZebraTester V5.4-H / automatically generated
//
 ...
 * Automatically generated load test program.
 */
public class LT_nSG_REG_v6 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	public static final String prxVersion = "V5.4-H";
	public static final int    prxCharEncoding = 2;                         // 1 = OS Default, 2 = ISO-8859-1, 3 = UTF-8
	public static final String testDescription = "";

	public static String USER_AGENT_1 = "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:38.0) Gecko/20100101 Firefox/38.0";
	...
	private static final String EXTERNAL_PARAM_FILE = "LT_nSG_REG_v6ExtParams.dat";         // input file name for very large parameter values
	...
	private static VarInputFileReader usernames_txt = null;          // input file usernames.txt, file scope: new line per loop
	...
	private String usernamesfull = null;                 // var declaration from web admin var handler: scope = per loop
	private String usernamecut = null;                   // var declaration from web admin var handler: scope = per loop
	private String usernameLEN = null;                   // var declaration from web admin var handler: scope = per loop
	
	private String innerloopcounter = null;              // var declaration from web admin var handler: scope = per inner loop
	

	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public LT_nSG_REG_v6()
	{
		super();
	}
	 ...
	 */
	public LT_nSG_REG_v6(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		usernameLEN = "";
		log();
		log("<<< usernamesfull = " + usernamesfull);
		log("<<< usernamecut = " + usernamecut);
		log("<<< usernameLEN = " + usernameLEN);
		...
		try
		{
			...
			{
				if (!usernames_txt.getNextLine())
				{
					abortByEofInputFile();
					return true;		// eof of input file - no error from web server
				}
				usernamesfull = new String(usernames_txt.getLineVar(1));
			}
			log();
			log("<<< usernamesfull = " + usernamesfull);
		}
		...
		// execute inline script "countCHARS" at start of loop
		LoadtestInlineScriptContext inlineScriptContext_1456322686050 = new LoadtestInlineScriptContext("countCHARS", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_LOOP_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler);
		InlineScriptExecutor inlineScriptExecutor_1456322686050 = new InlineScriptExecutor(getInlineScriptCode_1456322686050(), inlineScriptContext_1456322686050);
		log("Executing inline script \"" + inlineScriptContext_1456322686050.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1_1456322686050 = new LoadtestInlineScriptVar("usernameLEN", Lib.nullToBlank(usernameLEN), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext_1456322686050.addInputVar(inputVar1_1456322686050);
		LoadtestInlineScriptVar inputVar2_1456322686050 = new LoadtestInlineScriptVar("usernamesfull", Lib.nullToBlank(usernamesfull), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext_1456322686050.addInputVar(inputVar2_1456322686050);
		LoadtestInlineScriptVar outputVar1_1456322686050 = new LoadtestInlineScriptVar("usernameLEN", Lib.nullToBlank(usernameLEN), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext_1456322686050.addOutputVar(outputVar1_1456322686050);
		inlineScriptExecutor_1456322686050.execute();		// execute inline script
		usernameLEN = inlineScriptContext_1456322686050.getOutputVar("usernameLEN").getVarValue();
		log("<<< usernameLEN = " + usernameLEN);
		for (String stdoutLine : inlineScriptContext_1456322686050.getOutputStreamData())
			log(inlineScriptContext_1456322686050.getScriptTitle() + ": " + stdoutLine);
		for (String stderrLine : inlineScriptContext_1456322686050.getErrorStreamData())
			System.err.println(inlineScriptContext_1456322686050.getScriptTitle() + ": " + stderrLine);
		if (!inlineScriptExecutor_1456322686050.wasSuccessFulExecution())
		{
			if (inlineScriptContext_1456322686050.isScriptAbort())
			{
				log("*** INLINE SCRIPT \"" + inlineScriptContext_1456322686050.getScriptTitle() + "\" ABORTED / START OF CURRENT LOOP ABORTED ***");
				log("Abort Message = " + inlineScriptContext_1456322686050.getScriptAbortMessage());
				return false;		// the session failure rate is also increased
			}
			else
				throw new InlineScriptExecutionException("*** Execution of inline script \"" + inlineScriptContext_1456322686050.getScriptTitle() + "\" failed ***");
		}
		...
		log("# Page #1: Register");
		log("# -----------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #1: Register", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		...
		// start of inner loop: "usernamecheckloop"
		// ---------------------------------------
		...
		for (innerLoopCounter1 = 0; innerLoopCounter1 < Integer.valueOf(usernameLEN).intValue(); innerLoopCounter1++)
		{
			threadStep = innerLoopStartThreadStep1;		// reset thread step counter for this loop
			log("   - inner loop \"usernamecheckloop\" iteration[" + (innerLoopCounter1 + 1) + "] started");

			// customised vars from web admin var handler: scope = per inner loop
			innerloopcounter = "" + innerLoopCounter1;

			log();
			log("<<< innerloopcounter = " + innerloopcounter);
			log();
			...
			log();
			log();
			log("# Page #2: innerloop1");
			log("# -------------------");
			threadStep = setPageBreak(performanceData, threadStep, "Page #2: innerloop1", 0, 0, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
			pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
			log();
		...
		pageThreadHandler.addPageResponseTimeToResult(this);
			...
			log("   - inner loop \"usernamecheckloop\" iteration[" + (innerLoopCounter1 + 1) + "] done");
			...
			if ((plannedTestDuration > 0) && (((System.currentTimeMillis() - testDurationStart) / 1000) >= plannedTestDuration))
				break;
		}		// end of inner loop: "usernamecheckloop"
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// execute inline script "partialstringblabla"
		inlineScriptContext = executeInlineScript_1456323300371(threadStep, totalLoopCounter, innerLoopContext);
		...
		String requestHost0002 = "beta.stargames.com";
		int    requestPort0002 = 443;
		String requestFile0002 = "/de/register/CheckNickName" +
				"?NickName=" + usernamecut;		// recorded value = apicatestreg
		String requestHeader0002 = "GET " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: beta.stargames.com\r\n" + 
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 6
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("LT_nSG_REG_v6", concurrentUsers);
		...
		try
		{
			usernames_txt = new VarInputFileReader("usernames.txt", "#", ";", AbstractInputFileReader.EOF_REOPEN_FILE, prxCharEncoding);
			usernames_txt.setTrimValues(true);
		}
		catch (IOException ie)
		{
		...
		System.out.println("# input file = usernames.txt, file scope = new line per loop");
		...
		performanceData.setInfoText("LT_nSG_REG_v6");
		performanceData.setExecutorsAnnotation(testRunAnnotation);
		performanceData.addTestDescription("*** inner loop - page #2 to page #2 (usernamecheckloop) ***");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				LT_nSG_REG_v6 simulatedUser = new LT_nSG_REG_v6(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "countCHARS"
	public static String getInlineScriptCode_1456322686050()
	{
		return "usernameLEN = strLen(usernamesfull)";
	}
	
	// source code of inline script "partialstringblabla"
	public static String getInlineScriptCode_1456323300371()
	{
		return "aaaa = innerloopcounter + 2\r\n\r\nif(aaaa <= strLen(usernamesfull))then\r\nusernamecut  = strSubString(usernamesfull  ,1, aaaa )\r\nwriteToFile(\"kkj.txt\", \"___________________________________________________________\" )\r\nwriteToFile(\"kkj.txt\",usernamesfull)\r\nwriteToFile(\"kkj.txt\", aaaa )\r\nwriteToFile(\"kkj.txt\",strSubString(usernamesfull  ,1, aaaa ))\r\nendif";
	}
	
	
	/**
	* Execute the inline script "partialstringblabla"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1456323300371(int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("partialstringblabla", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_LOOP_ITEM_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler);
		inlineScriptContext.setThreadStep(threadStep);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1456323300371(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("innerloopcounter", Lib.nullToBlank(innerloopcounter), 0);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar inputVar2 = new LoadtestInlineScriptVar("usernamecut", Lib.nullToBlank(usernamecut), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar2);
		LoadtestInlineScriptVar inputVar3 = new LoadtestInlineScriptVar("usernameLEN", Lib.nullToBlank(usernameLEN), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar3);
		LoadtestInlineScriptVar inputVar4 = new LoadtestInlineScriptVar("usernamesfull", Lib.nullToBlank(usernamesfull), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar4);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("innerloopcounter", Lib.nullToBlank(innerloopcounter), 0);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar1);
		LoadtestInlineScriptVar outputVar2 = new LoadtestInlineScriptVar("usernamecut", Lib.nullToBlank(usernamecut), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar2);
		LoadtestInlineScriptVar outputVar3 = new LoadtestInlineScriptVar("usernameLEN", Lib.nullToBlank(usernameLEN), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar3);
		LoadtestInlineScriptVar outputVar4 = new LoadtestInlineScriptVar("usernamesfull", Lib.nullToBlank(usernamesfull), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar4);
		inlineScriptExecutor.execute();		// execute inline script
		innerloopcounter = inlineScriptContext.getOutputVar("innerloopcounter").getVarValue();
		log("<<< innerloopcounter = " + innerloopcounter);
		usernamecut = inlineScriptContext.getOutputVar("usernamecut").getVarValue();
		log("<<< usernamecut = " + usernamecut);
		usernameLEN = inlineScriptContext.getOutputVar("usernameLEN").getVarValue();
		log("<<< usernameLEN = " + usernameLEN);
		usernamesfull = inlineScriptContext.getOutputVar("usernamesfull").getVarValue();
		log("<<< usernamesfull = " + usernamesfull);
