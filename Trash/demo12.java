 ...
 * Automatically generated load test program.
 */
public class demo12 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "demo12ExtParams.dat";                // input file name for very large parameter values
	...
	private static String var2 = "Rock done";                                    // var declaration from web admin var handler: scope = global
	 ...
	 */
	public demo12(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// verify response: status code = 101, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {101}, null, null, "Rock","all");		// Test [11] <- Index 11
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("demo12", concurrentUsers);
		...
		performanceData.setInfoText("demo12");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				demo12 simulatedUser = new demo12(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	public static WebSocketData getWebSocketData_1469092471915()
	{
		...
		// search and replace frame payload "done" with variable 'var2' on all occurrences of frame no 4
		payLoad_4 = Lib.replaceTextPattern(payLoad_4, "done", var2, 0);
