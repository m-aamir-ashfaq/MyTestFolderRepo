...
// -----------------------------------------
//
// Source: demoCollectionSeq.java
// Date  : 05 May 2017 14:15:23 ECT
 ...
 * Automatically generated load test program.
 */
public class demoCollectionSeq extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static final String USER_INPUT_FIELD_kkj = "AQADa2tqAQADa2tqAQADa2tq";		// internal object-string representation of user input field 'kkj'
	...
	private static final String EXTERNAL_PARAM_FILE = "demoCollectionSeqExtParams.dat";     // input file name for very large parameter values
	...
	private static String kkj = null;                                            // var declaration from web admin var handler: scope = global
	 ...
	 */
	public demoCollectionSeq(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log("# -------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #1: Start Page", 4000, 0, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// # -------------------
		 ...
		 boolean startLoop14878627170000 = false;
		// check for break of URL loop
		if (kkj != null)
		{
			String checkVarValueText = "kkj";
			ConvertToDoubleNumber testDoubleNumber1 = new ConvertToDoubleNumber(kkj);
			ConvertToDoubleNumber testDoubleNumber2 = new ConvertToDoubleNumber(checkVarValueText);
			if (testDoubleNumber1.isNumber() && testDoubleNumber2.isNumber())
			{
				// perform a number check
				double doubleValue1 = testDoubleNumber1.getValue();
				double doubleValue2 = testDoubleNumber2.getValue();
				if (doubleValue1 == doubleValue2)
				{
					startLoop14878627170000 = true;
				}
			}
			else
			{
				// perform a string check
				int stringCompareResult = kkj.compareTo("kkj");
		...
		// start of URL loop: "loop 1"
		// ----------------------------
		int urlLoopStartThreadStep1 = threadStep;		// save threadStep counter for start of this URL loop
		int urlLoopEndThreadStep1 = threadStep + 1;		// save threadStep counter for end of this URL loop
		int urlLoopCounter1 = -1;
		for (urlLoopCounter1 = 0; urlLoopCounter1 < 20; urlLoopCounter1++)
		{
			if(urlLoopCounter1 == 1 && !startLoop14878627170000)
				break;
			threadStep = urlLoopStartThreadStep1;		// reset thread step counter for this loop
			log("   - URL loop \"loop 1\" iteration[" + (urlLoopCounter1 + 1) + "] started");
		...
		// execute inline script "addCollectionData"
		inlineScriptContext = executeInlineScript_1488195478317(testURL, urlCallPassed, threadStep - 1, totalLoopCounter, innerLoopContext);
		if (inlineScriptContext.isScriptAbort())
		{
			...
			log("   - URL loop \"loop 1\" iteration[" + (urlLoopCounter1 + 1) + "] done");
		}		// end of inner loop: "loop 1"
		threadStep = urlLoopEndThreadStep1;		// adjust threadStep counter after end of this URL loop
	...
	public static void main(String[] args)
	{
		...
		if ((ParseArgs.hasOption(args, "-h")) || (ParseArgs.hasOption(args, "-help")))
		{
			...
			System.out.println();
			System.out.println("-kkj <user-input-field>                  ->> required argument: kkj");
			System.out.println();
		...
		// parse -kkj argument or ask --> <user-input-field>: proposed value = "kkj"
		try
		{
			kkj = UserInputField.readFromObjectString(USER_INPUT_FIELD_kkj).readUserInputValue(args);
			if (kkj.startsWith("\"") && kkj.endsWith("\""))
				kkj = kkj.substring(1, kkj.length() - 1);
		}
		catch (IOException ie)
		{
			System.out.println("*** INTERNAL ERROR: invalid user input field object-string declaration for 'kkj' ***");
			ie.printStackTrace();
			System.exit(-2);
		}
		...
		String genericFileName = PerformanceData.proposeFileName("demoCollectionSeq", concurrentUsers);
		...
		performanceData.setInfoText("demoCollectionSeq");
		...
		performanceData.addUserInputField("kkj", kkj);		// add user input field name and value to performance data
		...
		{
			System.out.println("global var <<< kkj = " + kkj);
			System.out.println();
		}
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new demoCollectionSeq());
		...
		if (hasExecutionPlan())
		{
			...
			try
			{
				...
				demoCollectionSeq simulatedUser = new demoCollectionSeq(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "addCollectionData"
	public static String getInlineScriptCode_1488195478317()
	{
		return "createCollectionSequence(\"someSeq\" , \"mainDiagram\", \"subtitle\" , \"yAxislabel\")\r\n\r\n\r\nsleep(500)\r\naddDataCollectionFloatItem(\"someSeq\" , random(100))\r\nsleep(500)\r\naddDataCollectionFloatItem(\"someSeq\" , random(100))\r\n\r\n\r\n\r\n//createCollectionSequence(\"otherSeq\" , \"otherLetssee\", \"subtitle\" , \"yAxislabel\")\r\n\r\n//sleep(500)\r\n//addDataCollectionFloatItem(\"otherSeq\" , random(100,200))\r\n//sleep(500)\r\n//addDataCollectionFloatItem(\"otherSeq\" , random(100,200))\r\n\r\n\r\n//print getCollectionSequenceList()\r\n\r\n//addClusterOption(\"someSeq\" , 1 , \"\")\r\n\r\n\r\n//print hasClusterOption(\"someSeq\",1)\r\n//print hasClusterOption(\"someSeq\",2)\r\n//print hasClusterOption(\"otherSeq\",1)\r\n\r\n\r\n//sleep(500)\r\n//addDataCollectionFloatItem(\"someSeq\" , random(100))\r\n//sleep(500)\r\n//addDataCollectionFloatItem(\"someSeq\" , random(100))\r\n//sleep(500)\r\n\r\n\r\n//addDataCollectionErrorEventItem(\"otherSeq\" , \"\")\r\n//addDataCollectionYZFloatItem(\"otherSeq\" , 12.78 ,19.99 )\r\n//addDataCollectionTextItem(\"otherSeq\" , \"some text\" )\r\n\r\n//print getCollectionSequenceItemType(\"someSeq\" )\r\n//print getCollectionSequenceItemType(\"otherSeq\" )\r\n\r\n\r\n\r\n\r\n";
	}
	
	
	/**
	* Execute the inline script "addCollectionData"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1488195478317(HttpTestURL testURL, boolean urlCallPassed, int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("addCollectionData", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_URL_END, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler, testURL, threadStep);
		inlineScriptContext.setUrlCallPassed(urlCallPassed);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1488195478317(), inlineScriptContext);
