 ...
 * Automatically generated load test program.
 */
public class latest extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "latestExtParams.dat";                // input file name for very large parameter values
	 ...
	 */
	public latest(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0001 = "/submit/telemetry/50cedaad-6dfe-fc46-a805-3c45cdea5d6d/main/Firefox/52.0/release/20170302120751" +
				"?v=4";
		String requestContent0001 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "latest_RequestContent0001.txt").getFileDataAsString("UTF-8");		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
		...
		String requestHeader0001 = "POST " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Date: Fri, 17 Mar 2017 14:23:29 GMT\r\n" + 
		...
		String requestHeader0002 = "GET " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: 6b0c4a8bn8MPGqlTwfrLUA==\r\n" + 
		...
		webSocketData = getWebSocketData_1489760612373();
		...
		String requestFile0005 = "/ajax/libs/fancybox/2.1.5/fancybox_sprite@2x.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 5901
		// content test algorithm: check size +/- 5%, defined content size = 5901
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 5901, 5);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("latest", concurrentUsers);
		...
		performanceData.setInfoText("latest");
		...
		try
		{
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_1);		// latest_RequestContent0001.txt
		}
		catch (IOException ie)
		{
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new latest());
		...
		if (hasExecutionPlan())
		{
			...
			try
			{
				...
				latest simulatedUser = new latest(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// --------------------------------------------------------------------------------
	public static final String REQUEST_FILE_CACHE_FILE_1 = "latest_RequestContent0001.txt";		// must be zipped together with the compiled class of this load test program
	
	
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1489760612373()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1489760612402L);
		webSocketData.setConnectionStopMillis(1489760638610L);
		webSocketData.setSecProtocol("mqtt");
		String payLoad_0 = "{\"DUP\":false,\"QoS\":0,\"userNameFlag\":0,\"last_will_Flag\":0,\"keep_alive\":60,\"retain\":false,\"client_id\":\"clientId-tDrUjQMYHh\",\"clean_session\":true,\"type\":\"CONNECT\",\"passwordFlag\":0,\"protocol_name\":\"MQIsdp\",\"protocol_version\":3}";
		String payLoad_2 = "{\"subscribe_topics\":[{\"subscribe_topic\":\"testtopic/Apica\",\"subscribe_QoS\":2}],\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"SUBSCRIBE\"}";
		String payLoad_4 = "{\"DUP\":false,\"QoS\":0,\"contents\":\"q\",\"publish_topic\":\"testtopic/Apica\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_6 = "{\"DUP\":false,\"QoS\":0,\"contents\":\"r\",\"publish_topic\":\"testtopic/Apica\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_8 = "{\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"UNSUBSCRIBE\",\"unsubscribe_topic\":[\"testtopic/Apica\"]}";
		String payLoad_10 = "{\"DUP\":false,\"QoS\":0,\"retain\":false,\"type\":\"DISCONNECT\"}";
		String payLoad_11 = "";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760612409L,1494239245587L , payLoad_0, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760620384L,1494239253567L , payLoad_2, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760629805L,1494239262993L , payLoad_4, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760633085L,1494239266277L , payLoad_6, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760636777L,1494239269973L , payLoad_8, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760638557L,1494239271757L , payLoad_10, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1489760638562L,1494239271868L , payLoad_11, WebSocketFrame.MQTT ));
