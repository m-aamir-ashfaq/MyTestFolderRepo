...
// -----------------------------------------
//
// Source: sessionWithWebSocketDa26.java
// Date  : 30 Jun 2016 12:32:45 ECT
 ...
 * Automatically generated load test program.
 */
public class sessionWithWebSocketDa26 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "sessionWithWebSocketDa26ExtParams.dat";// input file name for very large parameter values
	...
	private static String testAssign1 = "Ifra Eshaal";                           // var declaration from web admin var handler: scope = global
	private static String testAssign = "amir ashfaq";                            // var declaration from web admin var handler: scope = global
	...
	private String Rock = null;                          // var declaration from web admin var handler: scope = per loop
	

	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public sessionWithWebSocketDa26()
	{
		super();
	}
	 ...
	 */
	public sessionWithWebSocketDa26(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log("# Page #2: connect");
		log("# ----------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: connect", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0001 = "/collect" +
				"?v=1" +
				"&_v=j44" +
				"&a=1161958917" +
				"&t=pageview" +
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/GIF", null, 35, 5);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHeader0003 = "GET " + requestFile0003 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: q5N4IHP5vrWMd2gVHs7lyw==\r\n" + 
		...
		webSocketData = getWebSocketData_1467197446437();
		...
		{
			// extract var 'Rock'
			Rock = Lib.extractTextToken( testURL.getWebSocketContext().getRecordedData().getWebSocketFrames().get(5).getPayloadString(), 1, "< >'=&\"25");		// recorded value = Rock
			log("<<< Rock = " + Rock);
		}
		catch (Exception e) { Rock = null; log(e); }
		if (Rock == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'Rock' from websocket frame pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("sessionWithWebSocketDa26", concurrentUsers);
		...
		performanceData.setInfoText("sessionWithWebSocketDa26");
		...
		{
			System.out.println("global var <<< testAssign = " + testAssign);
			System.out.println("global var <<< testAssign1 = " + testAssign1);
			System.out.println();
		}
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				sessionWithWebSocketDa26 simulatedUser = new sessionWithWebSocketDa26(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	public static WebSocketData getWebSocketData_1467197446437()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1467197446552L);
		webSocketData.setConnectionStopMillis(1467197454719L);
		String payLoad_0 = "Rock it with HTML5 WebSocket";
		String payLoad_1 = "Rock it with HTML5 WebSocket";
		String payLoad_2 = "Rock it with HTML5 WebSocket 1";
		String payLoad_3 = "Rock it with HTML5 WebSocket 1";
		String payLoad_4 = "Rock it with HTML5 WebSocket 2";
		String payLoad_5 = "Rock it with HTML5 WebSocket 2";
		String payLoad_6 = "";
		String payLoad_7 = "";

		// search and replace frame payload "WebSocket" with variable 'testAssign1' on all occurrences of frame no 2
		payLoad_2 = Lib.replaceTextPattern(payLoad_2, "WebSocket", testAssign1, 0);

		// search and replace frame payload "HTML5" with variable 'testAssign' on all occurrences of all frames.
		payLoad_0 = Lib.replaceTextPattern(payLoad_0, "HTML5", testAssign, 0);
		payLoad_1 = Lib.replaceTextPattern(payLoad_1, "HTML5", testAssign, 0);
		payLoad_2 = Lib.replaceTextPattern(payLoad_2, "HTML5", testAssign, 0);
		payLoad_3 = Lib.replaceTextPattern(payLoad_3, "HTML5", testAssign, 0);
		payLoad_4 = Lib.replaceTextPattern(payLoad_4, "HTML5", testAssign, 0);
		payLoad_5 = Lib.replaceTextPattern(payLoad_5, "HTML5", testAssign, 0);
		payLoad_6 = Lib.replaceTextPattern(payLoad_6, "HTML5", testAssign, 0);
		payLoad_7 = Lib.replaceTextPattern(payLoad_7, "HTML5", testAssign, 0);


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1467197447211L,1467197447214L , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1467197447324L,1467197447326L , payLoad_1 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1467197451497L,1467197451498L , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1467197451608L,1467197451609L , payLoad_3 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1467197453598L,1467197453599L , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1467197453709L,1467197453711L , payLoad_5 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 8 , 1467197454606L,1467197454608L , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 8 , 1467197454717L,1467197454718L , payLoad_7 ));
