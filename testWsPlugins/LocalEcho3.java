...
// -----------------------------------------
//
// Source: LocalEcho3.java
// Date  : 26 Oct 2016 13:44:30 ECT
 ...
 * Automatically generated load test program.
 */
public class LocalEcho3 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "LocalEcho3ExtParams.dat";            // input file name for very large parameter values
	...
	private static String newPort = "8090";                                      // var declaration from web admin var handler: scope = global
	private static String globalVar = "0";                                       // var declaration from web admin var handler: scope = global
	...
	private Object wsplugin01 = null;                      // plug-in #1: "WSPluginAmirModifyWSAll" - definition from web admin var handler: construction scope = per user
	private static String wsplugin01WSPluginAmirModifyWSAll = null;// internal - not used in this load test program, but used by the GUI - reflective information
	 ...
	 */
	public LocalEcho3(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		try
		{
			...
			Class wspluginClass0 = getPluginClassLoader().getPluginClass("WSPluginAmirModifyWSAll");
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [1] <- WEB ADMIN Index 50 ---
		String requestProt0001 = "http";
		String requestHost0001 = "localhost";
		int    requestPort0001 = Integer.valueOf(newPort).intValue();
		String requestFile0001 = "/SocketServer_war_exploded/Echo" +
				"?encoding=text";
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: " + ParseUrl.reconstructHostHeaderField(requestProt0001, requestHost0001, requestPort0001) + "\r\n" + 		// recorded value = Host: localhost:8080
				...
				"Sec-WebSocket-Key: wAdfB+TwSsBElcdJDGIcjQ==\r\n" + 
		...
		webSocketData = getWebSocketData_1477295831017();

		WebSocketPluginInterface[] webSocketPluginsForURL = new WebSocketPluginInterface[1];

		((GenericPluginInterface) wsplugin01).setInputParameter(0, globalVar);			// set plug-in input parameter
		((GenericPluginInterface) wsplugin01).setInputParameter(1, loopCounter);			// set plug-in input parameter
		((GenericPluginInterface) wsplugin01).setInputParameter(2, userCounter);			// set plug-in input parameter
		...
		globalVar = (String) ((GenericPluginInterface) wsplugin01).getOutputParameter(0);		// get web socket plug-in output parameter
		log("<<< globalVar = " + globalVar);
		loopCounter = (String) ((GenericPluginInterface) wsplugin01).getOutputParameter(1);		// get web socket plug-in output parameter
		log("<<< loopCounter = " + loopCounter);
		userCounter = (String) ((GenericPluginInterface) wsplugin01).getOutputParameter(2);		// get web socket plug-in output parameter
		log("<<< userCounter = " + userCounter);
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("LocalEcho3", concurrentUsers);
		...
		performanceData.setInfoText("LocalEcho3");
		...
		{
			System.out.println("global var <<< globalVar = " + globalVar);
			System.out.println("global var <<< loopCounter = " + loopCounter_AtomicLong.longValue());
			System.out.println("global var <<< newPort = " + newPort);
			System.out.println();
		}
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new LocalEcho3());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				LocalEcho3 simulatedUser = new LocalEcho3(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	public WebSocketData getWebSocketData_1477295831017()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1477295831019L);
		webSocketData.setConnectionStopMillis(1477295963210L);
		String payLoad_0 = "Rock it with HTML5 WebSocket 1";
		String payLoad_2 = "Rock it with HTML5 WebSocket 2";
		String payLoad_4 = "Rock it with HTML5 WebSocket 3";
		String payLoad_6 = "seems no ping pong??/";
		String payLoad_8 = "";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477295835457L,1477295835458L , "UTF-8" , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477295838804L,1477295838805L , "UTF-8" , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477295842385L,1477295842386L , "UTF-8" , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477295914483L,1477295914484L , "UTF-8" , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1477295963205L,1477295963207L , "UTF-8" , payLoad_8 ));
