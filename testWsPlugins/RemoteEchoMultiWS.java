...
// -----------------------------------------
//
// Source: RemoteEchoMultiWS.java
// Date  : 01 Nov 2016 13:11:07 ECT
 ...
 * Automatically generated load test program.
 */
public class RemoteEchoMultiWS extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "RemoteEchoMultiWSExtParams.dat";     // input file name for very large parameter values
	...
	private static Object wsplugin01 = null;                      // plug-in #1: "forAllUrls" - definition from web admin var handler: construction scope = global
	private static String wsplugin01forAllUrls = null;            // internal - not used in this load test program, but used by the GUI - reflective information
	 ...
	 */
	public RemoteEchoMultiWS(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [1] <- WEB ADMIN Index 52 ---
		String requestProt0001 = "https";
		String requestHost0001 = "ie12.zopim.com";
		int    requestPort0001 = 443;
		String requestFile0001 = "/s/W/ws/BEPkDaHm7dXLoPUM/c/1477493721559";
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: /PTCF79sDWEqotYvIoiOOg==\r\n" + 
		...
		LoadtestPluginContext urlPluginBeforeCallContext0001 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);

		webSocketData = getWebSocketData_1477493721918();

		webSocketPluginsForURL = new WebSocketPluginInterface[1];

		webSocketPluginsForURL[0] = (WebSocketPluginInterface) wsplugin01;

		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, webSocketPluginsForURL, urlPluginBeforeCallContext0001));
		...
		// --- HTTP REQUEST: Test [2] <- WEB ADMIN Index 53 ---
		String requestProt0002 = "http";
		String requestHost0002 = "echo.websocket.org";
		...
		String requestHeader0002 = "GET " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"origin: http://www.websocket.org\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: JUgWQf9H4sK+M70jiU5gYg==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		LoadtestPluginContext urlPluginBeforeCallContext0002 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);

		webSocketData = getWebSocketData_1477493723720();

		webSocketPluginsForURL = new WebSocketPluginInterface[1];

		webSocketPluginsForURL[0] = (WebSocketPluginInterface) wsplugin01;

		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, webSocketPluginsForURL, urlPluginBeforeCallContext0002));
		...
		// --- HTTP REQUEST: Test [3] <- WEB ADMIN Index 94 ---
		String requestProt0003 = "https";
		String requestHost0003 = "ie12.zopim.com";
		int    requestPort0003 = 443;
		String requestFile0003 = "/s/W/ws/U0WSVa6qA-51E9a3/c/1477493762192";
		String requestHeader0003 = "GET " + requestFile0003 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"origin: http://www.websocket.org\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: G9jwvGadgVvgHxmLnH1nJw==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		LoadtestPluginContext urlPluginBeforeCallContext0003 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);

		webSocketData = getWebSocketData_1477493762760();

		webSocketPluginsForURL = new WebSocketPluginInterface[1];

		webSocketPluginsForURL[0] = (WebSocketPluginInterface) wsplugin01;

		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, webSocketPluginsForURL, urlPluginBeforeCallContext0003));
		...
		// --- HTTP REQUEST: Test [4] <- WEB ADMIN Index 95 ---
		String requestProt0004 = "http";
		String requestHost0004 = "echo.websocket.org";
		int    requestPort0004 = 80;
		String requestFile0004 = "/" +
				"?encoding=text";
		String requestHeader0004 = "GET " + requestFile0004 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"origin: http://www.websocket.org\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: 1O/4cCTJFu+Qed8c91zlrw==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		LoadtestPluginContext urlPluginBeforeCallContext0004 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);

		webSocketData = getWebSocketData_1477493764970();

		webSocketPluginsForURL = new WebSocketPluginInterface[1];

		webSocketPluginsForURL[0] = (WebSocketPluginInterface) wsplugin01;

		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, webSocketPluginsForURL, urlPluginBeforeCallContext0004));
		...
		// verify response: status code = 101, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: check size +/- 5%, defined content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {101}, null, null, 0, 5);		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [5] <- WEB ADMIN Index 136 ---
		String requestProt0005 = "https";
		String requestHost0005 = "ie12.zopim.com";
		int    requestPort0005 = 443;
		String requestFile0005 = "/s/W/ws/6kJKYbEhDKWOIGe9/c/1477493792600";
		String requestHeader0005 = "GET " + requestFile0005 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"origin: http://www.websocket.org\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: l3Bgn0/hat12LBfLa4Y3Vw==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		LoadtestPluginContext urlPluginBeforeCallContext0005 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);

		webSocketData = getWebSocketData_1477493793185();

		webSocketPluginsForURL = new WebSocketPluginInterface[1];

		webSocketPluginsForURL[0] = (WebSocketPluginInterface) wsplugin01;

		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, webSocketPluginsForURL, urlPluginBeforeCallContext0005));
		...
		// verify response: status code = 101, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: check size +/- 5%, defined content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {101}, null, null, 0, 5);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [6] <- WEB ADMIN Index 137 ---
		String requestProt0006 = "http";
		String requestHost0006 = "echo.websocket.org";
		...
		String requestHeader0006 = "GET " + requestFile0006 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"origin: http://www.websocket.org\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: 0hUdQ7BMMPSgU657F4BkbA==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		LoadtestPluginContext urlPluginBeforeCallContext0006 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);

		webSocketData = getWebSocketData_1477493794156();

		webSocketPluginsForURL = new WebSocketPluginInterface[1];

		webSocketPluginsForURL[0] = (WebSocketPluginInterface) wsplugin01;

		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, webSocketPluginsForURL, urlPluginBeforeCallContext0006));
		...
		// verify response: status code = 101, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: check size +/- 5%, defined content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {101}, null, null, 0, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [7] <- WEB ADMIN Index 138 ---
		String requestProt0007 = "http";
		String requestHost0007 = "echo.websocket.org";
		...
		String requestHeader0007 = "GET " + requestFile0007 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"origin: http://www.websocket.org\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: kIoZvhd6+wkxuiFXVQI6lw==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		LoadtestPluginContext urlPluginBeforeCallContext0007 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);

		webSocketData = getWebSocketData_1477493801030();

		webSocketPluginsForURL = new WebSocketPluginInterface[1];

		webSocketPluginsForURL[0] = (WebSocketPluginInterface) wsplugin01;

		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, webSocketPluginsForURL, urlPluginBeforeCallContext0007));
		...
		// verify response: status code = 101, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: check size +/- 5%, defined content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {101}, null, null, 0, 5);		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("RemoteEchoMultiWS", concurrentUsers);
		...
		final int MAX_THREAD_STEPS = 7;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("RemoteEchoMultiWS");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new RemoteEchoMultiWS());
		
		try
		{
			// define object for web socket plug-in #1 and call web socket plug-in constructor
			Class wspluginClass0 = getPluginClassLoader().getPluginClass("forAllUrls");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				RemoteEchoMultiWS simulatedUser = new RemoteEchoMultiWS(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	public WebSocketData getWebSocketData_1477493721918()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1477493721989L);
		webSocketData.setConnectionStopMillis(1477493761085L);
		String payLoad_1 = "1477493721993\n0\n1\n0\nd\n{\"__type\":\"register\",\"accountKey\":\"3vCaGRDlZbbiX5tJXSlIABEcEDu5cGAg\",\"cookie_law\":false,\"dt\":\"desktop\",\"mID\":\"c4flcS4pJyF2SF\",\"ref\":\"\",\"rev\":\"2b77e9bd66aa9ed078596996564b23d67171d6ae\",\"source\":\"widget_v2\",\"theme\":\"simple\",\"title\":\"websocket.org Echo Test - Powered by Kaazing\",\"ua\":\"Mozilla/5.0 (Macintosh; Intel Mac OS X 10.12; rv:49.0) Gecko/20100101 Firefox/49.0\",\"url\":\"http://www.websocket.org/echo.html\"}";
		String payLoad_6 = "1477493723077\n74.03399839962265\n2\n5\nd\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"start_exec\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.334},{\"method\":\"increment\",\"name\":\"conn\",\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\",\"tries:1\"],\"value\":1},{\"method\":\"histogram\",\"name\":\"conn_open\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\",\"proxy:ie12.zopim.com\"],\"value\":0.432},{\"method\":\"histogram\",\"name\":\"conn_status_connected\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.497}]}\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"conn_status_registered\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":1.18}]}\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"conn_status_reattached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":1.181}]}\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"active$int\":1477493721560}}\n{\"path\":[\"root\",\"livechat\",\"channel\",\"tags\"],\"value\":{\"added$string\":\"websocket_org\"}}\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"disconnect_timeout$int\":20}}\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"init_ui_components_no_cached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":1.183}]}";
		String payLoad_7 = "1477493723123\n74.03399839962265\n3\n5\nd\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"theme_loaded_no_cached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":1.539}]}";
		String payLoad_8 = "1477493741564\n74.03399839962265\n4\n5\nd\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"active$int\":1477493741562}}";
		String payLoad_9 = "\u0003\u00e9";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493722016L,1477493722019L , "UTF-8" , payLoad_1 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493723077L,1477493723079L , "UTF-8" , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493723124L,1477493723125L , "UTF-8" , payLoad_7 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493741565L,1477493741566L , "UTF-8" , payLoad_8 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1477493761019L,1477493761021L , "ISO-8859-1" , payLoad_9 ));
		return webSocketData;
	}
	
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1477493723720()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1477493723835L);
		webSocketData.setConnectionStopMillis(1477493761138L);
		String payLoad_0 = "Rock it with HTML5 WebSocket";
		String payLoad_2 = "Rock it with HTML5 WebSocket";
		String payLoad_4 = "Rock it with HTML5 WebSocket";
		String payLoad_6 = "Rock it with HTML5 WebSocket";
		String payLoad_8 = "Rock it with HTML5 WebSocket";
		String payLoad_10 = "Rock it with HTML5 WebSocket";
		String payLoad_12 = "\u0003\u00e9";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493725071L,1477493725072L , "UTF-8" , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493735403L,1477493735404L , "UTF-8" , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493737539L,1477493737540L , "UTF-8" , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493739842L,1477493739843L , "UTF-8" , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493746736L,1477493746737L , "UTF-8" , payLoad_8 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493755002L,1477493755003L , "UTF-8" , payLoad_10 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1477493761019L,1477493761022L , "ISO-8859-1" , payLoad_12 ));
		return webSocketData;
	}
	
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1477493762760()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1477493762825L);
		webSocketData.setConnectionStopMillis(1477493791534L);
		String payLoad_1 = "1477493762837\n0\n1\n0\nd\n{\"__type\":\"register\",\"accountKey\":\"3vCaGRDlZbbiX5tJXSlIABEcEDu5cGAg\",\"cookie_law\":false,\"dt\":\"desktop\",\"mID\":\"c4flcS4pJyF2SF\",\"ref\":\"\",\"rev\":\"2b77e9bd66aa9ed078596996564b23d67171d6ae\",\"source\":\"widget_v2\",\"theme\":\"simple\",\"title\":\"websocket.org Echo Test - Powered by Kaazing\",\"ua\":\"Mozilla/5.0 (Macintosh; Intel Mac OS X 10.12; rv:49.0) Gecko/20100101 Firefox/49.0\",\"url\":\"http://www.websocket.org/echo.html\"}";
		String payLoad_4 = "1477493762978\n47.9730586972647\n2\n3\nd\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"start_exec\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.242},{\"method\":\"histogram\",\"name\":\"init_ui_components_cached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.036},{\"method\":\"histogram\",\"name\":\"theme_loaded_cached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.361},{\"method\":\"increment\",\"name\":\"conn\",\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\",\"tries:1\"],\"value\":1},{\"method\":\"histogram\",\"name\":\"conn_open\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\",\"proxy:ie12.zopim.com\"],\"value\":0.642},{\"method\":\"histogram\",\"name\":\"conn_status_connected\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.708}]}\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"conn_status_registered\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.804}]}\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"conn_status_reattached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.805}]}\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"active$int\":1477493762193}}\n{\"path\":[\"root\",\"livechat\",\"channel\",\"tags\"],\"value\":{\"added$string\":\"websocket_org\"}}\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"disconnect_timeout$int\":20}}";
		String payLoad_5 = "1477493782207\n47.9730586972647\n3\n3\nd\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"active$int\":1477493782205}}";
		String payLoad_6 = "\u0003\u00e9";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493762838L,1477493762839L , "UTF-8" , payLoad_1 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493762978L,1477493762980L , "UTF-8" , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493782207L,1477493782208L , "UTF-8" , payLoad_5 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1477493791462L,1477493791463L , "ISO-8859-1" , payLoad_6 ));
		return webSocketData;
	}
	
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1477493764970()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1477493765086L);
		webSocketData.setConnectionStopMillis(1477493791579L);
		String payLoad_0 = "Rock it with HTML5 WebSocket";
		String payLoad_2 = "Rock it with HTML5 WebSocket";
		String payLoad_4 = "Rock it with HTML5 WebSocket";
		String payLoad_6 = "\u0003\u00e9";
		...
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493766508L,1477493766510L , "UTF-8" , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493768727L,1477493768728L , "UTF-8" , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493771122L,1477493771123L , "UTF-8" , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1477493791462L,1477493791463L , "ISO-8859-1" , payLoad_6 ));
		return webSocketData;
	}
	
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1477493793185()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1477493793251L);
		webSocketData.setConnectionStopMillis(1477493813839L);
		String payLoad_1 = "1477493793262\n0\n1\n0\nd\n{\"__type\":\"register\",\"accountKey\":\"3vCaGRDlZbbiX5tJXSlIABEcEDu5cGAg\",\"cookie_law\":false,\"dt\":\"desktop\",\"mID\":\"c4flcS4pJyF2SF\",\"ref\":\"\",\"rev\":\"2b77e9bd66aa9ed078596996564b23d67171d6ae\",\"source\":\"widget_v2\",\"theme\":\"simple\",\"title\":\"websocket.org Echo Test - Powered by Kaazing\",\"ua\":\"Mozilla/5.0 (Macintosh; Intel Mac OS X 10.12; rv:49.0) Gecko/20100101 Firefox/49.0\",\"url\":\"http://www.websocket.org/echo.html\"}";
		String payLoad_4 = "1477493793409\n52.95733306438564\n2\n3\nd\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"start_exec\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.247},{\"method\":\"histogram\",\"name\":\"init_ui_components_cached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.037},{\"method\":\"histogram\",\"name\":\"theme_loaded_cached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.373},{\"method\":\"increment\",\"name\":\"conn\",\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\",\"tries:1\"],\"value\":1},{\"method\":\"histogram\",\"name\":\"conn_open\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\",\"proxy:ie12.zopim.com\"],\"value\":0.657},{\"method\":\"histogram\",\"name\":\"conn_status_connected\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.728}]}\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"conn_status_registered\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.824}]}\n{\"__type\":\"instrumentation\",\"metrics\":[{\"method\":\"histogram\",\"name\":\"conn_status_reattached\",\"sample_rate\":0.25,\"tags\":[\"browser:firefox\",\"device:desktop\",\"platform:mac\",\"rev:2b77e9bd66aa9ed078596996564b23d67171d6ae\"],\"value\":0.825}]}\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"active$int\":1477493792601}}\n{\"path\":[\"root\",\"livechat\",\"channel\",\"tags\"],\"value\":{\"added$string\":\"websocket_org\"}}\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"disconnect_timeout$int\":20}}";
		String payLoad_5 = "1477493812616\n52.95733306438564\n3\n3\nd\n{\"path\":[\"root\",\"livechat\",\"profile\"],\"value\":{\"active$int\":1477493812614}}";
		String payLoad_6 = "\u0003\u00e9";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493793264L,1477493793265L , "UTF-8" , payLoad_1 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493793410L,1477493793411L , "UTF-8" , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493812616L,1477493812617L , "UTF-8" , payLoad_5 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1477493813774L,1477493813774L , "ISO-8859-1" , payLoad_6 ));
		return webSocketData;
	}
	
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1477493794156()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1477493794271L);
		webSocketData.setConnectionStopMillis(1477493798444L);
		String payLoad_0 = "Rock it with HTML5 WebSocket";
		String payLoad_2 = "Rock it with HTML5 WebSocket";
		String payLoad_4 = "Rock it with HTML5 WebSocket";
		String payLoad_6 = "Rock it with HTML5 WebSocket";
		...
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493795212L,1477493795213L , "UTF-8" , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493796420L,1477493796421L , "UTF-8" , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493796581L,1477493796582L , "UTF-8" , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493796732L,1477493796734L , "UTF-8" , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493796870L,1477493796871L , "UTF-8" , payLoad_8 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493797015L,1477493797015L , "UTF-8" , payLoad_10 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1477493798329L,1477493798329L , "UTF-8" , payLoad_12 ));
		return webSocketData;
	}
	
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1477493801030()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1477493801140L);
		webSocketData.setConnectionStopMillis(1477493804757L);
		String payLoad_0 = "Rock it with HTML5 WebSocket";
		String payLoad_2 = "Rock it with HTML5 WebSocket";
		String payLoad_4 = "Rock it with HTML5 WebSocket";
		String payLoad_6 = "Rock it with HTML5 WebSocket";
		...
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493803175L,1477493803176L , "UTF-8" , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493803345L,1477493803345L , "UTF-8" , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493803514L,1477493803514L , "UTF-8" , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493803667L,1477493803668L , "UTF-8" , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1477493803844L,1477493803845L , "UTF-8" , payLoad_8 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1477493804645L,1477493804646L , "UTF-8" , payLoad_10 ));
		return webSocketData;
	}
