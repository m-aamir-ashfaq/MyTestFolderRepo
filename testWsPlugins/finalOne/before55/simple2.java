 ...
 * Automatically generated load test program.
 */
public class simple2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "simple2ExtParams.dat";               // input file name for very large parameter values
	...
	private static Object wsplugin02 = null;                      // plug-in #2: "testPlugin" - definition from web admin var handler: construction scope = global
	private static String wsplugin02testPlugin = null;            // internal - not used in this load test program, but used by the GUI - reflective information
	 ...
	 */
	public simple2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		webSocketPluginsForURL[0] = (WebSocketPluginInterface) wsplugin02;
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("simple2", concurrentUsers);
		...
		performanceData.setInfoText("simple2");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new simple2());
		
		try
		{
			// define object for web socket plug-in #2 and call web socket plug-in constructor
			Class wspluginClass0 = getPluginClassLoader().getPluginClass("testPlugin");
			wsplugin02 = wspluginClass0.newInstance();
			((GenericPluginInterface) wsplugin02).construct(globalPluginContext);
		}
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				simple2 simulatedUser = new simple2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
			...
			if (abortedByRemote() || abortedByEofInputFile())
			{
				...
				synchronized (dumpLock)
				{
					...
					try
					{
						...
						((GenericPluginInterface) wsplugin02).deconstruct(globalPluginContext);
					}
					catch (Throwable pluginThrowable)
					{
