...
// -----------------------------------------
//
// Source: finalTestWithtestTest.java
// Date  : 16 Nov 2016 08:27:05 ECT
 ...
 * Automatically generated load test program.
 */
public class finalTestWithtestTest extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "finalTestWithtestTestExtParams.dat"; // input file name for very large parameter values
	...
	private static AtomicLong loopCounter_AtomicLong = new AtomicLong(0);
	private String loopCounter = "0";                                     // var declaration from web admin var handler: scope = global
	private static String globalCounter = "1";                                   // var declaration from web admin var handler: scope = global
	...
	private static Object wsplugin01 = null;                      // plug-in #1: "testTest" - definition from web admin var handler: construction scope = global
	private static String wsplugin01testTest = null;              // internal - not used in this load test program, but used by the GUI - reflective information

	private String userCounter = "0";                                     // var declaration from web admin var handler: scope = per user

	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public finalTestWithtestTest()
	{
		super();
	}
	 ...
	 */
	public finalTestWithtestTest(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
		...
		userCounter = "" + threadNumber;		// initialize current user counter
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "www.websocket.org";
		int    requestPort0001 = 80;
		String requestFile0001 = "/echo.html";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "The following link lists the browsers that support WebSocket:");		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 635, 5);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 670, 5);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [4] <- WEB ADMIN Index 34 ---
		String requestProt0004 = "http";
		String requestHost0004 = "www.websocket.org";
		int    requestPort0004 = 80;
		String requestFile0004 = "/img/websocketlogo-medium.png";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 6580, 5);		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 82, 5);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [6] <- WEB ADMIN Index 36 ---
		String requestProt0006 = "http";
		String requestHost0006 = "www.websocket.org";
		int    requestPort0006 = 80;
		String requestFile0006 = "/img/websocketlogo-medium-white.png";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 18496, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, null, null, 3638, 5);		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [8] <- WEB ADMIN Index 47 ---
		String requestProt0008 = "http";
		String requestHost0008 = "assets.zendesk.com";
		int    requestPort0008 = 80;
		String requestFile0008 = "/embeddable_framework/update.html" +
				"?t=24637449";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 207, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [9] <- WEB ADMIN Index 57 ---
		String requestProt0009 = "http";
		String requestHost0009 = "echo.websocket.org";
		int    requestPort0009 = 80;
		String requestFile0009 = "/" +
				"?encoding=text";
		String requestHeader0009 = "GET " + requestFile0009 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: RgeGQxP9z+/6pvO1j37t1A==\r\n" + 
		...
		LoadtestPluginContext urlPluginBeforeCallContext0009 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);

		webSocketData = getWebSocketData_1478246952973();

		webSocketPluginsForURL = new WebSocketPluginInterface[1];

		((GenericPluginInterface) wsplugin01).setInputParameter(0, loopCounter);			// set plug-in input parameter
		webSocketPluginsForURL[0] = (WebSocketPluginInterface) wsplugin01;

		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, webSocketPluginsForURL, urlPluginBeforeCallContext0009));
		...
		loopCounter = (String) ((GenericPluginInterface) wsplugin01).getOutputParameter(0);		// get web socket plug-in output parameter
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 15477
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "The following link lists the browsers that support WebSocket:");		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 635
		// content test algorithm: check size +/- 5%, defined content size = 635
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 635, 5);		// Test [11] <- Index 11
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 6580
		// content test algorithm: check size +/- 5%, defined content size = 6580
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 6580, 5);		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 82
		// content test algorithm: check size +/- 5%, defined content size = 82
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 82, 5);		// Test [13] <- Index 13
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 670
		// content test algorithm: check size +/- 5%, defined content size = 670
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 670, 5);		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [15] <- WEB ADMIN Index 92 ---
		String requestProt0015 = "https";
		String requestHost0015 = "www.websocket.org";
		int    requestPort0015 = 443;
		String requestFile0015 = "/img/websocketlogo-medium-white.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 18496
		// content test algorithm: check size +/- 5%, defined content size = 18496
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 18496, 5);		// Test [15] <- Index 15
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// verify response: status code = 200, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 3638
		// content test algorithm: check size +/- 5%, defined content size = 3638
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, null, null, 3638, 5);		// Test [16] <- Index 16
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [17] <- WEB ADMIN Index 97 ---
		String requestProt0017 = "https";
		String requestHost0017 = "assets.zendesk.com";
		int    requestPort0017 = 443;
		String requestFile0017 = "/embeddable_framework/update.html" +
				"?t=24637449";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 207
		// content test algorithm: check size +/- 5%, defined content size = 207
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 207, 5);		// Test [17] <- Index 17
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [18] <- WEB ADMIN Index 103 ---
		String requestProt0018 = "https";
		String requestHost0018 = "echo.websocket.org";
		int    requestPort0018 = 443;
		String requestFile0018 = "/" +
				"?encoding=text";
		String requestHeader0018 = "GET " + requestFile0018 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: I15pb7rlLslhA+lFSundYQ==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		webSocketData = getWebSocketData_1478246980538();
		...
		// verify response: status code = 101, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: check size +/- 5%, defined content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {101}, null, null, 0, 5);		// Test [18] <- Index 18
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 245, 5);		// Test [19] <- Index 19
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	public void run()
	{
		...
		try
		{
			while (true)
			{
				...
				loopCounter = "" + loopCounter_AtomicLong.getAndIncrement();		// increment global loop counter after assignment
					log("<<< loopCounter = " + loopCounter);
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("finalTestWithtestTest", concurrentUsers);
		...
		performanceData.setInfoText("finalTestWithtestTest");
		...
		{
			System.out.println("global var <<< globalCounter = " + globalCounter);
			System.out.println("global var <<< loopCounter = " + loopCounter_AtomicLong.longValue());
			System.out.println();
		}
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new finalTestWithtestTest());
		
		try
		{
			// define object for web socket plug-in #1 and call web socket plug-in constructor
			Class wspluginClass0 = getPluginClassLoader().getPluginClass("testTest");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				finalTestWithtestTest simulatedUser = new finalTestWithtestTest(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	public WebSocketData getWebSocketData_1478246952973()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1478246953094L);
		webSocketData.setConnectionStopMillis(1478246967656L);
		String payLoad_0 = "Rock it with HTML5 WebSocket plain 1";
		String payLoad_2 = "Rock it with HTML5 WebSocket plain 2";
		String payLoad_4 = "";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1478246959263L,1478246959263L , "UTF-8" , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1478246963764L,1478246963764L , "UTF-8" , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1478246967537L,1478246967538L , "UTF-8" , payLoad_4 ));
		return webSocketData;
	}
	
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1478246980538()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1478246980653L);
		webSocketData.setConnectionStopMillis(1478246990898L);
		String payLoad_0 = "Rock it with HTML5 WebSocket ssl 1";
		String payLoad_2 = "Rock it with HTML5 WebSocket ssl 2";
		String payLoad_4 = "";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1478246983816L,1478246983816L , "UTF-8" , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1478246987414L,1478246987414L , "UTF-8" , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1478246990781L,1478246990782L , "UTF-8" , payLoad_4 ));
		return webSocketData;
	}
