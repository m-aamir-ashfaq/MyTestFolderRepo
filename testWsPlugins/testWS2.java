 ...
 * Automatically generated load test program.
 */
public class testWS2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "testWS2ExtParams.dat";               // input file name for very large parameter values
	...
	private static String var_amir = "dfv";                                      // var declaration from web admin var handler: scope = global
	...
	private String kkj1Loop = null;                      // var declaration from web admin var handler: scope = per loop
	
	private Object wsplugin01 = null;                      // plug-in #1: "TestWS_MAIN" - definition from web admin var handler: construction scope = per user
	private static String wsplugin01TestWS_MAIN = null;           // internal - not used in this load test program, but used by the GUI - reflective information
	 ...
	 */
	public testWS2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		try
		{
			...
			Class wspluginClass0 = getPluginClassLoader().getPluginClass("TestWS_MAIN");
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [1] <- WEB ADMIN Index 49 ---
		...
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: 0F5Bh9KmVuVWOKJFjN9JAA==\r\n" + 
		...
		webSocketData = getWebSocketData_1476876542359();

		WebSocketPluginInterface[] webSocketPluginsForURL = new WebSocketPluginInterface[1];

		((GenericPluginInterface) wsplugin01).setInputParameter(0, kkj1Loop);			// set plug-in input parameter
		...
		kkj1Loop = (String) ((GenericPluginInterface) wsplugin01).getOutputParameter(0);		// get web socket plug-in output parameter
		log("<<< kkj1Loop = " + kkj1Loop);
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("testWS2", concurrentUsers);
		...
		performanceData.setInfoText("testWS2");
		...
		{
			System.out.println("global var <<< var_amir = " + var_amir);
			System.out.println();
		}
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new testWS2());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				testWS2 simulatedUser = new testWS2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	public WebSocketData getWebSocketData_1476876542359()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1476876542479L);
		webSocketData.setConnectionStopMillis(1476876606521L);
		...
		String payLoad_6 = "Rock it with HTML5 WebSocket 3 (last)";
		String payLoad_8 = "";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1476876545180L,1476876545181L , "UTF-8" , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1476876549826L,1476876549827L , "UTF-8" , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1476876561175L,1476876561176L , "UTF-8" , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1476876604398L,1476876604399L , "UTF-8" , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1476876606404L,1476876606405L , "UTF-8" , payLoad_8 ));
