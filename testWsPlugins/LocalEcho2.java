...
// -----------------------------------------
//
// Source: LocalEcho2.java
// Date  : 26 Oct 2016 13:20:00 ECT
 ...
 * Automatically generated load test program.
 */
public class LocalEcho2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "LocalEcho2ExtParams.dat";            // input file name for very large parameter values
	...
	private String userVar = "0";                                         // var declaration from web admin var handler: scope = per user

	private String loopVar = null;                       // var declaration from web admin var handler: scope = per loop
	 ...
	 */
	public LocalEcho2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
		...
		userVar = "" + threadNumber;		// initialize current user counter
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		((GenericPluginInterface) wsplugin01).setInputParameter(2, userVar);			// set plug-in input parameter
		((GenericPluginInterface) wsplugin01).setInputParameter(1, loopVar);			// set plug-in input parameter
		...
		newPort = (String) ((GenericPluginInterface) wsplugin01).getOutputParameter(1);		// get web socket plug-in output parameter
		log("<<< newPort = " + newPort);
		...
		userVar = (String) ((GenericPluginInterface) wsplugin01).getOutputParameter(2);		// get web socket plug-in output parameter
		log("<<< userVar = " + userVar);
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("LocalEcho2", concurrentUsers);
		...
		performanceData.setInfoText("LocalEcho2");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new LocalEcho2());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				LocalEcho2 simulatedUser = new LocalEcho2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
