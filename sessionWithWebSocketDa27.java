...
// -----------------------------------------
//
// Source: sessionWithWebSocketDa27.java
// Date  : 06 Jul 2016 09:49:19 ECT
 ...
 * Automatically generated load test program.
 */
public class sessionWithWebSocketDa27 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "sessionWithWebSocketDa27ExtParams.dat";// input file name for very large parameter values
	...
	private static String temp2 = Lib.getFormattedCurrentDateAndTime("yyyy-MM-dd", 0, "ECT"); // var declaration from web admin var handler: scope = global
	private static String temp1 = "amir ashfaq";                                 // var declaration from web admin var handler: scope = global
	...
	private String env = null;                           // var declaration from web admin var handler: scope = per loop
	

	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public sessionWithWebSocketDa27()
	{
		super();
	}
	 ...
	 */
	public sessionWithWebSocketDa27(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 14742
		...
		// --- HTTP REQUEST: Test [2] <- WEB ADMIN Index 4 ---
		String requestProt0002 = "https";
		String requestHost0002 = "fonts.googleapis.com";
		int    requestPort0002 = 443;
		String requestFile0002 = "/css" +
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 314, 5);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
		...
		// --- HTTP REQUEST: Test [3] <- WEB ADMIN Index 5 ---
		...
		String requestHost0005 = "script.crazyegg.com";
		int    requestPort0005 = 443;
		String requestFile0005 = "/pages/scripts/0037/0800.js" +
				"?407578";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 3507, 5);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0006 = "stats.g.doubleclick.net";
		int    requestPort0006 = 443;
		String requestFile0006 = "/r/collect" +
				"?v=1" +
				"&aip=1" +
				"&t=dc" +
				"&_r=3" +
		...
		String requestHeader0007 = "GET " + requestFile0007 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: MPi2q7Nh/ItpOt2NPoLNdg==\r\n" + 
		...
		webSocketData = getWebSocketData_1467284204574();
		...
		{
			// extract var 'env'
			env = Lib.extractTextToken( testURL.getWebSocketContext().getRecordedData().getWebSocketFramesWithoutPingPong().get(5).getPayloadString(), 3, "< >'=&\"");		// recorded value = environments
			log("<<< env = " + env);
		}
		catch (Exception e) { env = null; log(e); }
		if (env == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'env' from websocket frame pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0008 = "/img/tls-lock.png";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 245, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("sessionWithWebSocketDa27", concurrentUsers);
		...
		final int MAX_THREAD_STEPS = 8;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("sessionWithWebSocketDa27");
		...
		{
			System.out.println("global var <<< temp1 = " + temp1);
			System.out.println("global var <<< temp2 = " + temp2);
			System.out.println();
		}
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				sessionWithWebSocketDa27 simulatedUser = new sessionWithWebSocketDa27(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	public static WebSocketData getWebSocketData_1467284204574()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1467284204688L);
		webSocketData.setConnectionStopMillis(1467284271539L);
		String payLoad_0 = "Rock it with HTML5 WebSocket";
		String payLoad_1 = "Rock it with HTML5 WebSocket";
		String payLoad_2 = "Wait for pingpong to happen????";
		String payLoad_3 = "Wait for pingpong to happen????";
		String payLoad_4 = "In some environments the WebSocket connection may fail due to intermediary firewalls, proxies, routers, etc. In that case take advantage of WebSocket's secure capability and check Use secure WebSocket (TLS). Even if you have no issues you can still feel free to test using a secure connection. ";
		String payLoad_5 = "In some environments the WebSocket connection may fail due to intermediary firewalls, proxies, routers, etc. In that case take advantage of WebSocket's secure capability and check Use secure WebSocket (TLS). Even if you have no issues you can still feel free to test using a secure connection. ";
		String payLoad_6 = "";
		String payLoad_7 = "";

		// search and replace frame payload "connection" with variable 'temp2' at occurrence position 2 of frame no 4
		payLoad_4 = Lib.replaceTextPattern(payLoad_4, "connection", temp2, 2);

		// search and replace frame payload "?" with variable 'temp1' at occurrence position 2 of frame no 2
		payLoad_2 = Lib.replaceTextPattern(payLoad_2, "?", temp1, 2);


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1467284209824L,1467284209828L , payLoad_0 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1467284209952L,1467284209954L , payLoad_1 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1467284229581L,1467284229582L , payLoad_2 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1467284229692L,1467284229694L , payLoad_3 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 1 , 1467284267507L,1467284267508L , payLoad_4 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 1 , 1467284267710L,1467284267712L , payLoad_5 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , (byte) 8 , 1467284271405L,1467284271406L , payLoad_6 ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , false , (byte) 8 , 1467284271518L,1467284271534L , payLoad_7 ));
