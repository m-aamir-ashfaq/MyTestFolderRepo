...
public class demo8_IO2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static String timeStart = "" + System.currentTimeMillis();           // var declaration from web admin var handler: scope = global
	...
	private String idFromResponseContect = null;         // var declaration from web admin var handler: scope = per loop
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		idFromResponseContect = null;
		log();
		log("<<< idFromResponseContect = " + idFromResponseContect);
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0001 = "/socket.io/1/" +
				"?t=1470903812860";
		requestFile0001 = Lib.replaceTextPattern(requestFile0001, "1470903812860", timeStart, 0);		// search and replace request file fragment "1470903812860" with variable 'timeStart' on all occurrences
		...
		// setup var extractor for 'idFromResponseContect'
		...
		try
		{
			// extract var 'idFromResponseContect'
			idFromResponseContect = lineTokenExtractorItem0001_1.getResultToken();
			log("<<< idFromResponseContect = " + idFromResponseContect);
		}
		catch (Exception e) { idFromResponseContect = null; log(e); }
		if (idFromResponseContect == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'idFromResponseContect' from text line pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		requestFile0002 = Lib.replaceTextPattern(requestFile0002, "1686981358193297010", idFromResponseContect, 0);		// search and replace request file fragment "1686981358193297010" with variable 'idFromResponseContect' on all occurrences
	...
	public static void main(String[] args)
	{
		...
		if (debugLoops || debugFailedLoops)
		{
			...
			System.out.println("global var <<< timeStart = " + timeStart);
			System.out.println();
		}
	...
	{
		return "//print idFromResponseContect\r\n//idFromResponseContect = \"/socket.io/1/websocket/\"+ idFromResponseContect\r\n\r\nstr = strSubString(time,1,10)\r\ntime = toNumeric(str)\r\n";
	}
