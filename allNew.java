 ...
 * Automatically generated load test program.
 */
public class allNew extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "allNewExtParams.dat";                // input file name for very large parameter values
	 ...
	 */
	public allNew(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHeader0002 = "GET " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: 4xjjD9xH6ZROYEFxsRt3uw==\r\n" + 
		...
		webSocketData.setConnectionStartMillis(1465915079946L);
		webSocketData.setConnectionStopMillis(1465915088480L);
		...
		payload[28] = 91;
		payload[29] = 113;
		webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,true,30,maskingKey,payload,false,0,(byte)-127,(byte)-98,extendedPayloadByte,1465915083309L,1465915083311L));
		payload  = new byte[30];
		payload[0] = 82;		payload[1] = 111;
		payload[2] = 99;
		payload[3] = 107;
		payload[4] = 32;
		payload[5] = 105;
		...
		payload[28] = 32;
		payload[29] = 49;
		webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,false,30,maskingKey,payload,false,0,(byte)-127,(byte)30,extendedPayloadByte,1465915083422L,1465915083423L));
		 maskingKey  = new byte[4];
		maskingKey[0] = -14;
		maskingKey[1] = -118;
		maskingKey[2] = 59;
		...
		payload[28] = -46;
		payload[29] = -72;
		webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,true,30,maskingKey,payload,false,0,(byte)-127,(byte)-98,extendedPayloadByte,1465915086424L,1465915086425L));
		payload  = new byte[30];
		payload[0] = 82;		payload[1] = 111;
		payload[2] = 99;
		payload[3] = 107;
		...
		payload[28] = 32;
		payload[29] = 50;
		webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)1,false,30,maskingKey,payload,false,0,(byte)-127,(byte)30,extendedPayloadByte,1465915086534L,1465915086536L));
		 maskingKey  = new byte[4];
		maskingKey[0] = -16;
		maskingKey[1] = -27;
		maskingKey[2] = -13;
		maskingKey[3] = 8;
		payload  = new byte[0];
		webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)8,true,0,maskingKey,payload,false,0,(byte)-120,(byte)-128,extendedPayloadByte,1465915088357L,1465915088358L));
		payload  = new byte[0];
		webSocketData.addWebSocketFrame(new WebSocketFrame(true,false,false,false,(byte)8,false,0,maskingKey,payload,false,0,(byte)-120,(byte)0,extendedPayloadByte,1465915088478L,1465915088480L));
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("allNew", concurrentUsers);
		...
		performanceData.setInfoText("allNew");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				allNew simulatedUser = new allNew(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
