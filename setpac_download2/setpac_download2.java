...
// -----------------------------------------
//
// Source: setpac_download2.java
// Date  : 17 Aug 2016 13:53:56 ECT
 ...
 * Automatically generated load test program.
 */
public class setpac_download2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.10; rv:42.0) Gecko/20100101 Firefox/42.0";
	...
	private static final String EXTERNAL_PARAM_FILE = "setpac_download2ExtParams.dat";      // input file name for very large parameter values
	...
	private static String Etag_tag = "659b2350f042170c99586f95d4b0ebbd";         // var declaration from web admin var handler: scope = global
	...
	private static Object plugin0001 = null;                      // plug-in #1: "VerifyTextExists" - definition from web admin var handler: construction scope = global
	private static String plugin0001VerifyTextExists = null;      // internal - not used in this load test program, but used by the GUI - reflective information

	private String Etag = null;                          // var declaration from web admin var handler: scope = per loop
	

	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public setpac_download2()
	{
		super();
	}
	 ...
	 */
	public setpac_download2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log();
		log();
		log("# Page #1: Setpac download");
		log("# ------------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #1: Setpac download", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		...
		log();
		log();
		log("# Page #2: Setpac download");
		log("# ------------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: Setpac download", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "cloudproxy.setpac.ge.com";
		int    requestPort0001 = 443;
		String requestFile0001 = "/pac.pac";
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: cloudproxy.setpac.ge.com\r\n" + 
		...
		LoadtestPluginContext urlPluginContext0001 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);

		// verify response: status code = 200, content type = "APPLICATION/OCTET-STREAM", header text fragment = [verification disabled], recorded content size = 2745
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/OCTET-STREAM", null, "function findHostInArray(host, aElements) {");		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
			...
			urlPluginContext0001.setUrlPassed(false);
			log(">>> execute plug-in #1: VerifyTextExists");
			synchronized (plugin0001)
			{
				((GenericPluginInterface) plugin0001).setInputParameter(0, Etag_tag);		// set plug-in input parameter
				((GenericPluginInterface) plugin0001).setInputParameter(1, Etag);		// set plug-in input parameter
				((GenericPluginInterface) plugin0001).execute(urlPluginContext0001);			// execute plug-in
		...
		{
			// extract var 'Etag'
			Etag = testURL.getHeaderField("ETag");		// recorded value = "d5dffdc008c9c904698f0a0d682cc69"
			log("<<< Etag = " + Etag);
		}
		catch (Exception e) { Etag = null; log(e); }
		if (Etag == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'Etag' from http response header field";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// execute plug-in #1 after url call
		urlPluginContext0001.setUrlPassed(urlCallPassed);
		log(">>> execute plug-in #1: VerifyTextExists");
		synchronized (plugin0001)
		{
			((GenericPluginInterface) plugin0001).setInputParameter(0, Etag_tag);		// set plug-in input parameter
			...
			if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
				log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
			if (urlPluginContext0001.getSpecificErrorMessage() != null)
			{
				...
				log(urlPluginContext0001.getSpecificErrorMessage());
				performanceData.getPerformanceDataRecord(threadStep).modifyFailureActionType(HttpTestURL.FAILURE_ACTION_HANDLE_AS_ERROR);
				performanceData.setFailed(threadStep, urlPluginContext0001.getSpecificErrorStatusType(), urlPluginContext0001.getSpecificErrorMessage(), testURL, this);
				terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
				endOfExecuteLoop(false, testURL, threadStep, loopPluginContext);
				return false;
			}
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0003 = "cloudproxy.setpac.ge.com";
		int    requestPort0003 = 443;
		String requestFile0003 = "/pac.pac";
		...
		// verify response: status code = 200, content type = "APPLICATION/OCTET-STREAM", header text fragment = [verification disabled], recorded content size = 2745
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/OCTET-STREAM", null, "function findHostInArray(host, aElements) {");		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("setpac_download2", concurrentUsers);
		...
		performanceData.setInfoText("setpac_download2");
		...
		{
			System.out.println("global var <<< Etag_tag = " + Etag_tag);
			System.out.println();
		}
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new setpac_download2());
		try
		{
			// define object for plug-in #1 and call plug-in constructor
			Class pluginClass0 = getPluginClassLoader().getPluginClass("VerifyTextExists");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				setpac_download2 simulatedUser = new setpac_download2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
