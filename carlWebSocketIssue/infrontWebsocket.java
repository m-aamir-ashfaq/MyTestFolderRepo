...
// -----------------------------------------
//
// Source: infrontWebsocket.java
// Date  : 20 Jan 2017 12:57:35 ECT
 ...
 * Automatically generated load test program.
 */
public class infrontWebsocket extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "infrontWebsocketExtParams.dat";      // input file name for very large parameter values
	 ...
	 */
	public infrontWebsocket(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0009 = "api.infrontservices.com";
		int    requestPort0009 = 443;
		String requestFile0009 = "/id/assets/styles.min.css";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 100790, 5);		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0010 = "/id/Content/site.css";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 1636, 5);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0011 = "/id/refresher.aspx";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 154, 5);		// Test [11] <- Index 11
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0012 = "api.infrontservices.com";
		int    requestPort0012 = 443;
		String requestFile0012 = "/id/Content/infront-logo_top_white.png";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 1779, 5);		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0015 = "api.infrontservices.com";
		int    requestPort0015 = 443;
		String requestFile0015 = "/id/login" +
				"?signin=1d85703de7fb8fb96421af07cef312f5";
		requestFile0015 = Lib.replaceTextPattern(requestFile0015, "/id/login?signin=1d85703de7fb8fb96421af07cef312f5", loginRedirect2, 0);		// search and replace request file fragment "/id/login?signin=1d85703de7fb8fb96421af07cef312f5" with variable 'loginRedirect2' on all occurrences
		...
		String requestContent0015 = 
				...
				"&password=HCgUfm";
		requestContent0015 = Lib.replaceTextPattern(requestContent0015, "9JkgAuw-nFNTBU5uE4IYZpGdD6HIyQZjgkaSyoulOAkazJLR_ZTO7LUTtWxn0_oArjctqOSDIApV5U1YVKi-hPxnGneRXEKjjnoUjuB6zfk", token, 0);		// search and replace content fragment "9JkgAuw-nFNTBU5uE4IYZpGdD6HIyQZjgkaSyoulOAkazJLR_ZTO7LUTtWxn0_oArjctqOSDIApV5U1YVKi-hPxnGneRXEKjjnoUjuB6zfk" with variable 'token' on all occurrences
		...
		// verify response: status code = 302, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {302}, null, null, null);		// Test [15] <- Index 15
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0016 = "api.infrontservices.com";
		int    requestPort0016 = 443;
		String requestFile0016 = "/id/connect/authorize" +
				"?client_id=infront.authorization" +
				"&redirect_uri=https%3A%2F%2Ftrader.infrontfinance.com%2Finfront%2Fno%2Flogin%2Fcallback" +
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div class='container page-authorizeresponse' ng-cloak");		// Test [16] <- Index 16
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0017 = "api.infrontservices.com";
		int    requestPort0017 = 443;
		String requestFile0017 = "/id/scripts/infront.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 1131, 5);		// Test [17] <- Index 17
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0018 = "/id/assets/app.FormPostResponse.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 155, 5);		// Test [18] <- Index 18
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0019 = "trader.infrontfinance.com";
		int    requestPort0019 = 443;
		String requestFile0019 = "/infront/no/login/callback";
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {302}, null, null, null);		// Test [19] <- Index 19
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0020 = "trader.infrontfinance.com";
		int    requestPort0020 = 443;
		String requestFile0020 = "/";
		...
		String requestHost0021 = "trader.infrontfinance.com";
		int    requestPort0021 = 443;
		String requestFile0021 = "/infront/no/";
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "Logging out and back in will usually solve this issue.");		// Test [21] <- Index 21
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0022 = "trader.infrontfinance.com";
		int    requestPort0022 = 443;
		String requestFile0022 = "/content/js/wt-languages.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 5481, 5);		// Test [22] <- Index 22
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0023 = "wtk.infrontservices.com";
		int    requestPort0023 = 443;
		String requestFile0023 = "/css/InfrontFramework-2.1.12.css";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 12212, 5);		// Test [23] <- Index 23
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0024 = "wtk.infrontservices.com";
		int    requestPort0024 = 443;
		String requestFile0024 = "/js/InfrontUI-2.1.12.min.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 189940, 5);		// Test [24] <- Index 24
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0025 = "wtk.infrontservices.com";
		int    requestPort0025 = 443;
		String requestFile0025 = "/languages/da.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 5019, 5);		// Test [25] <- Index 25
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0026 = "wtk.infrontservices.com";
		int    requestPort0026 = 443;
		String requestFile0026 = "/languages/no.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 4219, 5);		// Test [26] <- Index 26
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0027 = "wtk.infrontservices.com";
		int    requestPort0027 = 443;
		String requestFile0027 = "/js/CommonFramework-1.2.5.min.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 54452, 5);		// Test [27] <- Index 27
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [28] <- WEB ADMIN Index 33 ---
		String requestProt0028 = "https";
		String requestHost0028 = "trader.infrontfinance.com";
		int    requestPort0028 = 443;
		String requestFile0028 = "/content/css/wt.css" +
				"?v=20170111141504";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 13174, 5);		// Test [28] <- Index 28
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 10001, 5);		// Test [29] <- Index 29
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 5121, 5);		// Test [30] <- Index 30
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [31] <- WEB ADMIN Index 36 ---
		String requestProt0031 = "https";
		String requestHost0031 = "trader.infrontfinance.com";
		int    requestPort0031 = 443;
		String requestFile0031 = "/Content/gfx/favicons/favicon.ico";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/X-ICON", null, 10990, 5);		// Test [31] <- Index 31
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0032 = "trader.infrontfinance.com";
		int    requestPort0032 = 443;
		String requestFile0032 = "/content/css/perfect-scrollbar.css" +
				"?v=20170111141504";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 1076, 5);		// Test [32] <- Index 32
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0033 = "/content/js/countrycodes.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 1535, 5);		// Test [33] <- Index 33
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0034 = "trader.infrontfinance.com";
		int    requestPort0034 = 443;
		String requestFile0034 = "/content/js/perfect-scrollbar.jquery.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 12529, 5);		// Test [34] <- Index 34
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	boolean executePage_2_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0035 = "trader.infrontfinance.com";
		int    requestPort0035 = 443;
		String requestFile0035 = "/content/css/jquery.gridster.css" +
				"?v=20170111141504";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 1660, 5);		// Test [35] <- Index 35
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0036 = "/content/js/jquery.gridster.with-extras.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 36940, 5);		// Test [36] <- Index 36
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0037 = "trader.infrontfinance.com";
		int    requestPort0037 = 443;
		String requestFile0037 = "/Content/js/charting-TEMP/indicators/indicators.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 12478, 5);		// Test [37] <- Index 37
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0038 = "/Content/js/charting-TEMP/indicators/sma.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 1873, 5);		// Test [38] <- Index 38
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0039 = "/Content/js/charting-TEMP/indicators/ema.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 1914, 5);		// Test [39] <- Index 39
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0040 = "/Content/js/charting-TEMP/indicators/wma.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 1981, 5);		// Test [40] <- Index 40
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0041 = "/Content/js/charting-TEMP/indicators/atr.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 2238, 5);		// Test [41] <- Index 41
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0042 = "/Content/js/charting-TEMP/indicators/rsi.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 2731, 5);		// Test [42] <- Index 42
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0043 = "/Content/js/charting-TEMP/indicators/macd.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 3568, 5);		// Test [43] <- Index 43
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0044 = "/Content/js/charting-TEMP/indicators/bb.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 2490, 5);		// Test [44] <- Index 44
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0045 = "/Content/js/charting-TEMP/indicators/stochastic.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 2940, 5);		// Test [45] <- Index 45
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0046 = "/content/js/Storage.js" +
				"?v=20170111141504";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 1834, 5);		// Test [46] <- Index 46
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0047 = "/content/js/CustomWorkspaces.js" +
				"?v=20170111141504";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 11024, 5);		// Test [47] <- Index 47
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [48] <- WEB ADMIN Index 54 ---
		String requestProt0048 = "https";
		String requestHost0048 = "trader.infrontfinance.com";
		int    requestPort0048 = 443;
		String requestFile0048 = "/a/Storage/GetAllWebTraderSettings/-infront-no-";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 939, 5);		// Test [48] <- Index 48
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [49] <- WEB ADMIN Index 55 ---
		String requestProt0049 = "https";
		String requestHost0049 = "trader.infrontfinance.com";
		int    requestPort0049 = 443;
		String requestFile0049 = "/Content/gfx/ic_expand_more_white_24px.svg";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 208, 5);		// Test [49] <- Index 49
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [50] <- WEB ADMIN Index 56 ---
		String requestProt0050 = "https";
		String requestHost0050 = "trader.infrontfinance.com";
		int    requestPort0050 = 443;
		String requestFile0050 = "/content/gfx/infront-logo_top_white.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 1779
		// content test algorithm: check size +/- 5%, defined content size = 1779
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 1779, 5);		// Test [50] <- Index 50
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [51] <- WEB ADMIN Index 57 ---
		String requestProt0051 = "https";
		String requestHost0051 = "trader.infrontfinance.com";
		int    requestPort0051 = 443;
		String requestFile0051 = "/Content/gfx/ic_settings_white_18px.svg";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 836, 5);		// Test [51] <- Index 51
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 404, 5);		// Test [52] <- Index 52
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [53] <- WEB ADMIN Index 59 ---
		String requestProt0053 = "https";
		String requestHost0053 = "trader.infrontfinance.com";
		int    requestPort0053 = 443;
		String requestFile0053 = "/Content/gfx/ic_account_box_white_18px.svg";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 340, 5);		// Test [53] <- Index 53
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [54] <- WEB ADMIN Index 60 ---
		String requestProt0054 = "https";
		String requestHost0054 = "trader.infrontfinance.com";
		int    requestPort0054 = 443;
		String requestFile0054 = "/Content/gfx/ic_feedback_white_18px.svg";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 268, 5);		// Test [54] <- Index 54
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	boolean executePage_2_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [55] <- WEB ADMIN Index 61 ---
		String requestProt0055 = "https";
		String requestHost0055 = "trader.infrontfinance.com";
		int    requestPort0055 = 443;
		String requestFile0055 = "/Content/gfx/ic_exit_to_app_white_18px.svg";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 328, 5);		// Test [55] <- Index 55
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [56] <- WEB ADMIN Index 62 ---
		String requestProt0056 = "https";
		String requestHost0056 = "trader.infrontfinance.com";
		int    requestPort0056 = 443;
		String requestFile0056 = "/content/fonts/roboto-v15-latin-regular.woff2";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "FONT/WOFF2", null, 14584, 5);		// Test [56] <- Index 56
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [57] <- WEB ADMIN Index 63 ---
		String requestProt0057 = "https";
		String requestHost0057 = "trader.infrontfinance.com";
		int    requestPort0057 = 443;
		String requestFile0057 = "/content/fonts/roboto-v15-latin-300.woff2";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "FONT/WOFF2", null, 14524, 5);		// Test [57] <- Index 57
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [58] <- WEB ADMIN Index 64 ---
		String requestProt0058 = "https";
		String requestHost0058 = "trader.infrontfinance.com";
		int    requestPort0058 = 443;
		String requestFile0058 = "/content/fonts/roboto-v15-latin-700.woff2";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "FONT/WOFF2", null, 14552, 5);		// Test [58] <- Index 58
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 102, 5);		// Test [59] <- Index 59
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [60] <- WEB ADMIN Index 66 ---
		String requestProt0060 = "https";
		String requestHost0060 = "mws.infrontfinance.com";
		int    requestPort0060 = 443;
		String requestFile0060 = "/mws";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {101}, null, null, 0, 5);		// Test [60] <- Index 60
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 398
		// content test algorithm: check size +/- 5%, defined content size = 398
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 398, 5);		// Test [61] <- Index 61
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [62] <- WEB ADMIN Index 68 ---
		String requestProt0062 = "https";
		String requestHost0062 = "wtk.infrontservices.com";
		int    requestPort0062 = 443;
		String requestFile0062 = "/themes/terminal-2.1.12/arrow_white_down_small.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 941
		// content test algorithm: check size +/- 5%, defined content size = 941
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 941, 5);		// Test [62] <- Index 62
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [63] <- WEB ADMIN Index 69 ---
		String requestProt0063 = "https";
		String requestHost0063 = "wtk.infrontservices.com";
		int    requestPort0063 = 443;
		String requestFile0063 = "/themes/terminal-2.1.12/icon_settings-wheel_white_20px.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 773
		// content test algorithm: check size +/- 5%, defined content size = 773
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 773, 5);		// Test [63] <- Index 63
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [64] <- WEB ADMIN Index 70 ---
		String requestProt0064 = "https";
		String requestHost0064 = "wtk.infrontservices.com";
		int    requestPort0064 = 443;
		String requestFile0064 = "/themes/terminal-2.1.12/icon_delete-row_light.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 967
		// content test algorithm: check size +/- 5%, defined content size = 967
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 967, 5);		// Test [64] <- Index 64
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [65] <- WEB ADMIN Index 71 ---
		String requestProt0065 = "https";
		String requestHost0065 = "wtk.infrontservices.com";
		int    requestPort0065 = 443;
		String requestFile0065 = "/themes/terminal-2.1.12/ic_fullscreen_white_18dp_1x.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 998
		// content test algorithm: check size +/- 5%, defined content size = 998
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 998, 5);		// Test [65] <- Index 65
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [66] <- WEB ADMIN Index 72 ---
		String requestProt0066 = "https";
		String requestHost0066 = "wtk.infrontservices.com";
		int    requestPort0066 = 443;
		String requestFile0066 = "/themes/terminal-2.1.12/flags.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 59509
		// content test algorithm: check size +/- 5%, defined content size = 59509
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 59509, 5);		// Test [66] <- Index 66
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [67] <- WEB ADMIN Index 73 ---
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("infrontWebsocket", concurrentUsers);
		...
		performanceData.setInfoText("infrontWebsocket");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new infrontWebsocket());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				infrontWebsocket simulatedUser = new infrontWebsocket(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
