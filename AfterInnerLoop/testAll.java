...
public class testAll extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "testAllExtParams.dat";               // input file name for very large parameter values
	 ...
	 */
	public testAll(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestContent0007 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "testAll_RequestContent0007.txt").getFileDataAsString("UTF-8");		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("testAll", concurrentUsers);
		...
		performanceData.setInfoText("testAll");
		...
		try
		{
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_1);		// testAll_RequestContent0007.txt
		}
		catch (IOException ie)
		{
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new testAll());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				testAll simulatedUser = new testAll(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// --------------------------------------------------------------------------------
	public static final String REQUEST_FILE_CACHE_FILE_1 = "testAll_RequestContent0007.txt";		// must be zipped together with the compiled class of this load test program
