...
public class test5 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "test5ExtParams.dat";                 // input file name for very large parameter values
	 ...
	 */
	public test5(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		// execute inline script "testError Handling" at start of loop
		LoadtestInlineScriptContext inlineScriptContext_1487853049440 = new LoadtestInlineScriptContext("testError Handling", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_LOOP_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, -1, threadNumber, threadLoopCounter, socketPool, cookieHandler);
		InlineScriptExecutor inlineScriptExecutor_1487853049440 = new InlineScriptExecutor(getInlineScriptCode_1487853049440(), inlineScriptContext_1487853049440);
		log("Executing inline script \"" + inlineScriptContext_1487853049440.getScriptTitle() + "\"");
		inlineScriptExecutor_1487853049440.execute();		// execute inline script
		for (String stdoutLine : inlineScriptContext_1487853049440.getOutputStreamData())
			log(inlineScriptContext_1487853049440.getScriptTitle() + ": " + stdoutLine);
		for (String stderrLine : inlineScriptContext_1487853049440.getErrorStreamData())
			System.err.println(inlineScriptContext_1487853049440.getScriptTitle() + ": " + stderrLine);
		if (!inlineScriptExecutor_1487853049440.wasSuccessFulExecution())
		{
			if (inlineScriptContext_1487853049440.isScriptAbort())
			{
				log("*** INLINE SCRIPT \"" + inlineScriptContext_1487853049440.getScriptTitle() + "\" ABORTED / START OF CURRENT LOOP ABORTED ***");
				log("Abort Message = " + inlineScriptContext_1487853049440.getScriptAbortMessage());
				return false;		// the session failure rate is also increased
			}
			else
				throw new InlineScriptExecutionException("*** Execution of inline script \"" + inlineScriptContext_1487853049440.getScriptTitle() + "\" failed ***");
		}
		...
		// --- VIRTUAL PAGE #0 ---
		if (!executePage_0(totalLoopCounter, loopPluginContext, new InnerLoopContext(), new UrlLoopContext(), httpLogVectorObject))
			return false;
		...
		if (!executePage_1(totalLoopCounter, loopPluginContext, new InnerLoopContext(), new UrlLoopContext(), httpLogVectorObject))
		{
	 ...
	 */
	boolean executePage_0(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, UrlLoopContext urlLoopContext, Object httpLogVectorObject) throws Exception
	{
	 ...
	 *          false: method/loop failed.
	 */
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, UrlLoopContext urlLoopContext, Object httpLogVectorObject) throws Exception
	{
		// # Page #1: Start Page
		// # -------------------
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("test5", concurrentUsers);
		...
		performanceData.setInfoText("test5");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new test5());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				test5 simulatedUser = new test5(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "testError Handling"
	public static String getInlineScriptCode_1487853049440()
	{
		return "ON ERROR GOTO \"error Handler\"\r\n//abort(\"suicide attempt.\")\r\n\r\ns = 1\r\nprint \"s = \" + s\r\nprint KKJ //error\r\nprint \"s = \" + s\r\n\r\n\r\n\r\n\r\nHandler \"error Handler\":\r\n ON ERROR GOTO \"nestedHandler\"\r\n print \"some error handler code ....\"\r\n //print KKY                                     //error\r\n print \"some error handler code ____\"\r\nEnd Handler\r\n\r\n\r\nHandler \"nestedHandler\":\r\n  print \"wadda masllaa\"\r\n  //print KKZ                                    //error\r\n  print \"All OK\"\r\nEnd Handler\r\n";
	}
