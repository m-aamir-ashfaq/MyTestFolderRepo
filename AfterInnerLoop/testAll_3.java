...
// -----------------------------------------
//
// Source: testAll_3.java
// Date  : 03 Feb 2017 10:19:16 ECT
 ...
 * Automatically generated load test program.
 */
public class testAll_3 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "testAll_3ExtParams.dat";             // input file name for very large parameter values
	...
	private String clientID = null;                      // var declaration from web admin var handler: scope = per loop
	
	private Object wsplugin01 = null;                      // plug-in #1: "testWithMqtt" - definition from web admin var handler: construction scope = per user
	private static String wsplugin01testWithMqtt = null;          // internal - not used in this load test program, but used by the GUI - reflective information
	 ...
	 */
	public testAll_3(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		// initialize all per loop constructed web socket plug-ins
		try
		{
			// define object for websocket plug-in #1 and call web socket plug-in constructor
			Class wspluginClass0 = getPluginClassLoader().getPluginClass("testWithMqtt");
			wsplugin01 = wspluginClass0.newInstance();
			((GenericPluginInterface) wsplugin01).construct(loopPluginContext);
		}
	...
	private void endOfExecuteLoop(boolean loopPassed, HttpTestURL testURL, int threadStep, LoadtestPluginContext loopPluginContext) throws Exception
	{
		...
		// deconstruct all per loop constructed web socket plug-ins
		((GenericPluginInterface) wsplugin01).deconstruct(loopPluginContext);
	}
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [1] <- WEB ADMIN Index 4 ---
		String requestProt0001 = "http";
		String requestHost0001 = "piwik.baycix.de";
		int    requestPort0001 = 80;
		String requestFile0001 = "/piwik.php" +
				"?action_name=MQTT%20Websocket%20Client" +
				"&idsite=5" +
				"&rec=1" +
				"&r=585618" +
				"&h=15" +
				"&m=40" +
				"&s=12" +
				"&url=http%3A%2F%2Fwww.hivemq.com%2Fdemos%2Fwebsocket-client%2F" +
				"&_id=990af0026b6494e2" +
				"&_idts=1480926241" +
				"&_idvc=13" +
				"&_idn=0" +
				"&_refts=0" +
		...
		// verify response: status code = 200, content type = "IMAGE/GIF", header text fragment = [verification disabled], recorded content size = 50
		// content test algorithm: check size +/- 5%, defined content size = 50
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/GIF", null, 50, 5);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		// --- HTTP REQUEST: Test [2] <- WEB ADMIN Index 5 ---
		// execute inline script "before5"
		inlineScriptContext = executeInlineScript_1485522449318(threadStep, totalLoopCounter, innerLoopContext);
		...
		String requestHost0002 = "broker.mqttdashboard.com";
		int    requestPort0002 = 8000;
		String requestFile0002 = "/mqtt";
		String requestHeader0002 = "GET " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: 3H0KQ3pp82H1l1nG2L0MTA==\r\n" + 
		...
		webSocketData = getWebSocketData_1484664015930();

		webSocketPluginsForURL = new WebSocketPluginInterface[1];

		((GenericPluginInterface) wsplugin01).setInputParameter(0, clientID);			// set plug-in input parameter
		...
		// --- HTTP REQUEST: Test [3] <- WEB ADMIN Index 9 ---
		// execute inline script "before9"
		inlineScriptContext = executeInlineScript_1485522470451(threadStep, totalLoopCounter, innerLoopContext);
		if (inlineScriptContext.isScriptAbort())
		{
			log("*** INLINE SCRIPT \"" + inlineScriptContext.getScriptTitle() + "\" ABORTED / CURRENT LOOP ABORTED ***");
			log("Abort Message = " + inlineScriptContext.getScriptAbortMessage());
		...
		String requestHost0003 = "shuttle3";
		int    requestPort0003 = 8000;
		String requestFile0003 = "/mqtt";
		String requestHeader0003 = "GET " + requestFile0003 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Protocol: mqttv3.1\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: IuafH+a/tZB9D3JlOFQ7lg==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		webSocketData = getWebSocketData_1484664054427();
		...
		// --- HTTP REQUEST: Test [4] <- WEB ADMIN Index 19 ---
		// execute inline script "before19"
		inlineScriptContext = executeInlineScript_1485524244876(threadStep, totalLoopCounter, innerLoopContext);
		if (inlineScriptContext.isScriptAbort())
		{
			log("*** INLINE SCRIPT \"" + inlineScriptContext.getScriptTitle() + "\" ABORTED / CURRENT LOOP ABORTED ***");
			log("Abort Message = " + inlineScriptContext.getScriptAbortMessage());
		...
		String requestHost0004 = "test.mosquitto.org";
		int    requestPort0004 = 8080;
		String requestFile0004 = "/mqtt";
		String requestHeader0004 = "GET " + requestFile0004 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: test.mosquitto.org:8080\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n" + 
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Sec-WebSocket-Version: 13\r\n" + 
				"origin: http://mitsuruog.github.io\r\n" + 
				"Sec-WebSocket-Protocol: mqttv3.1\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: HhoeinKIOBsMLnUR0RoYNw==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		webSocketData = getWebSocketData_1484664091570();
		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, null, null));
		...
		// --- HTTP REQUEST: Test [5] <- WEB ADMIN Index 20 ---
		// execute inline script "before20"
		inlineScriptContext = executeInlineScript_1485522511569(threadStep, totalLoopCounter, innerLoopContext);
		if (inlineScriptContext.isScriptAbort())
		{
			log("*** INLINE SCRIPT \"" + inlineScriptContext.getScriptTitle() + "\" ABORTED / CURRENT LOOP ABORTED ***");
			log("Abort Message = " + inlineScriptContext.getScriptAbortMessage());
		...
		String requestHost0005 = "test.mosquitto.org";
		int    requestPort0005 = 8080;
		String requestFile0005 = "/mqtt";
		String requestHeader0005 = "GET " + requestFile0005 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Protocol: mqttv3.1\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: 0uiTmrgzz1SdFv8qoh34kQ==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		webSocketData = getWebSocketData_1484664106436();
		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, null, null));
		...
		String requestHost0006 = "test.mosquitto.org";
		...
		// verify response: status code = 404, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 293
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {404}, null, null, null);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0007 = "test.mosquitto.org";
		int    requestPort0007 = 8080;
		String requestFile0007 = "/mqtt";
		String requestHeader0007 = "GET " + requestFile0007 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Sec-WebSocket-Version: 13\r\n" + 
				"origin: http://test.mosquitto.org\r\n" + 
				"Sec-WebSocket-Protocol: mqttv3.1\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: gWXR+IxpxeFXEZfCtQWhYw==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
		...
		webSocketData = getWebSocketData_1484664128884();
		testURL.setWebSocketContext(new HttpTestWebsocketContext(webSocketData, null, null));
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("testAll_3", concurrentUsers);
		...
		performanceData.setInfoText("testAll_3");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new testAll_3());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				testAll_3 simulatedUser = new testAll_3(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "before19"
	public static String getInlineScriptCode_1485524244876()
	{
		return "clientID = generateGUID(\"clientId-xxxxxxxxxx\")";
	}
	
	// source code of inline script "before20"
	public static String getInlineScriptCode_1485522511569()
	{
		return "clientID = generateGUID(\"clientId-xxxxxxxxxx\")";
	}
	
	// source code of inline script "before5"
	public static String getInlineScriptCode_1485522449318()
	{
		return "clientID = generateGUID(\"clientId-xxxxxxxxxx\")";
	}
	
	// source code of inline script "before9"
	public static String getInlineScriptCode_1485522470451()
	{
		return "clientID = generateGUID(\"clientId-xxxxxxxxxx\")";
	}
	...
	/**
	* Execute the inline script "before19"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1485524244876(int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("before19", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_LOOP_ITEM_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler);
		inlineScriptContext.setThreadStep(threadStep);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1485524244876(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("clientID", Lib.nullToBlank(clientID), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("clientID", Lib.nullToBlank(clientID), 1);		// note: parameter no. 3 is the scope of the var
	...
	/**
	* Execute the inline script "before20"
	* 
	* @param threadStep 	the current execution step of the simulated user (page break or URL call)
	* @param totalLoopCounter 	the total number of executed loops - counted overall simulated users
	* @param innerLoopContext 	the context of the current inner loop, or null if the inline script in not executed inside an inner loop
	...
	* @return the context of the inline script
	*/
	public LoadtestInlineScriptContext executeInlineScript_1485522511569(int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("before20", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_LOOP_ITEM_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler);
		inlineScriptContext.setThreadStep(threadStep);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1485522511569(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("clientID", Lib.nullToBlank(clientID), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("clientID", Lib.nullToBlank(clientID), 1);		// note: parameter no. 3 is the scope of the var
	...
	/**
	* Execute the inline script "before5"
	* 
	* @param threadStep 	the current execution step of the simulated user (page break or URL call)
	* @param totalLoopCounter 	the total number of executed loops - counted overall simulated users
	* @param innerLoopContext 	the context of the current inner loop, or null if the inline script in not executed inside an inner loop
	...
	* @return the context of the inline script
	*/
	public LoadtestInlineScriptContext executeInlineScript_1485522449318(int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("before5", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_LOOP_ITEM_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler);
		inlineScriptContext.setThreadStep(threadStep);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1485522449318(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("clientID", Lib.nullToBlank(clientID), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("clientID", Lib.nullToBlank(clientID), 1);		// note: parameter no. 3 is the scope of the var
	...
	/**
	* Execute the inline script "before9"
	* 
	* @param threadStep 	the current execution step of the simulated user (page break or URL call)
	* @param totalLoopCounter 	the total number of executed loops - counted overall simulated users
	* @param innerLoopContext 	the context of the current inner loop, or null if the inline script in not executed inside an inner loop
	...
	* @return the context of the inline script
	*/
	public LoadtestInlineScriptContext executeInlineScript_1485522470451(int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("before9", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_LOOP_ITEM_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler);
		inlineScriptContext.setThreadStep(threadStep);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1485522470451(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("clientID", Lib.nullToBlank(clientID), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("clientID", Lib.nullToBlank(clientID), 1);		// note: parameter no. 3 is the scope of the var
	...
	public WebSocketData getWebSocketData_1484664015930()
	{
		...
		String payLoad_6 = "{\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"UNSUBSCRIBE\",\"unsubscribe_topic\":[\"testtopic/hive\"]}";
		String payLoad_8 = "{\"DUP\":false,\"QoS\":0,\"retain\":false,\"type\":\"DISCONNECT\"}";
		String payLoad_9 = "";
		...
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484664026181L,1486113532477L , payLoad_2, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484664037942L,1486113544242L , payLoad_4, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484664041155L,1486113547460L , payLoad_6, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1484664043053L,1486113549361L , payLoad_8, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1484664043054L,1486113549464L , payLoad_9, WebSocketFrame.MQTT ));
	
		return webSocketData;
	}
	
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1484664054427()
	{
		...
		String payLoad_6 = "{\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"UNSUBSCRIBE\",\"unsubscribe_topic\":[\"testtopic/shuttle\"]}";
		String payLoad_8 = "{\"DUP\":false,\"QoS\":0,\"retain\":false,\"type\":\"DISCONNECT\"}";
		String payLoad_9 = "";
	...
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1484664091570()
	{
		...
		String payLoad_5 = "{\"DUP\":false,\"QoS\":1,\"messageID\":-27480,\"retain\":false,\"type\":\"UNSUBSCRIBE\",\"unsubscribe_topic\":[\"mitsuruog\"]}";
		String payLoad_6 = "{\"DUP\":false,\"QoS\":0,\"retain\":false,\"type\":\"DISCONNECT\"}";
		String payLoad_7 = "";
	...
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1484664106436()
	{
		...
		String payLoad_4 = "{\"DUP\":false,\"QoS\":0,\"contents\":\"Hello mitsuruog :)\",\"publish_topic\":\"mitsuruog\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_6 = "{\"DUP\":false,\"QoS\":1,\"messageID\":-27134,\"retain\":false,\"type\":\"UNSUBSCRIBE\",\"unsubscribe_topic\":[\"mitsuruog\"]}";
		String payLoad_8 = "{\"DUP\":false,\"QoS\":0,\"retain\":false,\"type\":\"DISCONNECT\"}";
		String payLoad_9 = "";
	...
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1484664128884()
	{
		...
		String payLoad_6 = "{\"DUP\":false,\"QoS\":0,\"contents\":\"sdfg\",\"publish_topic\":\"test/mosquitto/org\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_8 = "{\"DUP\":false,\"QoS\":1,\"messageID\":3,\"retain\":false,\"type\":\"UNSUBSCRIBE\",\"unsubscribe_topic\":[\"test/mosquitto/org\"]}";
		String payLoad_10 = "{\"DUP\":false,\"QoS\":0,\"retain\":false,\"type\":\"DISCONNECT\"}";
		String payLoad_11 = "";
