...
// -----------------------------------------
//
// Source: testPlain.java
// Date  : 20 Jan 2017 12:42:02 ECT
 ...
 * Automatically generated load test program.
 */
public class testPlain extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "testPlainExtParams.dat";             // input file name for very large parameter values
	 ...
	 */
	public testPlain(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [1] <- WEB ADMIN Index 36 ---
		String requestProt0001 = "http";
		String requestHost0001 = "script.crazyegg.com";
		int    requestPort0001 = 80;
		String requestFile0001 = "/pages/scripts/0037/0800.js" +
				"?412452";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, 0, 5);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		// --- HTTP REQUEST: Test [2] <- WEB ADMIN Index 38 ---
		String requestProt0002 = "http";
		String requestHost0002 = "tag.getdrip.com";
		int    requestPort0002 = 80;
		String requestFile0002 = "/6914321.js";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 8, 5);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [3] <- WEB ADMIN Index 40 ---
		String requestProt0003 = "http";
		String requestHost0003 = "assets.zendesk.com";
		int    requestPort0003 = 80;
		String requestFile0003 = "/embeddable_framework/update.html" +
				"?t=24747160";
		...
		// content test algorithm: check size +/- 5%, defined content size = 198
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 198, 5);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
		...
		String requestFile0005 = "/embeddable/config";
		...
		// --- HTTP REQUEST: Test [6] <- WEB ADMIN Index 45 ---
		...
		// --- HTTP REQUEST: Test [7] <- WEB ADMIN Index 46 ---
		...
		String requestContent0008 = 
				"{\"url\":\"http://www.websocket.org/echo.html\",\"buid\":\"6342f66c687e3ac852a76163a2f6b082\",\"suid\":\"56a52aed8b3e33bd0fd71a7bf1662fd6\",\"version\":\"b6dcc71\",\"timestamp\":\"2017-01-19T12:40:17.558Z\",\"pageView\":{\"referrer\":\"http://www.websocket.org/echo.html\",\"time\":965,\"loadTime\":79.359142,\"navigatorLanguage\":\"en-GB\",\"pageTitle\":\"websocket.org Echo Test - Powered by Kaazing\",\"userAgent\":\"Mozilla/5.0 (Macintosh; Intel Mac OS X 10.12; rv:50.0) Gecko/20100101 Firefox/50.0\",\"helpCenterDedup\":false}}";
		...
		// --- HTTP REQUEST: Test [9] <- WEB ADMIN Index 48 ---
		String requestProt0009 = "https";
		String requestHost0009 = "v2.zopim.com";
		int    requestPort0009 = 443;
		String requestFile0009 = "/bin/v/widget_v2.182.js";
		...
		// verify response: status code = 200, content type = "APPLICATION/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 238575
		// content test algorithm: check size +/- 5%, defined content size = 238575
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 238575, 5);		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [10] <- WEB ADMIN Index 49 ---
		...
		String requestHeader0010 = "GET " + requestFile0010 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: FAnOVyfP3YMq7bYY5wtj3g==\r\n" + 
		...
		webSocketData = getWebSocketData_1484829618608();
		...
		// --- HTTP REQUEST: Test [11] <- WEB ADMIN Index 50 ---
		String requestProt0011 = "https";
		String requestHost0011 = "jp02.zopim.com";
		int    requestPort0011 = 443;
		String requestFile0011 = "/s/W/ws/i7tHHSRzSmJtbZ78/c/1484829617992";
		String requestHeader0011 = "GET " + requestFile0011 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Origin: http://www.websocket.org\r\n" + 
				"Sec-WebSocket-Extensions: permessage-deflate\r\n" + 
				"Sec-WebSocket-Key: c+Vl/Ooa7RTHjFWWnXuBQA==\r\n" + 
				"Connection: Keep-Alive, Upgrade\r\n" + 
				"Pragma: no-cache\r\n" + 
				"Cache-Control: no-cache\r\n" + 
				"Upgrade: websocket\r\n" + 
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("testPlain", concurrentUsers);
		...
		performanceData.setInfoText("testPlain");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new testPlain());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				testPlain simulatedUser = new testPlain(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	public WebSocketData getWebSocketData_1484829618608()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1484829618716L);
		webSocketData.setConnectionStopMillis(1484829680174L);
		...
		String payLoad_4 = "wait for ping pong";
		String payLoad_6 = "Rock it with HTML5 WebSocket 3";
		String payLoad_8 = "";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1484829622076L,1484829622077L  , payLoad_0, WebSocketFrame.PLAIN  ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1484829625797L,1484829625798L  , payLoad_2, WebSocketFrame.PLAIN  ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1484829636554L,1484829636555L  , payLoad_4, WebSocketFrame.PLAIN  ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 1 , 1484829675724L,1484829675725L  , payLoad_6, WebSocketFrame.PLAIN  ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1484829680066L,1484829680067L  , payLoad_8, WebSocketFrame.PLAIN  ));
