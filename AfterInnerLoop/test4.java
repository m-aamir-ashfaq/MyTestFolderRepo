 ...
 * Automatically generated load test program.
 */
public class test4 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "test4ExtParams.dat";                 // input file name for very large parameter values
	...
	private String kkj = null;                           // var declaration from web admin var handler: scope = per loop
	 ...
	 */
	public test4(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// execute inline script "generateCID"
		inlineScriptContext = executeInlineScript_1486130040084(threadStep, totalLoopCounter, innerLoopContext);
		...
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: 4ltVE3I09gMcNnnPr2tjuQ==\r\n" + 
		...
		webSocketData = getWebSocketData_1486129818934();
		...
		{
			// extract var 'kkj'
			kkj = Lib.extractTextToken( testURL.getWebSocketContext().getWebSocketRecordedData().getWebSocketFramesWithoutPingPong().get(1).getPayloadString(), 4, "<>'=&\"");		// recorded value = Connection Accepted
			log("<<< kkj = " + kkj);
		}
		catch (Exception e) { kkj = null; log(e); }
		if (kkj == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'kkj' from websocket frame pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0002 = "/submit/telemetry/72bf463c-3339-ce4f-87b2-257dbb463dab/main/Firefox/50.1.0/release/20161208153507" +
				"?v=4";
		String requestContent0002 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "test4_RequestContent0002.txt").getFileDataAsString("UTF-8");		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
		...
		String requestHeader0002 = "POST " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Date: Fri, 03 Feb 2017 13:50:32 GMT\r\n" + 
		...
		// --- HTTP REQUEST: Test [3] <- WEB ADMIN Index 6 ---
		String requestProt0003 = "https";
		String requestHost0003 = "incoming.telemetry.mozilla.org";
		int    requestPort0003 = 443;
		String requestFile0003 = "/submit/telemetry/7dc71a73-2f33-194c-b4b5-123e5b8512a8/main/Firefox/50.1.0/release/20161208153507" +
				"?v=4";
		String requestContent0003 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "test4_RequestContent0003.txt").getFileDataAsString("UTF-8");		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
		...
		// verify response: status code = 200, content type = "TEXT/PLAIN", header text fragment = [verification disabled], recorded content size = 20
		// content test algorithm: check size +/- 5%, defined content size = 20
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/PLAIN", null, 20, 5);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("test4", concurrentUsers);
		...
		performanceData.setInfoText("test4");
		...
		try
		{
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_1);		// test4_RequestContent0002.txt
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_2);		// test4_RequestContent0003.txt
		}
		catch (IOException ie)
		{
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new test4());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				test4 simulatedUser = new test4(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// --------------------------------------------------------------------------------
	public static final String REQUEST_FILE_CACHE_FILE_1 = "test4_RequestContent0002.txt";		// must be zipped together with the compiled class of this load test program
	public static final String REQUEST_FILE_CACHE_FILE_2 = "test4_RequestContent0003.txt";		// must be zipped together with the compiled class of this load test program
	
	
	// source code of inline script "generateCID"
	public static String getInlineScriptCode_1486130040084()
	{
	...
	/**
	* Execute the inline script "generateCID"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1486130040084(int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("generateCID", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_LOOP_ITEM_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler);
		inlineScriptContext.setThreadStep(threadStep);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1486130040084(), inlineScriptContext);
	...
	public WebSocketData getWebSocketData_1486129818934()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1486129819096L);
		webSocketData.setConnectionStopMillis(1486129853074L);
		webSocketData.setSecProtocol("mqtt");
		String payLoad_0 = "{\"DUP\":false,\"QoS\":0,\"userNameFlag\":0,\"last_will_Flag\":0,\"keep_alive\":60,\"retain\":false,\"client_id\":\"clientId-Zbr1LGopid\",\"clean_session\":true,\"type\":\"CONNECT\",\"passwordFlag\":0,\"protocol_name\":\"MQIsdp\",\"protocol_version\":3}";
		String payLoad_2 = "{\"subscribe_topics\":[{\"subscribe_topic\":\"testtopic/#\",\"subscribe_QoS\":1}],\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"SUBSCRIBE\"}";
		String payLoad_4 = "{\"DUP\":false,\"QoS\":0,\"contents\":\"test message\",\"publish_topic\":\"testtopic/amir\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_6 = "{\"DUP\":false,\"QoS\":0,\"contents\":\"test message 2\",\"publish_topic\":\"testtopic/moona\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_8 = "{\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"UNSUBSCRIBE\",\"unsubscribe_topic\":[\"testtopic/#\"]}";
		...
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1486129819099L,1486129819100L , payLoad_0, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1486129827517L,1486129827517L , payLoad_2, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1486129836081L,1486129836082L , payLoad_4, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1486129846251L,1486129846252L , payLoad_6, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1486129850588L,1486129850589L , payLoad_8, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1486129852897L,1486129852898L , payLoad_10, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1486129852898L,1486129852899L , payLoad_11, WebSocketFrame.MQTT ));
