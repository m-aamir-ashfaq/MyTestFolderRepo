...
public class sree implements WebSocketPluginInterface
{
	...
	public WebSocketFrame beforeMessageSent(Object context,WebSocketFrame wsFrame)
	{
		...
		if(extracted)
		{
			logVector.log("ws_plugin_"+getPluginName()+" already extracted");
			logVector.log("ws_plugin_"+getPluginName()+" already extracted"+wsFrame.getPayloadString().length());
			if(wsFrame.getPayloadString().length() > 0)
			{
				wsFrame.setPayloadString(payLoadToTransfer);
				logVector.log("ws_plugin_"+getPluginName()+" already extracted"+payLoadToTransfer);
			}
		}
...
payLoadToTransfer = wsFrame.getPayloadString();
	extracted = true;
}
		// ^^^ --- sample code --- ^^^
	}
