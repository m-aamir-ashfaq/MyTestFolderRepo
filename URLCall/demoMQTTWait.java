...
// -----------------------------------------
//
// Source: demoMQTTWait.java
// Date  : 24 Mar 2017 13:56:11 ECT
 ...
 * Automatically generated load test program.
 */
public class demoMQTTWait extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "demoMQTTWaitExtParams.dat";          // input file name for very large parameter values
	 ...
	 */
	public demoMQTTWait(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestContent0001 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "demoMQTTWait_RequestContent0001.txt").getFileDataAsString("UTF-8");		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
		...
		// execute request
		testURL = new HttpTestURL(requestProt0002, requestHost0002, requestPort0002, requestHeader0002, null, 900000, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("demoMQTTWait", concurrentUsers);
		...
		performanceData.setInfoText("demoMQTTWait");
		...
		try
		{
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_1);		// demoMQTTWait_RequestContent0001.txt
		}
		catch (IOException ie)
		{
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new demoMQTTWait());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				demoMQTTWait simulatedUser = new demoMQTTWait(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// --------------------------------------------------------------------------------
	public static final String REQUEST_FILE_CACHE_FILE_1 = "demoMQTTWait_RequestContent0001.txt";		// must be zipped together with the compiled class of this load test program
	...
	public WebSocketData getWebSocketData_1489760612373()
	{
		...
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760612409L,1489760612443L , payLoad_0, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760620384L,1489760620390L , payLoad_2, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760629805L,1489760629807L , payLoad_4, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760633085L,1489760633088L , payLoad_6, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760636777L,1489760636778L , payLoad_8, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489760638557L,1489760638561L , payLoad_10, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1489760638562L,1489760638565L , payLoad_11, WebSocketFrame.MQTT ));
	

		 // Add all new Frames and delays 'ProxySnifferVarSourceWebSocketNewFrame'.
	
		long wait_1 = 30000;
		int offsetFrame_1 = 8500;
		int terminatingType_1 = 2;
		String terminatingPayload_1 = "";
		terminatingPayload_1 = "moonaJani";
		webSocketData.addWait(wait_1 , offsetFrame_1 , terminatingType_1 , terminatingPayload_1);
