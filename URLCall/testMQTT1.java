...
// -----------------------------------------
//
// Source: testMQTT1.java
// Date  : 17 Mar 2017 14:36:06 ECT
 ...
 * Automatically generated load test program.
 */
public class testMQTT1 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "testMQTT1ExtParams.dat";             // input file name for very large parameter values
	...
	private static String cID = "";                                              // var declaration from web admin var handler: scope = global
	...
	private static Object wsplugin01 = null;                      // plug-in #1: "wsTestPlugin" - definition from web admin var handler: construction scope = global
	private static String wsplugin01wsTestPlugin = null;          // internal - not used in this load test program, but used by the GUI - reflective information
	 ...
	 */
	public testMQTT1(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0001 = "/submit/telemetry/ae7d73c8-9315-d744-87c4-6ce1098b23e4/main/Firefox/52.0/release/20170302120751" +
				"?v=4";
		String requestContent0001 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "testMQTT1_RequestContent0001.txt").getFileDataAsString("UTF-8");		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
		...
		String requestHeader0001 = "POST " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Date: Fri, 17 Mar 2017 13:01:50 GMT\r\n" + 
		...
		String requestHeader0002 = "GET " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Sec-WebSocket-Key: TiDWzzqiccPOSJnRHem5yg==\r\n" + 
		...
		webSocketData = getWebSocketData_1489755713808();
		...
		String requestHost0003 = "cdnjs.cloudflare.com";
		int    requestPort0003 = 80;
		String requestFile0003 = "/ajax/libs/fancybox/2.1.5/fancybox_overlay.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 76
		// content test algorithm: check size +/- 5%, defined content size = 76
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 76, 5);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
		...
		String requestHost0004 = "www.hivemq.com";
		int    requestPort0004 = 80;
		String requestFile0004 = "/demos/websocket-client/assets/jquery.minicolors.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 77459
		// content test algorithm: check size +/- 5%, defined content size = 77459
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 77459, 5);		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0005 = "cdnjs.cloudflare.com";
		int    requestPort0005 = 80;
		String requestFile0005 = "/ajax/libs/fancybox/2.1.5/fancybox_sprite.png";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 1362, 5);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0006 = "detectportal.firefox.com";
		int    requestPort0006 = 80;
		String requestFile0006 = "/success.txt";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/PLAIN", null, 8, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("testMQTT1", concurrentUsers);
		...
		performanceData.setInfoText("testMQTT1");
		...
		try
		{
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_1);		// testMQTT1_RequestContent0001.txt
		}
		catch (IOException ie)
		{
		...
		{
			System.out.println("global var <<< cID = " + cID);
			System.out.println();
		}
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new testMQTT1());
		
		try
		{
			// define object for web socket plug-in #1 and call web socket plug-in constructor
			Class wspluginClass0 = getPluginClassLoader().getPluginClass("wsTestPlugin");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				testMQTT1 simulatedUser = new testMQTT1(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// --------------------------------------------------------------------------------
	public static final String REQUEST_FILE_CACHE_FILE_1 = "testMQTT1_RequestContent0001.txt";		// must be zipped together with the compiled class of this load test program
	
	
	// source code getting the webSocket context.
	public WebSocketData getWebSocketData_1489755713808()
	{

		WebSocketData webSocketData = new WebSocketData();
		webSocketData.setConnectionStartMillis(1489755713878L);
		webSocketData.setConnectionStopMillis(1489755732090L);
		webSocketData.setSecProtocol("mqtt");
		String payLoad_0 = "{\"DUP\":false,\"QoS\":0,\"userNameFlag\":0,\"last_will_Flag\":0,\"keep_alive\":60,\"retain\":false,\"client_id\":\"clientId-4emgcXIqcq\",\"clean_session\":true,\"type\":\"CONNECT\",\"passwordFlag\":0,\"protocol_name\":\"MQIsdp\",\"protocol_version\":3}";
		String payLoad_2 = "{\"subscribe_topics\":[{\"subscribe_topic\":\"testtopic/testApica\",\"subscribe_QoS\":2}],\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"SUBSCRIBE\"}";
		String payLoad_4 = "{\"DUP\":false,\"QoS\":0,\"contents\":\"msg 1\",\"publish_topic\":\"testtopic/testApica\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_6 = "{\"DUP\":false,\"QoS\":0,\"contents\":\"msg 2\",\"publish_topic\":\"testtopic/testApica\",\"retain\":false,\"type\":\"PUBLISH\"}";
		String payLoad_8 = "{\"DUP\":false,\"QoS\":1,\"messageID\":1,\"retain\":false,\"type\":\"UNSUBSCRIBE\",\"unsubscribe_topic\":[\"testtopic/testApica\"]}";
		String payLoad_10 = "{\"DUP\":false,\"QoS\":0,\"retain\":false,\"type\":\"DISCONNECT\"}";
		String payLoad_11 = "";


		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489755713898L,1489756307785L , payLoad_0, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489755717145L,1489756311038L , payLoad_2, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489755723406L,1489756317308L , payLoad_4, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489755726772L,1489756320675L , payLoad_6, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489755729506L,1489756323411L , payLoad_8, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 2 , 1489755732054L,1489756325963L , payLoad_10, WebSocketFrame.MQTT ));
		webSocketData.addWebSocketFrame(new WebSocketFrame( true  , true  , false , (byte) 8 , 1489755732056L,1489756326070L , payLoad_11, WebSocketFrame.MQTT ));
	

		 // Add all new Frames and delays 'ProxySnifferVarSourceWebSocketNewFrame'.
	
		String topicFrame_1 = "topicAmir";
		int qosFrame_1 = 0;
		long offsetFrame_1 = 4000;
		webSocketData.addWebSocketSubscribeFrame(topicFrame_1 , qosFrame_1 , offsetFrame_1);
	
		String topicFrame_2 = "topicAmir";
		int qosFrame_2 = 1;
		boolean retainFrame_2 = false;
		String contentFrame_2 = " Had this issue too, looks like my virtual host (through nginx) wasn't set up properly to accept the Upgrade header. @tylercb's fix from about a year ago fixed it^";
		long offsetFrame_2 = 6000;
		webSocketData.addWebSocketPublishFrame(topicFrame_2 , qosFrame_2 , retainFrame_2 , contentFrame_2 , offsetFrame_2);
	
		long delayFrame_3 = 90000;
		int offsetFrame_3 = 6000;
		webSocketData.addWebSocketDelay(delayFrame_3 , offsetFrame_3);
