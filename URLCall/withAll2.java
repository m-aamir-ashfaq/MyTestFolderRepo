...
// -----------------------------------------
//
// Source: withAll2.java
// Date  : 12 Mar 2017 01:14:10 ECT
 ...
 * Automatically generated load test program.
 */
public class withAll2 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "withAll2ExtParams.dat";              // input file name for very large parameter values
	 ...
	 */
	public withAll2(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log("# Page #2: after");
		log("# --------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: after", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0001 = "/submit/telemetry/33b12a21-c8e8-3e4d-ac54-a05b85ed8de2/main/Firefox/51.0.1/release/20170125094131" +
				"?v=4";
		String requestContent0001 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "withAll2_RequestContent0001.txt").getFileDataAsString("UTF-8");		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
		...
		String requestHeader0001 = "POST " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Date: Thu, 09 Mar 2017 11:16:54 GMT\r\n" + 
		...
		String requestHost0002 = "www.plain.com";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, 0, 5);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		log();
		log("# title: Unicorn! &middot; GitHub");
		String requestProt0003 = "http";
		String requestHost0003 = "www.json.com";
		...
		// verify response: status code = 403, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 39851
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {403}, null, null, null);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0004 = "www.xml.com";
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {301}, null, null, null);		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0005 = "www.xml.com";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 3843
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "Developing open business information exchange documents");		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0006 = "www.form.com";
		...
		// verify response: status code = 301, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {301}, null, null, null);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		log();
		log("# title: Page Not Found - Multicom");
		String requestProt0007 = "http";
		String requestHost0007 = "www.multi.com";
		int    requestPort0007 = 80;
		String requestFile0007 = "/";
		byte[][] requestContent0007 = new byte[14][];
			requestContent0007[0] = "-----------------------------134041521812551118962081255238\r\n".getBytes();
			requestContent0007[1] = "Content-Disposition: form-data; name=\"name\"\r\n".getBytes();
			requestContent0007[2] = "\r\n".getBytes();
			requestContent0007[3] = "amir\r\n".getBytes();
			requestContent0007[4] = "-----------------------------134041521812551118962081255238\r\n".getBytes();
			requestContent0007[5] = "Content-Disposition: form-data; name=\"work\"\r\n".getBytes();
			requestContent0007[6] = "\r\n".getBytes();
			requestContent0007[7] = "apica\r\n".getBytes();
			requestContent0007[8] = "-----------------------------134041521812551118962081255238\r\n".getBytes();
			requestContent0007[9] = "Content-Disposition: form-data; name=\"pic\"; filename=\"download.jpeg\"\r\n".getBytes();
			requestContent0007[10] = "Content-Type: image/jpeg\r\n".getBytes();
			requestContent0007[11] = "\r\n".getBytes();
			requestContent0007[12] = Lib.mergeByteArrays(contentFromDiskFile("download.jpeg"), "\r\n".getBytes());
			requestContent0007[13] = "-----------------------------134041521812551118962081255238--\r\n".getBytes();
		byte[] binaryRequestContent0007 = Lib.mergeByteArrays(requestContent0007);
		String requestHeader0007 = "POST " + requestFile0007 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Content-Length: " + binaryRequestContent0007.length + "\r\n" + 		// Content-Length: 136807
				"Content-Type: multipart/form-data; boundary=---------------------------134041521812551118962081255238\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0007, requestHost0007, requestPort0007, requestHeader0007, binaryRequestContent0007, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		// verify response: status code = 404, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 25282
		// content test algorithm: none - no content check
		...
		String requestFile0008 = "/submit/telemetry/e267b36b-0594-0344-9edc-e50b82c73764/main/Firefox/51.0.1/release/20170125094131" +
				"?v=4";
		String requestContent0008 = 
				requestFileCache.readFile(getSymmetricEncryptContext(), "withAll2_RequestContent0008.txt").getFileDataAsString("UTF-8");		// note: consider duplicated definition of this file name near end of the load test program - used for ZIP-dialogue in Project Navigator
		...
		String requestHeader0008 = "POST " + requestFile0008 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Encoding: gzip, deflate, br\r\n" + 
				"Content-Type: application/json; charset=UTF-8\r\n" + 
				"Date: Fri, 10 Mar 2017 17:27:52 GMT\r\n" + 
				"Content-Encoding: gzip\r\n" + 
		...
		// verify response: status code = 200, content type = "TEXT/PLAIN", header text fragment = [verification disabled], recorded content size = 2
		// content test algorithm: check size +/- 5%, defined content size = 2
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/PLAIN", null, 2, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0010 = "fonts.gstatic.com";
		int    requestPort0010 = 443;
		String requestFile0010 = "/s/roboto/v15/d-6IYplOFocCacKzxwXSOJBw1xU1rKptJj_0jans920.woff2";
		...
		// verify response: status code = 200, content type = "FONT/WOFF2", header text fragment = [verification disabled], recorded content size = 10276
		// content test algorithm: check size +/- 5%, defined content size = 10276
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "FONT/WOFF2", null, 10276, 5);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0011 = "www.zauq.com";
		int    requestPort0011 = 80;
		String requestFile0011 = "/";
		byte[][] requestContent0011 = new byte[6][];
			requestContent0011[0] = "-----------------------------156536899248193918981937352\r\n".getBytes();
			requestContent0011[1] = "Content-Disposition: form-data; name=\"cv\"; filename=\"CVAmir.pdf\"\r\n".getBytes();
			requestContent0011[2] = "Content-Type: application/pdf\r\n".getBytes();
			requestContent0011[3] = "\r\n".getBytes();
			requestContent0011[4] = Lib.mergeByteArrays(contentFromDiskFile("CVAmir.pdf"), "\r\n".getBytes());
			requestContent0011[5] = "-----------------------------156536899248193918981937352--\r\n".getBytes();
		byte[] binaryRequestContent0011 = Lib.mergeByteArrays(requestContent0011);
		String requestHeader0011 = "POST " + requestFile0011 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-GB,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Content-Length: " + binaryRequestContent0011.length + "\r\n" + 		// Content-Length: 170512
				"Content-Type: multipart/form-data; boundary=---------------------------156536899248193918981937352\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0011, requestHost0011, requestPort0011, requestHeader0011, binaryRequestContent0011, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div class=\"collapse navbar-collapse navbar-right\"");		// Test [11] <- Index 11
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("withAll2", concurrentUsers);
		...
		final int MAX_THREAD_STEPS = 10;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("withAll2");
		...
		try
		{
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_1);		// withAll2_RequestContent0001.txt
			requestFileCache.readFile(getSymmetricEncryptContext(), REQUEST_FILE_CACHE_FILE_2);		// withAll2_RequestContent0008.txt
		}
		catch (IOException ie)
		{
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new withAll2());
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				withAll2 simulatedUser = new withAll2(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// --------------------------------------------------------------------------------
	public static final String REQUEST_FILE_CACHE_FILE_1 = "withAll2_RequestContent0001.txt";		// must be zipped together with the compiled class of this load test program
	public static final String REQUEST_FILE_CACHE_FILE_2 = "withAll2_RequestContent0008.txt";		// must be zipped together with the compiled class of this load test program
